...
class InviteOptions(usage.Options):
    ...
    optParameters = [
        (
            ...
            None,
            None,
            "How many shares are needed to reconstruct files from this node",
        ),
        (
            "shares-happy",
            None,
            None,
            "Distinct storage servers new node will upload shares to",
        ),
    ...
    def parseArgs(self, *args):
        if len(args) != 1:
            raise usage.UsageError("Provide a single argument: the new node's nickname")
...
def _send_config_via_wormhole(options, config):
    ...
    if not "abilities" in client_intro:
        print("No 'abilities' from client", file=err)
        defer.returnValue(1)
    if not "client-v1" in client_intro["abilities"]:
...
def invite(options):
    ...
    try:
        ...
        print(
            "Can't find introducer FURL for node '{}': {}".format(basedir, str(e)),
            file=err,
        )
        raise SystemExit(1)
    ...
    remote_config = {
        "shares-needed": options["shares-needed"]
        or config.get_config("client", "shares.needed"),
        "shares-total": options["shares-total"]
        or config.get_config("client", "shares.total"),
        "shares-happy": options["shares-happy"]
        or config.get_config("client", "shares.happy"),
        "nickname": nick,
...
subCommands = [
    ("invite", None, InviteOptions, "Invite a new node to this grid"),
]  # type: SubCommands
