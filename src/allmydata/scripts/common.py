...
class BasedirOptions(BaseOptions):
    ...
    optParameters = [
        [
            ...
            "C",
            None,
            "Specify which Tahoe base directory should be used. [default: %s]"
    ...
    def parseArgs(self, basedir=None):
        ...
        if root["node-directory"] and self["basedir"]:
            raise usage.UsageError(
                "The --node-directory (or -d) and --basedir (or -C) options cannot both be used."
            )
        if root["node-directory"] and basedir:
            raise usage.UsageError(
                "The --node-directory (or -d) option and a basedir argument cannot both be used."
            )
        if self["basedir"] and basedir:
            raise usage.UsageError(
                "The --basedir (or -C) option and a basedir argument cannot both be used."
            )
        ...
        if basedir:
            ...
            raise usage.UsageError(
                "No default basedir available, you must provide one with --node-directory, --basedir, or a basedir argument"
            )
        self["basedir"] = b
...
class NoDefaultBasedirOptions(BasedirOptions):
    ...
    def getSynopsis(self):
        return "Usage:  %s [global-options] %s [options] NODEDIR" % (
            self.command_name,
...
def get_alias(aliases, path_unicode, default):
    ...
    if colon == -1:
        ...
        if default not in aliases:
            raise UnknownAliasError(
                "No alias specified, and the default %s alias doesn't exist. "
    ...
    if alias not in aliases:
        raise UnknownAliasError(
            "Unknown alias %s, please create it with 'tahoe add-alias' or 'tahoe create-alias'."
            % quote_output(alias)
        )
...
def escape_path(path):
    ...
    result = str(
        b"/".join([urllib.parse.quote(s).encode("ascii") for s in segments]), "ascii"
    )
