...
BASE32CHAR_4bits = backwardscompat_bytes(
    b"[" + get_trailing_chars_without_lsbs(1) + b"]"
)
BASE32CHAR_3bits = backwardscompat_bytes(
    b"[" + get_trailing_chars_without_lsbs(2) + b"]"
)
BASE32CHAR_2bits = backwardscompat_bytes(
    b"[" + get_trailing_chars_without_lsbs(3) + b"]"
)
BASE32CHAR_1bits = backwardscompat_bytes(
...
BASE32STR_anybytes = backwardscompat_bytes(
    bytes(b"((?:%s{8})*") % (BASE32CHAR,)
    + bytes(b"(?:|%s|%s|%s|%s))")
    % (BASE32STR_1byte, BASE32STR_2bytes, BASE32STR_3bytes, BASE32STR_4bytes)
)
...
def init_s8():
    ...
    for lenmod8 in (
        ...
        if NUM_QS_LEGIT[lenmod8]:
            add_check_array(
                get_trailing_chars_without_lsbs(4 - (NUM_QS_TO_NUM_BITS[lenmod8] % 5)),
                s8,
            )
        else:
...
def could_be_base32_encoded(
    s, s8=s8, tr=bytes.translate, identitytranstable=identitytranstable, chars=chars
):
