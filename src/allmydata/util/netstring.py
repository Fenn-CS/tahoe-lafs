...
def split_netstring(
    data, numstrings, position=0, required_trailer=None
):  # type (bytes, init, int, Optional[bytes]) -> Tuple[List[bytes], int]
    ...
    if required_trailer is not None:
        if ((len(data) - position) != len(required_trailer)) or (
            data[position:] != required_trailer
        ):
