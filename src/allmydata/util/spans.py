...
class Spans(object):
    ...
    def add(self, start, length):
        ...
        for i, (s_start, s_length) in enumerate(self._spans):
            ...
            if overlap(s_start, s_length, start, length) or adjacent(
                s_start, s_length, start, length
            ):
                last_overlap = i
    ...
    def dump(self):
        return "len=%d: %s" % (
            self.len(),
            ",".join(
                ["[%d-%d]" % (start, start + l - 1) for (start, l) in self._spans]
            ),
        )
    ...
    def __and__(self, other):
        ...
        bounds = self.__class__(
            self._spans[0][0], self._spans[-1][0] + self._spans[-1][1]
        )
    ...
    def add(self, start, data):
        ...
        while len(data):
            ...
            if s_start <= start < s_end:
                ...
                self.spans[i] = (s_start, s_data[:prefix_len] + data[:suffix_len])
                i += 1
                start += suffix_len
        ...
        newspans = []
        for (s_start, s_data) in self.spans:
            if newspans and adjacent(
                newspans[-1][0], len(newspans[-1][1]), s_start, len(s_data)
            ):
