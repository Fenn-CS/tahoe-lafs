...
from .common import OVERDUE, COMPLETE, CORRUPT, DEAD, BADSEGNUM, BadSegmentNumberError
...
class SegmentFetcher(object):
    ...
    def stop(self):
        log.msg(
            "SegmentFetcher(%r).stop" % self._node._si_prefix,
            level=log.NOISY,
            parent=self._lp,
            umid="LWyqpg",
        )
    ...
    def _do_loop(self):
        ...
        if authoritative and self.segnum >= numsegs:
            ...
            self.stop()
            e = BadSegmentNumberError(
                "segnum=%d, numsegs=%d" % (self.segnum, self._node.num_segments)
            )
            f = Failure(e)
        ...
        while len(set(self._blocks.keys()) | set(self._active_share_map.keys())) < k:
            ...
            if want_more_diversity:
                ...
                log.msg(
                    "SegmentFetcher(%r) increasing diversity limit to %d"
    ...
    def _no_shares_error(self):
        if not (
            self._shares
            or self._active_share_map
            or self._overdue_share_map
            or self._blocks
        ):
            format = "no shares (need %(k)d)." " Last failure: %(last_failure)s"
            args = {"k": self._k, "last_failure": self._last_failure}
            error = NoSharesError
        else:
            format = (
                "ran out of shares: complete=%(complete)s"
            ...
            pending_s = ",".join([str(sh) for sh in self._active_share_map.values()])
            overdue = set()
        ...
        log.msg(
            format=format, level=log.UNUSUAL, parent=self._lp, umid="1DsnTg", **args
        )
        e = error(format % args)
        f = Failure(e)
        self.stop()
    ...
    def _block_request_activity(self, share, shnum, state, block=None, f=None):
        ...
        if not self._running:
            return
        log.msg(
            "SegmentFetcher(%r)._block_request_activity: %s -> %r"
