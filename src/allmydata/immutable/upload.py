...
class Tahoe2ServerSelector(log.PrefixingLogMixin):
    ...
    def get_shareholders(
        ...
        for tracker in readonly_trackers:
            ...
            ds.append(d)
            self.log(
                "asking server %s for any existing shares" % (tracker.get_name(),),
                level=log.NOISY,
            )
    ...
    def _handle_existing_response(self, res, tracker):
        ...
        if isinstance(res, failure.Failure):
            ...
            self.log(
                "response to get_buckets() from server %s: alreadygot=%s"
    ...
    def _allocation_for(self, tracker):
        ...
        if self._status:
            self._status.set_status(
                "Contacting Servers [%s] (first query),"
    ...
    def _buckets_allocated(self, res, tracker, shares_to_ask):
        ...
        if isinstance(res, failure.Failure):
            ...
            self.log(
                "response to allocate_buckets() from server %s: alreadygot=%s, allocated=%s"
                % (
                    tracker.get_name(),
...
class EncryptAnUploadable(object):
    ...
    def read_encrypted(self, length, hash_only):
        ...
        d2 = defer.Deferred()
        d.addCallback(
            lambda ignored: self._read_encrypted(length, ciphertext, hash_only, d2)
        )
    ...
    def _read_encrypted(self, remaining, ciphertext, hash_only, fire_when_done):
        ...
        def _good(plaintext):
            ...
            self._read_encrypted(remaining, ciphertext, hash_only, fire_when_done)
...
class Uploader(service.MultiService, log.PrefixingLogMixin):
    ...
    def __init__(
        self, helper_furl=None, stats_provider=None, history=None, progress=None
    ):
    ...
    def startService(self):
        ...
        if self._helper_furl:
            self.parent.tub.connectTo(self._helper_furl, self._got_helper)
