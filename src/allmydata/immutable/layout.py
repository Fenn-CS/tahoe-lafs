...
class WriteBucketProxy(object):
    ...
    def put_block(self, segmentnum, data):
        ...
        if segmentnum < self._num_segments - 1:
            precondition(len(data) == self._block_size, len(data), self._block_size)
        else:
            precondition(
                len(data)
                == (self._data_size - (self._block_size * (self._num_segments - 1))),
                len(data),
                self._block_size,
            )
    ...
    def put_crypttext_hashes(self, hashes):
        ...
        precondition(
            offset + len(data) <= self._offsets["block_hashes"],
            offset,
            len(data),
            offset + len(data),
    ...
    def put_block_hashes(self, blockhashes):
        ...
        precondition(
            offset + len(data) <= self._offsets["share_hashes"],
            offset,
            len(data),
            offset + len(data),
    ...
    def put_share_hashes(self, sharehashes):
        ...
        data = b"".join(
            [
                struct.pack(">H", hashnum) + hashvalue
        ...
        precondition(
            offset + len(data) <= self._offsets["uri_extension"],
            offset,
            len(data),
            offset + len(data),
            self._offsets["uri_extension"],
    ...
    def _write(self, offset, data):
        ...
        return self._pipeline.add(
            len(data), self._rref.callRemote, "write", offset, data
        )
...
class ReadBucketProxy(object):
    ...
    def _fetch_sharehashtree_and_ueb(self, offsets):
        ...
        return self._read(
            offsets["share_hashes"], self.MAX_UEB_SIZE + sharehashtree_size
        )

    def _parse_sharehashtree_and_ueb(self, data):
        sharehashtree_size = (
            self._offsets["uri_extension"] - self._offsets["share_hashes"]
        )
        if len(data) < sharehashtree_size:
            raise LayoutInvalid(
                "share hash tree truncated -- should have at least %d bytes -- not %d"
                % (sharehashtree_size, len(data))
            )
        if sharehashtree_size % (2 + HASH_SIZE) != 0:
            raise LayoutInvalid(
                "share hash tree malformed -- should have an even multiple of %d bytes -- not %d"
                % (2 + HASH_SIZE, sharehashtree_size)
            )
        self._share_hashes = []
        ...
        if len(data) < i + self._fieldsize:
            raise LayoutInvalid(
                "not enough bytes to encode URI length -- should be at least %d bytes long, not %d "
                % (
                    i + self._fieldsize,
                    len(data),
                )
    ...
    def _str2l(self, s):
        ...
        return [s[i : i + HASH_SIZE] for i in range(0, len(s), HASH_SIZE)]
    ...
    def _get_share_hashes_the_old_way(self):
        ...
        if size % (2 + HASH_SIZE) != 0:
            raise LayoutInvalid(
                "share hash tree corrupted -- should occupy a multiple of %d bytes, not %d bytes"
                % ((2 + HASH_SIZE), size)
            )
        d = self._read(offset, size)
        ...
        def _unpack_share_hashes(data):
            if len(data) != size:
                raise LayoutInvalid(
                    "share hash tree corrupted -- got a short read of the share data -- should have gotten %d, not %d bytes"
                    % (size, len(data))
                )
            hashes = []
    ...
    def _get_uri_extension_the_old_way(self, unused=None):
        ...
        def _got_length(data):
            if len(data) != self._fieldsize:
                raise LayoutInvalid(
                    "not enough bytes to encode URI length -- should be %d bytes long, not %d "
                    % (
                        self._fieldsize,
                        len(data),
                    )
