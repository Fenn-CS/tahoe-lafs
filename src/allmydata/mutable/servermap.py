...
from foolscap.api import DeadReferenceError, RemoteException, eventually, fireEventually
...
class ServerMap(object):
    ...
    def dump(self, out=sys.stdout):
        ...
        for ((server, shnum), (verinfo, timestamp)) in list(self._known_shares.items()):
            ...
            print(
                "[%s]: sh#%d seq%d-%s %d-of-%d len%d"
                % (
                    unicode(server.get_name(), "utf-8"),
                    shnum,
                    seqnum,
                    unicode(base32.b2a(root_hash)[:4], "utf-8"),
                    k,
                    N,
                    datalength,
                ),
    ...
    def highest_seqnum(self):
        ...
        seqnums = [verinfo[0] for verinfo in available.keys()]
        seqnums.append(0)
        return max(seqnums)
    ...
    def unrecoverable_newer_versions(self):
        ...
        for (verinfo, shares) in list(versionmap.items()):
            ...
            if len(shnums) < k:
                ...
                highest_recoverable_seqnum = max(seqnum, highest_recoverable_seqnum)
    ...
    def needs_merge(self):
        ...
        recoverable_seqnums = [verinfo[0] for verinfo in self.recoverable_versions()]
...
class ServermapUpdater(object):
    def __init__(
        ...
        self._log_number = log.msg(
            format="SharemapUpdater(%(si)s): starting (%(mode)s)", si=prefix, mode=mode
        )
    ...
    def _build_initial_querylist(self):
        ...
        while (
            self.num_servers_to_query > len(initial_servers_to_query)
    ...
    def _do_query(self, server, storage_index, readsize):
        self.log(
            format="sending query to [%(name)s], readsize=%(readsize)d",
        ...
        d.addCallback(self._got_results, server, readsize, storage_index, started)
    ...
    def _got_results(self, datavs, server, readsize, storage_index, started):
        lp = self.log(
            format="got result from [%(name)s], %(numshares)d shares",
            name=server.get_name(),
        ...
        for shnum, datav in list(datavs.items()):
            ...
            if not self._node.get_pubkey():
                ...
                d.addCallback(
                    lambda results, shnum=shnum: self._try_to_set_pubkey(
                        results, server, shnum, lp
                    )
                )
                # XXX: Make self._pubkey_query_failed?
                d.addErrback(
                    lambda error, shnum=shnum, data=data: self._got_corrupt_share(
                        error, shnum, server, data, lp
                    )
                )
            else:
            ...
            if self._need_privkey:
                ...
                d4.addCallback(
                    lambda results, shnum=shnum: self._try_to_validate_privkey(
                        results, server, shnum, lp
                    )
                )
                d4.addErrback(
                    lambda error, shnum=shnum: self._privkey_query_failed(
                        error, server, shnum, lp
                    )
                )
            else:
            ...
            def _append_proxy(passthrough, shnum=shnum, reader=reader):
                ...
                self._servermap.proxies[
                    (verinfo, server.get_serverid(), storage_index, shnum)
                ] = reader
                return passthrough
            ...
            dl.addCallback(
                lambda results, shnum=shnum: self._got_signature_one_share(
                    results, shnum, server, lp
                )
            )
            dl.addErrback(
                lambda error, shnum=shnum, data=data: self._got_corrupt_share(
    ...
    def _try_to_set_pubkey(self, pubkey_s, server, shnum, lp):
        ...
        if fingerprint != self._node.get_fingerprint():
            raise CorruptShareError(server, shnum, "pubkey doesn't match fingerprint")
    ...
    def _got_signature_one_share(self, results, shnum, server, lp):
        ...
        self.log(
            format="_got_results: got shnum #%(shnum)d from serverid %(name)s",
            shnum=shnum,
            name=server.get_name(),
            level=log.NOISY,
            parent=lp,
        ...
        if verinfo not in self._valid_versions:
            ...
            try:
                ...
                raise CorruptShareError(server, shnum, "signature is invalid")

        # ok, it's a valid verinfo. Add it to the list of validated
        # versions.
        self.log(
            " found valid version %d-%s from %s-sh%d: %d-%d/%d/%d"
            % (
                seqnum,
                unicode(base32.b2a(root_hash)[:4], "utf-8"),
                ensure_str(server.get_name()),
                shnum,
                k,
                n,
                segsize,
        ...
        if (server, shnum) in self._servermap.get_bad_shares():
            ...
            self.log(
                "but we've been told this is a bad share", parent=lp, level=log.UNUSUAL
            )
            return verinfo
    ...
    def _got_update_results_one_share(self, results, share):
        ...
        self._servermap.set_update_data_for_share_and_verinfo(
            share, verinfo, update_data
        )
    ...
    def _try_to_validate_privkey(self, enc_privkey, server, shnum, lp):
        ...
        if alleged_writekey != self._node.get_writekey():
            self.log(
                "invalid privkey from %r shnum %d" % (server.get_name(), shnum),
                parent=lp,
                level=log.WEIRD,
                umid="aJVccw",
            )
        ...
        # it's good
        self.log(
            "got valid privkey from shnum %d on serverid %r"
    ...
    def _query_failed(self, f, server):
        ...
        if f.check(DeadReferenceError):
            level = log.UNUSUAL
        self.log(
            format="error during query: %(f_value)s",
            f_value=str(f.value),
            failure=f,
            level=level,
            umid="IHXuQg",
    ...
    def _check_for_done(self, res):
        ...
        lp = self.log(
            format=(
                "_check_for_done, mode is '%(mode)s', "
        ...
        if self._must_query:
            ...
            self.log(
                "%d 'must query' servers left" % len(self._must_query),
                level=log.NOISY,
                parent=lp,
            )
            return
        ...
        if not self._queries_outstanding and not self.extra_servers:
            ...
            self.log("all queries are retired, no extra servers: done", parent=lp)
            return self._done()
        ...
        if self.mode == MODE_ANYTHING:
            if recoverable_versions:
                self.log(
                    "%d recoverable versions: done" % len(recoverable_versions),
                    parent=lp,
                )
                return self._done()
        ...
        if self.mode == MODE_READ:
            ...
            if self._queries_completed < self.num_servers_to_query:
                self.log(
                    format="%(completed)d completed, %(query)d to query: need more",
            ...
            if not recoverable_versions:
                self.log(
                    "no recoverable versions: need more", level=log.NOISY, parent=lp
                )
                return self._send_more_queries(MAX_IN_FLIGHT)
            ...
            for unrec_verinfo in unrecoverable_versions:
                if unrec_verinfo[0] > highest_recoverable_seqnum:
                    ...
                    self.log(
                        "evidence of higher seqnum: need more",
                        level=log.UNUSUAL,
                        parent=lp,
                    )
                    return self._send_more_queries(MAX_IN_FLIGHT)
        ...
        if self.mode == MODE_WRITE:
            ...
            if not recoverable_versions:
                self.log(
                    "no recoverable versions: need more", parent=lp, level=log.NOISY
                )
                return self._send_more_queries(MAX_IN_FLIGHT)
                ...
                if server in self._bad_servers:
                    ...
                    if last_found != -1:
                        ...
                        if num_not_found >= self.EPSILON:
                            self.log(
                                "found our boundary, %s" % "".join(states),
                                parent=lp,
                                level=log.NOISY,
                            )
                            found_boundary = True
            ...
            if found_boundary:
                ...
                if last_not_responded == -1:
                    # we're done
                    self.log("have all our answers", parent=lp, level=log.NOISY)
                    # .. unless we're still waiting on the privkey
                    if self._need_privkey:
                        self.log(
                            "but we're still waiting for the privkey",
                            parent=lp,
                            level=log.NOISY,
                        )
            ...
            # waiting for servers
            self.log(
                "no boundary yet, %s" % "".join(states), parent=lp, level=log.NOISY
            )
            return self._send_more_queries(MAX_IN_FLIGHT)
    ...
    def _send_more_queries(self, num_outstanding):
        ...
        while True:
            self.log(
                format=" there are %(outstanding)d queries outstanding",
                outstanding=len(self._queries_outstanding),
                level=log.NOISY,
            )
        ...
        self.log(
            format="sending %(more)d more queries: %(who)s",
