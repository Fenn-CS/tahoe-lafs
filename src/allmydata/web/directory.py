...
class DirectoryNodeHandler(ReplaceMeMixin, Resource, object):
    ...
    def render_GET(self, req):
        ...
        if not self.node.is_mutable() and t in FIXED_OUTPUT_TYPES:
            ...
            if si and req.setETag(
                b"DIR:%s-%s" % (base32.b2a(si), t.encode("ascii") or b"")
            ):
                return b""
...
class RenameForm(Element, object):
    ...
    def title(self, req, tag):
        return tag(
            "Directory SI={}".format(str(abbreviated_dirnode(self.original), "ascii"))
        )
...
class ReloadableMonitorElement(Element):
    ...
    def refresh(self, req, tag):
        ...
        tag.attributes["content"] = "{}".format(self.refresh_time.seconds)
        return tag
    ...
    def reload(self, req, tag):
        if self.monitor.is_finished():
            return ""
        reload_url = URL.from_text("{}".format(req.path))
        cancel_button = tags.form(
            [
                tags.input(type="submit", value="Cancel"),
            ],
            action=reload_url.replace(query={"t": "cancel"}).to_uri().to_text(),
            method="POST",
            enctype="multipart/form-data",
        )
        ...
        return tag(
            [
                ...
                tags.a(
                    "Reload",
                    href=reload_url.replace(query={"output": "html"})
                    .to_uri()
                    .to_text(),
                ),
                cancel_button,
