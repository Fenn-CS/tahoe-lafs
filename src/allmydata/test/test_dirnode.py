...
from allmydata.test.common import make_chk_file_uri, make_mutable_file_uri, ErrorMixin
...
future_nonascii_write_uri = (
    "x-tahoe-even-more-crazy://I_am_from_the_future_rw_\u263A".encode("utf-8")
)
future_nonascii_read_uri = (
    "x-tahoe-even-more-crazy-readonly://I_am_from_the_future_ro_\u263A".encode("utf-8")
)
...
class Dirnode(
    ...
    def _do_create_test(self, mdmf=False):
        ...
        def _then(n):
            ...
            if mdmf:
                self.failUnlessEqual(backing_node.get_version(), MDMF_VERSION)
            else:
                self.failUnlessEqual(backing_node.get_version(), SDMF_VERSION)
            ...
            if mdmf:
                ...
                cap_formats = [b"URI:DIR2:", b"URI:DIR2-RO", b"URI:DIR2-Verifier:"]
            rw, ro, v = cap_formats
            ...
            self.expected_manifest.append((("child",), m.get_uri()))
            self.expected_verifycaps.add(ffu_v)
            self.expected_storage_indexes.add(base32.b2a(m.get_storage_index()))
            d.addCallback(lambda res: n.set_uri("child", fake_file_uri, fake_file_uri))
            d.addCallback(
            ...
            d.addCallback(lambda res: n.create_subdirectory("subdir"))
            ...
            def _created(subdir):
                ...
                self.expected_manifest.append((("subdir",), subdir.get_uri()))
            ...
            d.addCallback(
                lambda children: self.failUnlessReallyEqual(
                    set(children.keys()), set(["child", "subdir"])
                )
            )
            ...
            def _check_deepstats(stats):
                ...
                self.failUnless(
                    stats["size-directories"] > 500, stats["size-directories"]
                )
                self.failUnless(
                    stats["largest-directory"] > 500, stats["largest-directory"]
                )
            ...
            def _check_manifest(res):
                ...
                self.failUnlessReallyEqual(
                    sorted(manifest), sorted(self.expected_manifest)
                )
                stats = res["stats"]
                _check_deepstats(stats)
                self.failUnlessReallyEqual(self.expected_verifycaps, res["verifycaps"])
                self.failUnlessReallyEqual(
                    self.expected_storage_indexes, res["storage-index"]
                )

            d.addCallback(_check_manifest)

            def _add_subsubdir(res):
                return self.subdir.create_subdirectory("subsubdir")
            ...
            d.addCallback(
                lambda subsubdir: self.failUnless(
                    isinstance(subsubdir, dirnode.DirectoryNode)
                )
            )
            d.addCallback(lambda res: n.get_child_at_path(""))
            d.addCallback(
                lambda res: self.failUnlessReallyEqual(res.get_uri(), n.get_uri())
            )

            d.addCallback(lambda res: n.get_metadata_for("child"))
            d.addCallback(
                lambda metadata: self.failUnlessEqual(
                    set(metadata.keys()), set(["tahoe"])
                )
            )
            ...
            d.addCallback(lambda res: n.get_child_and_metadata_at_path(""))
            ...
            d.addCallback(lambda res: n.get_child_and_metadata_at_path("child"))

            def _check_child_and_metadata2(res):
                child, metadata = res
                self.failUnlessReallyEqual(child.get_uri(), fake_file_uri)
                self.failUnlessEqual(set(metadata.keys()), set(["tahoe"]))

            d.addCallback(_check_child_and_metadata2)

            d.addCallback(
                lambda res: n.get_child_and_metadata_at_path("subdir/subsubdir")
            )
            ...
            d.addCallback(lambda res: n.set_uri("c2", fake_file_uri, fake_file_uri, {}))
            ...
            d.addCallback(lambda res: n.set_uri("c3", fake_file_uri, fake_file_uri))
            ...
            d.addCallback(
                lambda res: n.set_uri(
                    "c4", fake_file_uri, fake_file_uri, {"key": "value"}
                )
            )
            d.addCallback(lambda res: n.get_metadata_for("c4"))
            d.addCallback(
                lambda metadata: self.failUnless(
                    (set(metadata.keys()) == set(["key", "tahoe"]))
            ...
            d.addCallback(lambda res: n.set_node("d4", n, {"key": "value"}))
            ...
            d.addCallback(
                lambda res: n.set_children(
                    {
                        "e1": (fake_file_uri, fake_file_uri),
                        "e2": (fake_file_uri, fake_file_uri, {}),
                        "e3": (fake_file_uri, fake_file_uri, {"key": "value"}),
                    }
                )
            )
            ...
            d.addCallback(
                lambda res: self.shouldFail(
                    ...
                    n.set_children,
                    {
                        "e1": (other_file_uri, other_file_uri),
                        "new": (other_file_uri, other_file_uri),
                    },
                    overwrite=False,
                )
            )
            ...
            d.addCallback(lambda res: n.list())
            d.addCallback(lambda children: self.failIf("new" in children))
            ...
            d.addCallback(
                lambda res: n.set_metadata_for(
                    "child", {"tags": ["web2.0-compatible"], "tahoe": {"bad": "mojo"}}
                )
            )
            d.addCallback(lambda n1: n1.get_metadata_for("child"))
            d.addCallback(
                lambda metadata: self.failUnless(
                    (set(metadata.keys()) == set(["tags", "tahoe"]))
                    and metadata["tags"] == ["web2.0-compatible"]
                    and "bad" not in metadata["tahoe"],
                    metadata,
                )
            )
            ...
            def _check_timestamp1(metadata):
                ...
                self.failUnlessEqual(
                    set(tahoe_md.keys()), set(["linkcrtime", "linkmotime"])
                )

                self.failUnlessGreaterOrEqualThan(
                    tahoe_md["linkcrtime"], self._start_timestamp
                )
                self.failUnlessGreaterOrEqualThan(
                    self._stop_timestamp, tahoe_md["linkcrtime"]
                )
                self.failUnlessGreaterOrEqualThan(
                    tahoe_md["linkmotime"], self._start_timestamp
                )
                self.failUnlessGreaterOrEqualThan(
                    self._stop_timestamp, tahoe_md["linkmotime"]
                )
            ...
            d.addCallback(
                lambda old_child: self.failUnlessReallyEqual(
                    old_child.get_uri(), self.subdir.get_uri()
                )
            )
            ...
            d.addCallback(
                lambda children: self.failUnlessReallyEqual(
                    set(children.keys()), set(["child"])
                )
            )

            uploadable1 = upload.Data(b"some data", convergence=b"converge")
            d.addCallback(lambda res: n.add_file("newfile", uploadable1))
            d.addCallback(
                lambda newnode: self.failUnless(IImmutableFileNode.providedBy(newnode))
            )
            ...
            d.addCallback(
                lambda children: self.failUnlessReallyEqual(
                    set(children.keys()), set(["child", "newfile"])
                )
            )
            ...
            d.addCallback(
                lambda res: n.add_file(
                    "newfile-metadata", uploadable3, {"key": "value"}
                )
            )
            d.addCallback(
                lambda newnode: self.failUnless(IImmutableFileNode.providedBy(newnode))
            ...
            d.addCallback(lambda res: n.create_subdirectory("subdir2"))

            def _created2(subdir2):
                self.subdir2 = subdir2
                # put something in the way, to make sure it gets overwritten
                return subdir2.add_file(
                    "child", upload.Data(b"overwrite me", b"converge")
                )

            d.addCallback(_created2)

            d.addCallback(lambda res: n.move_child_to("child", self.subdir2))
            d.addCallback(lambda res: n.list())
            d.addCallback(
                lambda children: self.failUnlessReallyEqual(
                    set(children.keys()), set(["newfile", "subdir2"])
                )
            )
            ...
            d.addCallback(lambda res: self.subdir2.get("child"))
            d.addCallback(
                lambda child: self.failUnlessReallyEqual(child.get_uri(), fake_file_uri)
            )

            # move it back, using new_child_name=
            d.addCallback(
                lambda res: self.subdir2.move_child_to("child", n, "newchild")
            )
            d.addCallback(lambda res: n.list())
            d.addCallback(
                lambda children: self.failUnlessReallyEqual(
                    set(children.keys()), set(["newchild", "newfile", "subdir2"])
                )
            )
            d.addCallback(lambda res: self.subdir2.list())
            d.addCallback(
                lambda children: self.failUnlessReallyEqual(
                    set(children.keys()), set([])
                )
            )

            # now make sure that we honor overwrite=False
            d.addCallback(
                lambda res: self.subdir2.set_uri(
                    "newchild", other_file_uri, other_file_uri
                )
            )
            ...
            d.addCallback(lambda res: self.subdir2.get("newchild"))
            d.addCallback(
                lambda child: self.failUnlessReallyEqual(
                    child.get_uri(), other_file_uri
                )
            )
            ...
            d.addCallback(
                lambda ign: n.set_uri("mutable", other_file_uri, other_file_uri)
            )
            d.addCallback(lambda ign: n.get("mutable"))
            d.addCallback(lambda mutable: self.failIf(mutable.is_readonly(), mutable))
            d.addCallback(lambda ign: n.set_metadata_for("mutable", {"no-write": True}))
            d.addCallback(lambda ign: n.get("mutable"))
            d.addCallback(
                lambda mutable: self.failUnless(mutable.is_readonly(), mutable)
            )
            d.addCallback(lambda ign: n.set_metadata_for("mutable", {"no-write": True}))
            d.addCallback(lambda ign: n.get("mutable"))
            d.addCallback(
            ...
            d.addCallback(lambda ign: n.get("subdir2"))
            d.addCallback(lambda subdir2: self.failIf(subdir2.is_readonly()))
            d.addCallback(lambda ign: n.set_metadata_for("subdir2", {"no-write": True}))
            d.addCallback(lambda ign: n.get("subdir2"))
            d.addCallback(
                lambda subdir2: self.failUnless(subdir2.is_readonly(), subdir2)
            )

            d.addCallback(
                lambda ign: n.set_uri(
                    "mutable_ro",
                    other_file_uri,
                    other_file_uri,
            ...
            d.addCallback(lambda ign: n.get("mutable_ro"))
            d.addCallback(
                lambda mutable_ro: self.failUnless(mutable_ro.is_readonly(), mutable_ro)
            )

            d.addCallback(
                lambda ign: n.create_subdirectory(
                    "subdir_ro", metadata={"no-write": True}
                )
            )
            d.addCallback(lambda ign: n.get("subdir_ro"))
            d.addCallback(
                lambda subdir_ro: self.failUnless(subdir_ro.is_readonly(), subdir_ro)
            )
    ...
    def _do_initial_children_test(self, mdmf=False):
        ...
        kids = {
            one_nfd: (nm.create_from_cap(one_uri), {}),
            "two": (nm.create_from_cap(setup_py_uri), {"metakey": "metavalue"}),
            "mut": (nm.create_from_cap(mut_write_uri, mut_read_uri), {}),
            "mdmf": (nm.create_from_cap(mdmf_write_uri, mdmf_read_uri), {}),
            "fut": (nm.create_from_cap(future_write_uri, future_read_uri), {}),
            "fro": (nm.create_from_cap(None, future_read_uri), {}),
            "fut-unic": (
                nm.create_from_cap(future_nonascii_write_uri, future_nonascii_read_uri),
                {},
            ),
            "fro-unic": (nm.create_from_cap(None, future_nonascii_read_uri), {}),
            "empty_litdir": (nm.create_from_cap(empty_litdir_uri), {}),
            "tiny_litdir": (nm.create_from_cap(tiny_litdir_uri), {}),
        }
        if mdmf:
        ...
        def _check_kids(children):
            ...
            two_node, two_metadata = children["two"]
            mut_node, mut_metadata = children["mut"]
            mdmf_node, mdmf_metadata = children["mdmf"]
            fut_node, fut_metadata = children["fut"]
            fro_node, fro_metadata = children["fro"]
            futna_node, futna_metadata = children["fut-unic"]
            frona_node, frona_metadata = children["fro-unic"]
            emptylit_node, emptylit_metadata = children["empty_litdir"]
            tinylit_node, tinylit_metadata = children["tiny_litdir"]
            ...
            self.failUnlessReallyEqual(
                fut_node.get_readonly_uri(), b"ro." + future_read_uri
            )
            ...
            self.failUnlessReallyEqual(
                futna_node.get_readonly_uri(), b"ro." + future_nonascii_read_uri
            )
            ...
            self.failUnlessReallyEqual(
                frona_node.get_uri(), b"ro." + future_nonascii_read_uri
            )
            self.failUnlessReallyEqual(
                futna_node.get_readonly_uri(), b"ro." + future_nonascii_read_uri
            )
            ...
            d2.addCallback(
                lambda children: self.failUnlessReallyEqual(
                    set(children.keys()), set(["short"])
                )
            )
            d2.addCallback(lambda ignored: tinylit_node.list())
            d2.addCallback(lambda children: children["short"][0].read(MemAccum()))
            d2.addCallback(
                lambda accum: self.failUnlessReallyEqual(accum.data, b"The end.")
            )
            return d2
    ...
    def test_immutable(self):
        ...
        kids = {
            one_nfd: (nm.create_from_cap(one_uri), {}),
            "two": (nm.create_from_cap(setup_py_uri), {"metakey": "metavalue"}),
            "fut": (nm.create_from_cap(None, future_read_uri), {}),
            "futna": (nm.create_from_cap(None, future_nonascii_read_uri), {}),
            "empty_litdir": (nm.create_from_cap(empty_litdir_uri), {}),
            "tiny_litdir": (nm.create_from_cap(tiny_litdir_uri), {}),
        }
        ...
        def _check_kids(children):
            self.failUnlessReallyEqual(
                set(children.keys()),
                set([one_nfc, "two", "fut", "futna", "empty_litdir", "tiny_litdir"]),
            )
            one_node, one_metadata = children[one_nfc]
            two_node, two_metadata = children["two"]
            fut_node, fut_metadata = children["fut"]
            futna_node, futna_metadata = children["futna"]
            emptylit_node, emptylit_metadata = children["empty_litdir"]
            tinylit_node, tinylit_metadata = children["tiny_litdir"]
            ...
            self.failUnlessReallyEqual(
                fut_node.get_readonly_uri(), b"imm." + future_read_uri
            )
            self.failUnless(isinstance(fut_metadata, dict), fut_metadata)

            self.failUnless(futna_node.is_unknown())
            self.failUnlessReallyEqual(
                futna_node.get_uri(), b"imm." + future_nonascii_read_uri
            )
            self.failUnlessReallyEqual(
                futna_node.get_readonly_uri(), b"imm." + future_nonascii_read_uri
            )
            self.failUnless(isinstance(futna_metadata, dict), futna_metadata)
        ...
        smallkids = {"o": (nm.create_from_cap(one_uri), {})}
        ...
        d.addCallback(lambda kids: self.failUnlessReallyEqual(list(kids.keys()), ["o"]))

        # now test n.create_subdirectory(mutable=False)
        d.addCallback(lambda ign: c.create_dirnode())

        def _made_parent(n):
            d = n.create_subdirectory("subdir", kids, mutable=False)
            d.addCallback(lambda sd: sd.list())
            d.addCallback(_check_kids)
            d.addCallback(lambda ign: n.list())
            d.addCallback(
                lambda children: self.failUnlessReallyEqual(
                    list(children.keys()), ["subdir"]
                )
            )
            d.addCallback(lambda ign: n.get("subdir"))
            d.addCallback(lambda sd: sd.list())
            d.addCallback(_check_kids)
            d.addCallback(lambda ign: n.get("subdir"))
    ...
    def test_directory_representation(self):
        ...
        self.failUnlessReallyEqual(
            child.get_readonly_uri(), b"ro." + spacedout_read_uri
        )

        child_dottedi = "ch\u0131\u0307ld"

        kids_in = {child_dottedi: (child, {}), one_nfd: (child, {})}
        kids_out = {child_dottedi: (child, {}), one_nfc: (child, {})}
        kids_norm = {"child": (child, {}), one_nfc: (child, {})}
        ...
        def _check_data(data):
            ...
            while position < len(data):
                ...
                entry = entries[0]
                (name_utf8, ro_uri, rwcapdata, metadata_s), subpos = split_netstring(
                    entry, 4
                )
                ...
                self.failUnlessReallyEqual(
                    b"ro." + ro_uri, expected_child.get_readonly_uri()
                )
                numkids += 1
        ...
        def future_normalize(form, s):
            ...
            return old_normalize(form, s).replace("\u0131\u0307", "i")
        ...
        def _check_kids(children):
            ...
            child_node, child_metadata = children["child"]

            self.failUnlessReallyEqual(child_node.get_write_uri(), stripped_write_uri)
            self.failUnlessReallyEqual(
                child_node.get_readonly_uri(), b"ro." + stripped_read_uri
            )
    ...
    def _test_deepcheck_create(self, version=SDMF_VERSION):
        ...
        def _created_root(rootnode):
            ...
            return rootnode.create_subdirectory("subdir")

        d.addCallback(_created_root)

        def _created_subdir(subdir):
            self._subdir = subdir
            d = subdir.add_file("file1", upload.Data(b"data" * 100, None))
            d.addCallback(lambda res: subdir.set_node("link", self._rootnode))
            d.addCallback(lambda res: c.create_dirnode())
            d.addCallback(
                lambda dn: self._rootnode.set_uri(
                    "rodir", dn.get_uri(), dn.get_readonly_uri()
                )
            )
            return d
    ...
    def test_deepcheck_cachemisses(self):
        ...
        d.addCallback(
            lambda rootnode: not [
                ss._clear_counters() for ss in self.g.wrappers_by_id.values()
            ]
            or rootnode
        )
        d.addCallback(lambda rootnode: rootnode.start_deep_check().when_done())
        ...
        def _check(ign):
            count = sum(
                [
                    ss.counter_by_methname["slot_readv"]
                    for ss in self.g.wrappers_by_id.values()
                ]
            )
            self.failIf(
                count > 60,
                "Expected only 60 cache misses,"
    ...
    def test_deepcheck_and_repair(self):
        ...
        d = self._test_deepcheck_create()
        d.addCallback(
            lambda rootnode: rootnode.start_deep_check_and_repair().when_done()
        )
    ...
    def _do_readonly_test(self, version=SDMF_VERSION):
        ...
        def _created(rw_dn):
            ...
            d2 = rw_dn.set_uri("child", filecap, filecap)
        ...
        def _ready(rw_dn):
            ...
            self.shouldFail(
                dirnode.NotWriteableError, "set_uri ro", None, ro_dn.delete, "child"
            )
            self.shouldFail(
                dirnode.NotWriteableError,
                "set_uri ro",
                None,
                ro_dn.create_subdirectory,
                "newchild",
            )
            self.shouldFail(
                dirnode.NotWriteableError,
    ...
    def test_update_metadata(self):
        ...
        md2 = dirnode.update_metadata(
            md1, {"key": "value", "tahoe": {"bad": "mojo"}}, t3
        )
        self.failUnlessEqual(
            md2, {"key": "value", "tahoe": {"linkcrtime": t1, "linkmotime": t3}}
        )
        ...
        self.failUnlessEqual(
            md4,
            {"bool": True, "number": 42, "tahoe": {"linkcrtime": t1, "linkmotime": t1}},
        )
    ...
    def _do_create_subdirectory_test(self, version=SDMF_VERSION):
        ...
        def _then(n):
            ...
            kids = {
                "kid1": (nm.create_from_cap(fake_file_uri), {}),
                "kid2": (nm.create_from_cap(other_file_uri), md),
            }
            d = n.create_subdirectory("subdir", kids, mutable_version=version)

            def _check(sub):
                d = n.get_child_at_path("subdir")
                d.addCallback(
                    lambda sub2: self.failUnlessReallyEqual(
                        sub2.get_uri(), sub.get_uri()
                    )
                )
            ...
            def _check_kids(kids2):
                ...
                self.failUnlessEqual(kids2["kid2"][1]["metakey"], "metavalue")
...
class Packing(testutil.ReallyEqualMixin, unittest.TestCase):
    ...
    def test_unpack_and_pack_behavior(self):
        ...
        nodemaker = NodeMaker(
            None, None, None, None, None, {"k": 3, "n": 10}, None, None
        )
    ...
    def _check_children(self, children):
        ...
        self.failUnless("file1" in children)
        self.failUnless("file2" in children)
        self.failUnless("file3" in children)
        ...
        file3_metadata = {
            "ctime": 1246663897.4336269,
            "tahoe": {
                "linkmotime": 1246663897.4336269,
                "linkcrtime": 1246663897.4336269,
            },
            "mtime": 1246663897.4336269,
        }
        self.failUnlessEqual(file3_metadata, children["file3"][1])
        self.failUnlessReallyEqual(file3_rocap, children["file3"][0].get_readonly_uri())
        self.failUnlessReallyEqual(file3_rwcap, children["file3"][0].get_uri())
        ...
        file2_metadata = {
            "ctime": 1246663897.430218,
            "tahoe": {"linkmotime": 1246663897.430218, "linkcrtime": 1246663897.430218},
            "mtime": 1246663897.430218,
        }
        self.failUnlessEqual(file2_metadata, children["file2"][1])
        self.failUnlessReallyEqual(file2_rocap, children["file2"][0].get_readonly_uri())
        self.failUnlessReallyEqual(file2_rwcap, children["file2"][0].get_uri())
        ...
        file1_metadata = {
            "ctime": 1246663897.4275661,
            "tahoe": {
                "linkmotime": 1246663897.4275661,
                "linkcrtime": 1246663897.4275661,
            },
            "mtime": 1246663897.4275661,
        }
        self.failUnlessEqual(file1_metadata, children["file1"][1])
        self.failUnlessReallyEqual(file1_rocap, children["file1"][0].get_readonly_uri())
        self.failUnlessReallyEqual(file1_rwcap, children["file1"][0].get_uri())

    def _make_kids(self, nm, which):
        caps = {
            "imm": b"URI:CHK:n7r3m6wmomelk4sep3kw5cvduq:os7ijw5c3maek7pg65e5254k2fzjflavtpejjyhshpsxuqzhcwwq:3:20:14861",
    ...
    def test_pack_unpack_unknown(self):
        ...
        kids = {"unknown_rw": (unknown_rw, {}), "unknown_ro": (unknown_ro, {})}
    ...
    def test_deep_immutable(self):
        ...
        kids = self._make_kids(
            nm, ["imm", "lit", "write", "read", "dirwrite", "dirread"]
        )
        packed = dirnode.pack_children(kids, fn.get_writekey(), deep_immutable=False)
        ...
        self.failUnlessRaises(
            dirnode.MustBeDeepImmutableError,
...
class Dirnode2(testutil.ReallyEqualMixin, testutil.ShouldFailMixin, unittest.TestCase):
    ...
    def test_from_future(self):
        ...
        future_write_uri = "x-tahoe-crazy://I_am_from_the_future_rw_\u263A".encode(
            "utf-8"
        )
        future_read_uri = (
            "x-tahoe-crazy-readonly://I_am_from_the_future_ro_\u263A".encode("utf-8")
        )
        future_imm_uri = (
            "x-tahoe-crazy-immutable://I_am_from_the_future_imm_\u263A".encode("utf-8")
        )
        future_node = UnknownNode(future_write_uri, future_read_uri)

        def _then(n):
            self._node = n
            return n.set_node("future", future_node)
        ...
        # unknown URIs.
        d.addCallback(
            lambda ign: self._node.set_uri(
                "add-pair", future_write_uri, future_read_uri
            )
        )
        ...
        d.addCallback(
            lambda ign: self._node.set_uri("add-ro", b"ro." + future_read_uri, None)
        )
        d.addCallback(
            lambda ign: self._node.set_uri("add-imm", b"imm." + future_imm_uri, None)
        )
        ...
        def _check(children):
            ...
            (fn2, metadata2) = children["add-pair"]
            ...
            (fn4, metadata4) = children["add-imm"]
            ...
            return self._node.set_node("copy", fn)
    ...
    def test_unknownnode(self):
        ...
        bad_uri = (
            [  # These are errors because the URI is bad once we've stripped the prefix.
        ...
        ro_prefixed = (
            [  # These are valid, and the readcap should end up with a ro. prefix.
        ...
        imm_prefixed = (
            [  # These are valid, and the readcap should end up with an imm. prefix.
...
class DeepStats(testutil.ReallyEqualMixin, unittest.TestCase):
    def test_stats(self):
        ...
        self.failUnlessReallyEqual(s["count-literal-files"], 0)
        self.failUnlessReallyEqual(
            s["size-files-histogram"], [(101, 316, 1), (317, 1000, 1)]
        )
...
class Adder(GridTestMixin, unittest.TestCase, testutil.ShouldFailMixin):
    def test_overwrite(self):
        ...
        def _create_directory_tree(root_node):
            ...
            # root/file2
            # root/dir1
            d = root_node.add_file("file1", upload.Data(b"Important Things", None))
            d.addCallback(
                lambda res: root_node.add_file(
                    "file2", upload.Data(b"Sekrit Codes", None)
                )
            )
            d.addCallback(lambda res: root_node.create_subdirectory("dir1"))
        ...
        def _test_adder(root_node):
            d = root_node.set_node("file1", filenode)
            # We've overwritten file1. Let's try it with a directory
            d.addCallback(lambda res: root_node.create_subdirectory("dir2"))
            d.addCallback(lambda res: root_node.set_node("dir2", filenode))
            ...
            d.addCallback(
                lambda res: root_node.set_node(
                    "file1", filenode, overwrite=dirnode.ONLY_FILES
                )
            )
            d.addCallback(
                lambda res: self.shouldFail(
