...
class TestRegression(unittest.TestCase):
    ...
    def test_encrypt_data_not_bytes(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            aes.encrypt_data(encryptor, "not bytes")
    ...
    def test_iv_not_bytes(self):
        ...
        with self.assertRaises(TypeError) as ctx:
            aes.create_encryptor(key, iv="1234567890abcdef")
        self.assertIn("must be bytes", str(ctx.exception))
...
class TestEd25519(unittest.TestCase):
    ...
    def test_deserialize_private_not_bytes(self):
        ...
        """
        with self.assertRaises(ValueError) as ctx:
            ed25519.signing_keypair_from_string("not bytes")
        self.assertIn("must be bytes", str(ctx.exception))
    ...
    def test_deserialize_public_not_bytes(self):
        ...
        """
        with self.assertRaises(ValueError) as ctx:
            ed25519.verifying_key_from_string("not bytes")
        self.assertIn("must be bytes", str(ctx.exception))
    ...
    def test_signed_data_not_bytes(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            ed25519.sign_data(priv, "not bytes")
        self.assertIn("must be bytes", str(ctx.exception))
    ...
    def test_signature_not_bytes(self):
        ...
        priv, pub = ed25519.create_signing_keypair()
        with self.assertRaises(ValueError) as ctx:
            ed25519.verify_signature(pub, "not bytes", b"data")
        self.assertIn("must be bytes", str(ctx.exception))
    ...
    def test_signature_data_not_bytes(self):
        ...
        priv, pub = ed25519.create_signing_keypair()
        with self.assertRaises(ValueError) as ctx:
            ed25519.verify_signature(pub, b"signature", "not bytes")
        self.assertIn("must be bytes", str(ctx.exception))
