...
class Statistics(unittest.TestCase):
    ...
    def test_binomial_distribution_pmf(self):
        ...
        self.should_assert("Should assert if n < 1", f, 0, 0.1)
    ...
    def test_survival_pmf(self):
        ...
        # method.
        p_list = [0.9999] * 100 + [0.99] * 50 + [0.8] * 20
        ...
        self.should_assert("Should assert if p_i < 0", f, [-0.1])

    def test_repair_count_pmf(self):
        survival_pmf = statistics.binomial_distribution_pmf(5, 0.9)
        ...
        self.failUnlessListAlmostEqual(
            repair_pmf, [0.00001 + 0.00045 + 0.0081 + 0.59049, 0.32805, 0.0729, 0, 0, 0]
        )
    ...
    def test_repair_cost(self):
        ...
        cost = statistics.mean_repair_cost(
            bwcost, 1000, survival_pmf, 3, ul_dl_ratio=1.0
        )
        self.failUnlessAlmostEqual(cost, 558.90)
        cost = statistics.mean_repair_cost(
            bwcost, 1000, survival_pmf, 3, ul_dl_ratio=8.0
        )
        self.failUnlessAlmostEqual(cost, 1664.55)

        # I haven't manually checked the math beyond here -warner
        cost = statistics.eternal_repair_cost(
            bwcost, 1000, survival_pmf, 3, discount_rate=0, ul_dl_ratio=1.0
        )
        self.failUnlessAlmostEqual(cost, 65292.056074766246)
        cost = statistics.eternal_repair_cost(
            bwcost, 1000, survival_pmf, 3, discount_rate=0.05, ul_dl_ratio=1.0
        )
    ...
    def test_convolve(self):
        ...
        r2 = f(tmp2, v2)
        self.failUnlessListEqual(
            r1, r2, "Convolution should be scalar multiplication associative"
        )

    def test_find_k(self):
        f = statistics.find_k
        g = statistics.pr_file_loss
        plist = [0.9] * 10 + [0.8] * 10  # N=20
        t = 0.0001
        k = f(plist, t)
    ...
    def test_pr_file_loss(self):
        ...
        plist = [0.5] * 10
        self.failUnlessEqual(f(plist, 3), 0.0546875)

    def test_pr_backup_file_loss(self):
        f = statistics.pr_backup_file_loss
        plist = [0.5] * 10
        self.failUnlessEqual(f(plist, 0.5, 3), 0.02734375)
