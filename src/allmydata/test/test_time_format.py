...
class TimeFormat(unittest.TestCase, TimezoneMixin):
    ...
    def test_epoch_in_London(self):
        ...
        if not self.have_working_tzset():
            raise unittest.SkipTest(
                "This test can't be run on a platform without time.tzset()."
            )
    ...
    def _help_test_epoch(self):
        ...
        self.failUnlessEqual(time_format.iso_utc(1.0, sep=" "), "1970-01-01 00:00:01")
        ...
        self.failUnlessEqual(time_format.iso_utc(t=my_time), "1970-01-01_00:00:01")
        e = self.failUnlessRaises(
            ValueError, time_format.iso_utc_time_to_seconds, "invalid timestring"
        )
    ...
    def test_parse_duration(self):
        ...
        self.failUnlessIn("no unit (like day, month, or year) in '123'", str(e))
    ...
    def test_format_time(self):
        self.failUnlessEqual(
            time_format.format_time(time.gmtime(0)), "1970-01-01 00:00:00"
        )
        self.failUnlessEqual(
            time_format.format_time(time.gmtime(60)), "1970-01-01 00:01:00"
        )
        self.failUnlessEqual(
            time_format.format_time(time.gmtime(60 * 60)), "1970-01-01 01:00:00"
        )
    ...
    def test_format_time_y2038(self):
        ...
        t = seconds_per_day * ((2048 - 1970) * 365 + leap_years_1970_to_2047_inclusive)
        try:
            gm_t = time.gmtime(t)
        except ValueError:
            raise unittest.SkipTest("Note: this system cannot handle dates after 2037.")
        self.failUnlessEqual(time_format.format_time(gm_t), "2048-01-01 00:00:00")
    ...
    def test_format_delta(self):
        ...
        self.failUnlessEqual(time_format.format_delta(time_1, time_5s_delta), "5s")
        self.failUnlessEqual(
