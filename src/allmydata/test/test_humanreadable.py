...
def foo():
    pass  # FYI foo()'s line number is used in the test below
...
class HumanReadable(unittest.TestCase):
    def test_repr(self):
        ...
        self.failUnlessEqual(
            hr(self),
            "<allmydata.test.test_humanreadable.HumanReadable testMethod=test_repr>",
        )
        ...
        try:
            ...
            self.failUnless(
                hr(e) == "<ValueError: ()>" or hr(e) == "ValueError()"  # python-2.4
            )  # python-2.5
        try:
