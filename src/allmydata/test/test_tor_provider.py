...
def mock_txtorcon(txtorcon):
    return mock.patch(
        "allmydata.util.tor_provider._import_txtorcon", return_value=txtorcon
    )


def mock_tor(tor):
    return mock.patch("allmydata.util.tor_provider._import_tor", return_value=tor)
...
class TryToConnect(unittest.TestCase):
    def test_try(self):
        ...
        stdout = StringIO()
        with mock.patch(
            "allmydata.util.tor_provider.clientFromString", return_value=ep
        ) as cfs:
...
class LaunchTor(unittest.TestCase):
    def _do_test_launch(self, tor_executable):
        ...
        with mock.patch(
            "allmydata.util.tor_provider.allocate_tcp_port", return_value=999999
        ):
            d = tor_provider._launch_tor(reactor, tor_executable, private_dir, txtorcon)
...
class ConnectToTor(unittest.TestCase):
    def _do_test_connect(self, endpoint, reachable):
        ...
        with mock.patch("allmydata.util.tor_provider._try_to_connect", _try_to_connect):
            d = tor_provider._connect_to_tor(reactor, cli_config, txtorcon)
        if not reachable:
            f = self.failureResultOf(d)
            self.assertIsInstance(f.value, ValueError)
            self.assertEqual(
                str(f.value), "unable to reach any default Tor control port"
            )
            return
...
class CreateOnion(unittest.TestCase):
    ...
    def _do_test_launch(self, executable):
        ...
        protocol = object()
        launch_tor = mock.Mock(
            return_value=defer.succeed(("control_endpoint", protocol))
        )
        txtorcon = mock.Mock()
        ehs = mock.Mock()
        ...
        launch_tor.assert_called_with(
            reactor, executable, os.path.abspath(private_dir), txtorcon
        )
    ...
    def test_control_endpoint(self):
        ...
        protocol = object()
        connect_to_tor = mock.Mock(return_value=defer.succeed(("goodport", protocol)))
        txtorcon = mock.Mock()
        ehs = mock.Mock()
        ...
        with mock_txtorcon(txtorcon):
            with mock.patch(
                "allmydata.util.tor_provider._connect_to_tor", connect_to_tor
            ):
                with mock.patch(
                    "allmydata.util.tor_provider.allocate_tcp_port", return_value=999999
                ):
...
class Provider(unittest.TestCase):
    ...
    def test_handler_launch(self):
        ...
        tor.control_endpoint_maker.assert_called_with(
            p._make_control_endpoint, takes_status=True
        )
        ...
        with mock.patch("allmydata.util.tor_provider._launch_tor", launch_tor):
            with mock.patch("allmydata.util.tor_provider.clientFromString", cfs):
                d = p._make_control_endpoint(reactor, update_status=lambda status: None)
                yield flushEventualQueue()
                self.assertIs(self.successResultOf(d), ep)
                launch_tor.assert_called_with(
                    reactor, None, os.path.join("basedir", "private"), txtorcon
                )
    ...
    def test_handler_socks_endpoint(self):
        ...
        with mock_tor(tor):
            p = tor_provider.create(reactor, FakeConfig(**{"socks.port": "ep_desc"}))
            with mock.patch("allmydata.util.tor_provider.clientFromString", cfs):
                h = p.get_tor_handler()
        cfs.assert_called_with(reactor, "ep_desc")
    ...
    def test_handler_socks_unix_endpoint(self):
        ...
        with mock_tor(tor):
            p = tor_provider.create(reactor, FakeConfig(**{"socks.port": "unix:path"}))
            with mock.patch("allmydata.util.tor_provider.clientFromString", cfs):
                h = p.get_tor_handler()
    ...
    def test_handler_socks_tcp_endpoint(self):
        ...
        with mock_tor(tor):
            p = tor_provider.create(
                reactor, FakeConfig(**{"socks.port": "tcp:127.0.0.1:1234"})
            )
            with mock.patch("allmydata.util.tor_provider.clientFromString", cfs):
                h = p.get_tor_handler()
    ...
    def test_handler_control_endpoint(self):
        ...
        with mock_tor(tor):
            p = tor_provider.create(reactor, FakeConfig(**{"control.port": "ep_desc"}))
            with mock.patch("allmydata.util.tor_provider.clientFromString", cfs):
                h = p.get_tor_handler()
        self.assertIs(h, handler)
        cfs.assert_called_with(reactor, "ep_desc")
...
class ProviderListener(unittest.TestCase):
    def test_listener(self):
        ...
        with mock_tor(tor):
            p = tor_provider.create(reactor, FakeConfig(**{"onion.local_port": "321"}))
        fake_ep = object()
        with mock.patch(
            "allmydata.util.tor_provider.TCP4ServerEndpoint", return_value=fake_ep
        ) as e:
            endpoint_or_description = p.get_listener()
        self.assertIs(endpoint_or_description, fake_ep)
        self.assertEqual(e.mock_calls, [mock.call(reactor, 321, interface="127.0.0.1")])
...
class Provider_CheckOnionConfig(unittest.TestCase):
    ...
    def test_no_launch_no_control(self):
        ...
        self.assertEqual(
            str(ctx.exception),
            "[tor] onion = true, but we have neither " "launch=true nor control.port=",
        )
    ...
    def test_onion_no_local_port(self):
        ...
        self.assertEqual(
            str(ctx.exception),
            "[tor] onion = true, " "but onion.local_port= is missing",
        )
