...
class SystemTest(SystemTestMixin, RunBinTahoeMixin, unittest.TestCase):
    ...
    def test_mutable(self):
        ...
        def _test_debug(res):
            ...
            try:
                ...
                expected = "  verify-cap: URI:SSK-Verifier:%s:" % base32.b2a(
                    storage_index
                )
    ...
    def _do_publish_private(self, res):
        ...
        def _got_new_dir(privnode):
            ...
            d1.addCallback(
                lambda res: rootnode.get_child_at_path([u"subdir1", u"subdir2"])
            )

            def _got_s2(s2node):
                d2 = privnode.set_uri(
                    u"s2-rw", s2node.get_uri(), s2node.get_readonly_uri()
                )
                d2.addCallback(
                    lambda node: privnode.set_uri(
                        u"s2-ro", s2node.get_readonly_uri(), s2node.get_readonly_uri()
                    )
                )
                return d2
    ...
    def _check_publish_private(self, resnode):
        ...
        def _got_s2ro(dirnode):
            ...
            ut = upload.Data(
                "I will disappear, unrecorded and unobserved. The tragedy of my demise is made more poignant by its silence, but this beauty is not for you to ever know.",
                convergence="99i-p1x4-xd4-18yc-ywt-87uu-msu-zo -- completely and totally unguessable string (unless you read this)",
            )
            d1.addCallback(
                lambda res: self.shouldFail2(
                    NotWriteableError,
            ...
            d1.addCallback(
                lambda res: self.shouldFail2(
                    NoSuchChildError, "get(missing)", "missing", dirnode.get, u"missing"
                )
            )
        ...
        def _got_home(dummy):
            ...
            d1.addCallback(
                lambda res: personal.move_child_to(u"sekrit data", home, u"sekrit")
            )

            d1.addCallback(self.log, "mv P/sekrit 'P/sekrit data'")
            d1.addCallback(
                lambda res: home.move_child_to(u"sekrit", home, u"sekrit data")
            )

            d1.addCallback(self.log, "mv 'P/sekret data' P/personal/")
            d1.addCallback(lambda res: home.move_child_to(u"sekrit data", personal))
    ...
    def _test_web(self, res):
        ...
        def _got_welcome(page):
            ...
            nodeid_re = r'<th>Node ID:</th>\s*<td title="TubID: %s">%s</td>' % (
                ...
                self.clients[0].get_long_nodeid(),
            )
            self.failUnless(
        ...
        def _get_from_bogus_uri(res):
            d1 = self.GET(
                "uri/%s?filename=%s" % (self.mangle_uri(self.uri), "mydata567")
            )
        ...
        d.addCallback(
            lambda res: self.PUT(public + "/subdir3/new.txt", "new.txt contents")
        )
        ...
        d.addCallback(_new_happy_semantics)
        d.addCallback(
            lambda res: self.PUT(public + "/subdir3/big.txt", "big" * 500000)
        )  # 1.5MB
        ...
        d.addCallback(
            lambda res: self.PUT(public + "/subdir3/new.txt", "NEWER contents")
        )
        d.addCallback(lambda res: self.GET(public + "/subdir3/new.txt"))
        ...
        # test unlinked POST
        d.addCallback(
            lambda res: self.POST("uri", t="upload", file=("new.txt", "data" * 10000))
        )
        # and again using the helper, which exercises different upload-status
        # display code
        d.addCallback(
            lambda res: self.POST(
                "uri", use_helper=True, t="upload", file=("foo.txt", "data2" * 10000)
            )
        )
