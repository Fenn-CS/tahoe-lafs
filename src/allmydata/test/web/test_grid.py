...
class Grid(
    ...
    def test_filecheck(self):
        ...
        d.addCallback(lambda ign: c0.upload(upload.Data(DATA + b"1", convergence=b"")))
        d.addCallback(_stash_uri, "sick")
        d.addCallback(lambda ign: c0.upload(upload.Data(DATA + b"2", convergence=b"")))
        ...
        d.addCallback(
            lambda ign: c0.create_mutable_file(publish.MutableData(DATA + b"3"))
        )
        ...
        def _got_json_sick(res):
            r = json.loads(res)
            self.failUnlessEqual(r["summary"], "Not Healthy: 9 shares (enc 3-of-10)")
        ...
        def _got_json_dead(res):
            r = json.loads(res)
            self.failUnlessEqual(r["summary"], "Not Healthy: 1 shares (enc 3-of-10)")
            self.failIf(r["results"]["healthy"])
    ...
    def test_unknown(self, immutable=False):
        ...
        expected_info_url = "uri/%s?t=info" % url_quote(unknown_rwcap, safe="")
        ...
        def _check_directory_html(res, expected_type_suffix):
            pattern = re.compile(
                br"<td>\?%s</td>[ \t\n\r]*"
                b"<td>%s</td>" % (expected_type_suffix, name.encode("ascii")),
                re.DOTALL,
            )
        ...
        def _check_directory_json(res, expect_rw_uri):
            ...
            if expect_rw_uri:
                self.failUnlessReallyEqual(
                    to_bytes(f[1]["rw_uri"]), unknown_rwcap, data
                )
            else:
                self.failIfIn("rw_uri", f[1])
            if immutable:
                self.failUnlessReallyEqual(
                    to_bytes(f[1]["ro_uri"]), unknown_immcap, data
                )
            else:
                self.failUnlessReallyEqual(
                    to_bytes(f[1]["ro_uri"]), unknown_rocap, data
                )
        ...
        def _check_json(res, expect_rw_uri):
            ...
            if expect_rw_uri:
                self.failUnlessReallyEqual(
                    to_bytes(data[1]["rw_uri"]), unknown_rwcap, data
                )
            else:
            ...
            if immutable:
                self.failUnlessReallyEqual(
                    to_bytes(data[1]["ro_uri"]), unknown_immcap, data
                )
                self.failUnlessReallyEqual(data[1]["mutable"], False)
            elif expect_rw_uri:
                self.failUnlessReallyEqual(
                    to_bytes(data[1]["ro_uri"]), unknown_rocap, data
                )
    ...
    def test_mutant_dirnodes_are_omitted(self):
        ...
        kids = {
            "lonely": (lonely_child, {}),
            "ro": (mutant_ro_child, {}),
            "write-in-ro": (mutant_write_in_ro_child, {}),
        }
        ...
        def _check_kids(children):
            self.failUnlessReallyEqual(sorted(children.keys()), ["lonely"])
            lonely_node, lonely_metadata = children["lonely"]
        ...
        def _check_html(res):
            ...
            for td in soup.find_all("td"):
                if td.text != "FILE":
                    continue
                a = td.findNextSibling()("a")[0]
                self.assertIn(url_quote(lonely_uri), a["href"])
                self.assertEqual("lonely", a.text)
                self.assertEqual(a["rel"], ["noreferrer"])
                self.assertEqual(
                    "{}".format(len("one")), td.findNextSibling().findNextSibling().text
                )
                found = True
                break
            self.assertTrue(found)

            infos = list(a["href"] for a in soup.find_all("a") if a.text == "More Info")
        ...
        def _check_json(res):
            ...
            self.failUnlessReallyEqual(sorted(listed_children.keys()), ["lonely"])
            ll_type, ll_data = listed_children["lonely"]
    ...
    def test_deep_check(self):
        ...
        def _stash_root_and_create_file(n):
            ...
            return n.add_file("good", upload.Data(DATA, convergence=b""))
        ...
        d.addCallback(_stash_uri, "good")
        d.addCallback(
            lambda ign: self.rootnode.add_file(
                "small", upload.Data(b"literal", convergence=b"")
            )
        )
        d.addCallback(_stash_uri, "small")
        d.addCallback(
            lambda ign: self.rootnode.add_file(
                "sick", upload.Data(DATA + b"1", convergence=b"")
            )
        )
        d.addCallback(_stash_uri, "sick")
        ...
        future_node = UnknownNode(unknown_rwcap, unknown_rocap)
        d.addCallback(lambda ign: self.rootnode.set_node("future", future_node))
        ...
        def _done(res):
            try:
                units = [json.loads(line) for line in res.splitlines() if line]
            except ValueError:
            ...
            ugood = [u for u in units if u["type"] == "file" and u["path"] == ["good"]][
                0
            ]
        ...
        d.addCallback(lambda ign: self.rootnode.create_subdirectory("subdir"))
        d.addCallback(_stash_uri, "subdir")
        d.addCallback(
            lambda subdir_node: subdir_node.add_file(
                "grandchild", upload.Data(DATA + b"2", convergence=b"")
            )
        )
        d.addCallback(_stash_uri, "grandchild")

        d.addCallback(
            lambda ign: self.delete_shares_numbered(
                self.uris["subdir"], list(range(1, 10))
            )
        )
        ...
        def _check_broken_manifest(res):
            ...
            error_msg_s = "\n".join(error_msg) + "\n"
            self.failUnlessIn(
                "ERROR: UnrecoverableFileError(no recoverable versions)", error_line
            )
    ...
    def test_deep_check_and_repair(self):
        ...
        def _done(res):
            ...
            self.failUnlessReallyEqual(
                u0crr["pre-repair-results"]["results"]["count-happiness"], 10
            )
            self.failUnlessReallyEqual(
                u0crr["pre-repair-results"]["results"]["count-shares-good"], 10
            )
            ...
            self.failUnlessReallyEqual(
                ugoodcrr["pre-repair-results"]["results"]["count-happiness"], 10
            )
            self.failUnlessReallyEqual(
                ugoodcrr["pre-repair-results"]["results"]["count-shares-good"], 10
            )

            usick = [u for u in units if u["type"] == "file" and u["path"] == ["sick"]][
                0
            ]
            ...
            self.failUnlessReallyEqual(
                usickcrr["pre-repair-results"]["results"]["count-happiness"], 9
            )
            self.failUnlessReallyEqual(
                usickcrr["pre-repair-results"]["results"]["count-shares-good"], 9
            )
            self.failUnlessReallyEqual(
                usickcrr["post-repair-results"]["results"]["count-happiness"], 10
            )
            self.failUnlessReallyEqual(
                usickcrr["post-repair-results"]["results"]["count-shares-good"], 10
            )
    ...
    def _assert_leasecount(self, lease_counts, expected):
        for (fn, num_leases) in lease_counts:
            if num_leases != expected:
                self.fail(
                    "expected %d leases, have %d, on %s" % (expected, num_leases, fn)
                )
    ...
    def test_add_lease(self):
        ...
        d.addCallback(
            lambda ign: c0.create_mutable_file(publish.MutableData(DATA + b"2"))
        )
        ...
        d.addCallback(self.CHECK, "mutable", "t=check&add-lease=true", clientnum=1)
        d.addCallback(_got_html_good)
    ...
    def test_deep_add_lease(self):
        ...
        def _stash_root_and_create_file(n):
            ...
            self.fileurls["root"] = "uri/" + url_quote(n.get_uri())
            return n.add_file("one", upload.Data(DATA, convergence=b""))
        ...
        d.addCallback(
            lambda ign: c0.create_mutable_file(publish.MutableData(b"mutable"))
        )
        d.addCallback(lambda fn: self.rootnode.set_node("mutable", fn))
        ...
        d.addCallback(
            self.CHECK, "root", "t=stream-deep-check&add-lease=true", clientnum=1
        )
        d.addCallback(_done)
    ...
    def test_exceptions(self):
        ...
        d.addCallback(
            lambda ignored: self.shouldHTTPError(
                "GET unrecoverable",
                410,
                "Gone",
                "NoSharesError",
                self.GET,
                self.fileurls["0shares"],
            )
        )
        ...
        def _check_0shares_dir_html(body):
            ...
            # the dirops forms
            body = " ".join(body.strip().split())
            self.failUnlessIn('href="?t=info">More info on this directory', body)
            exp = (
    ...
    def test_blacklist(self):
        ...
        def _stash_uri_and_create_dir(ur):
            ...
            return c0.create_dirnode({"blacklisted.txt": (childnode, {})})
        ...
        d.addCallback(
            lambda ign: self.shouldHTTPError(
                "get_from_blacklisted_uri",
                403,
                "Forbidden",
        ...
        d.addCallback(
            lambda ign: self.GET(self.dir_url + b"?t=json", followRedirect=True)
        )
        ...
        def _add_dir(ign):
            childnode = c0.create_node_from_uri(self.uri, None)
            return c0.create_dirnode({"child": (childnode, {})})
        ...
        d.addCallback(
            lambda ign: self.shouldHTTPError(
                "get_from_blacklisted_dir json1",
                403,
                "Forbidden",
                "Access Prohibited: dir-off-limits",
                self.GET,
        ...
        d.addCallback(
            lambda ign: self.shouldHTTPError(
                "get_from_blacklisted_dir json2",
                403,
                "Forbidden",
                "Access Prohibited: dir-off-limits",
                self.GET,
        ...
        d.addCallback(
            lambda ign: self.shouldHTTPError(
                "get_from_blacklisted_dir json_ro",
                403,
                "Forbidden",
                "Access Prohibited: dir-off-limits",
                self.GET,
                self.dir_url_json_ro,
            )
        )
        d.addCallback(
            lambda ign: self.shouldHTTPError(
                "get_from_blacklisted_dir child",
                403,
                "Forbidden",
                "Access Prohibited: dir-off-limits",
                self.GET,
