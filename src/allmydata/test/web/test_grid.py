...
class Grid(
    ...
    def test_filecheck(self):
        ...
        d.addCallback(lambda ign: c0.upload(upload.Data(DATA + "1", convergence="")))
        d.addCallback(_stash_uri, "sick")
        d.addCallback(lambda ign: c0.upload(upload.Data(DATA + "2", convergence="")))
        ...
        d.addCallback(
            lambda ign: c0.create_mutable_file(publish.MutableData(DATA + "3"))
        )
    ...
    def test_unknown(self, immutable=False):
        ...
        expected_info_url = "uri/%s?t=info" % urllib.quote(unknown_rwcap, safe="")
    ...
    def test_mutant_dirnodes_are_omitted(self):
        ...
        def _check_html(res):
            ...
            for td in soup.find_all(u"td"):
                ...
                self.assertEqual(
                    u"{}".format(len("one")),
                    td.findNextSibling().findNextSibling().text,
                )
                found = True
                break
            ...
            infos = list(
                a[u"href"] for a in soup.find_all(u"a") if a.text == u"More Info"
            )
    ...
    def test_deep_check(self):
        ...
        d.addCallback(
            lambda ign: self.rootnode.add_file(
                u"sick", upload.Data(DATA + "1", convergence="")
            )
        )
        d.addCallback(_stash_uri, "sick")
        ...
        def _done(res):
            ...
            ugood = [
                u for u in units if u["type"] == "file" and u["path"] == [u"good"]
            ][0]
        ...
        d.addCallback(
            lambda subdir_node: subdir_node.add_file(
                u"grandchild", upload.Data(DATA + "2", convergence="")
            )
        )
        d.addCallback(_stash_uri, "grandchild")

        d.addCallback(
            lambda ign: self.delete_shares_numbered(self.uris["subdir"], range(1, 10))
        )
    ...
    def test_deep_check_and_repair(self):
        ...
        def _done(res):
            ...
            usick = [
                u for u in units if u["type"] == "file" and u["path"] == [u"sick"]
            ][0]
    ...
    def test_add_lease(self):
        ...
        d.addCallback(
            lambda ign: c0.create_mutable_file(publish.MutableData(DATA + "2"))
        )
    ...
    def test_deep_add_lease(self):
        ...
        d.addCallback(
            lambda ign: c0.create_mutable_file(publish.MutableData("mutable"))
        )
    ...
    def test_blacklist(self):
        ...
        d.addCallback(
            lambda ign: self.GET(self.dir_url + "?t=json", followRedirect=True)
        )
