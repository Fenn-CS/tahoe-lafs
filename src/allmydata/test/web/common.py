...
unknown_rwcap = "lafs://from_the_future_rw_\u263A".encode("utf-8")
unknown_rocap = "ro.lafs://readonly_from_the_future_ro_\u263A".encode("utf-8")
unknown_immcap = "imm.lafs://immutable_from_the_future_imm_\u263A".encode("utf-8")
...
def assert_soup_has_favicon(testcase, soup):
    ...
    testcase.assert_(any(t["href"] == "/icon.png" for t in links), soup)
...
def assert_soup_has_tag_with_attributes(testcase, soup, tag_name, attrs):
    ...
    for tag in tags:
        if all(v in tag.attrs.get(k, []) for k, v in attrs.items()):
            ...
            return tag
    testcase.fail("No <{}> tags contain attributes: {}".format(tag_name, attrs))
...
def assert_soup_has_tag_with_content(testcase, soup, tag_name, content):
    ...
    for tag in tags:
        ...
        if content in _normalized_contents(tag):
            return
    testcase.fail("No <{}> tag contains the text '{}'".format(tag_name, content))
