...
class FakeNodeMaker(NodeMaker):
    ...
    def create_mutable_file(self, contents="", keysize=None, version=SDMF_VERSION):
...
class FakeClient(_Client):
    def __init__(self):
        ...
        self.storage_broker.test_add_server(
            "disconnected_nodeid",
            FakeDisplayableServer(
                serverid="disconnected_nodeid",
                nickname=u"disconnected_nickname \u263B",
                connected=False,
...
class WebMixin(TimezoneMixin):
    def setUp(self):
        ...
        def _then(res):
            ...
            self.public_root.set_uri(u"foo", foo.get_uri(), foo.get_readonly_uri())
            ...
            foo.set_uri(u"empty", res[3][1].get_uri(), res[3][1].get_readonly_uri())
            ...
            self.public_root.set_uri(
                u"reedownlee", rodir.get_readonly_uri(), rodir.get_readonly_uri()
            )
    ...
    def failUnlessIsFooJSON(self, res):
        ...
        kids = dict(
            [
                (unicode(name), value)
                for (name, value) in data[1]["children"].iteritems()
            ]
        )
        ...
        self.failUnlessReallyEqual(
            to_bytes(kids[u"bar.txt"][1]["ro_uri"]), self._bar_txt_uri
        )
        self.failUnlessReallyEqual(
            to_bytes(kids[u"bar.txt"][1]["verify_uri"]), self._bar_txt_verifycap
        )
        self.failUnlessIn("metadata", kids[u"bar.txt"][1])
        self.failUnlessIn("tahoe", kids[u"bar.txt"][1]["metadata"])
        self.failUnlessReallyEqual(
            kids[u"bar.txt"][1]["metadata"]["tahoe"]["linkcrtime"],
            self._bar_txt_metadata["tahoe"]["linkcrtime"],
        )
        self.failUnlessReallyEqual(
            to_bytes(kids[u"n\u00fc.txt"][1]["ro_uri"]), self._bar_txt_uri
        )
        self.failUnlessIn("quux.txt", kids)
        self.failUnlessReallyEqual(
            to_bytes(kids[u"quux.txt"][1]["rw_uri"]), self._quux_txt_uri
        )
        self.failUnlessReallyEqual(
            to_bytes(kids[u"quux.txt"][1]["ro_uri"]), self._quux_txt_readonly_uri
        )
...
class Web(
    ...
    def test_introducer_status(self):
        ...
        def _check_introducer_not_connected_unguessable(res):
            ...
            assert_soup_has_tag_with_content(
                self, soup, u"div", u"No introducers connected"
            )
        ...
        def _check_introducer_connected_unguessable(res):
            ...
            assert_soup_has_tag_with_content(
                self, soup, u"div", u"1 introducer connected"
            )
    ...
    def test_helper_status(self):
        ...
        def _check_no_helper(res):
            ...
            assert_soup_has_tag_with_attributes(
                ...
                u"img",
                {
                    u"src": u"img/connected-not-configured.png",
                    u"alt": u"Not Configured",
                },
            )
        ...
        def _check_helper_not_connected(res):
            soup = BeautifulSoup(res, "html5lib")
            assert_soup_has_tag_with_attributes_and_content(
                self, soup, u"div", u"pb://someHelper/[censored]", {u"class": u"furl"}
            )
    ...
    def test_storage(self):
        ...
        def _check(res):
            ...
            self.failUnlessIn(
                u'<li>Server Nickname: <span class="nickname mine">fake_nickname \u263A</span></li>',
                res_u,
            )
    ...
    def test_status(self):
        ...
        def _check_dl_json(res):
            ...
            self.failUnless(data["serverids"].has_key("1"), str(data["serverids"]))
            self.failUnlessEqual(
    ...
    def test_status_path_trailing_slashes(self):
        ...
        def _check_status(response):
            ...
            assert_soup_has_favicon(self, soup)
            assert_soup_has_tag_with_content(
                self, soup, u"title", u"Tahoe-LAFS - Recent and Active Operations"
            )
    ...
    def _check_status_subpath_result(self, result, expected_title):
        ...
        soup = BeautifulSoup(body, "html5lib")
        assert_soup_has_favicon(self, soup)
        assert_soup_has_tag_with_content(self, soup, u"title", expected_title)
    ...
    def test_status_up_subpath(self):
        ...
        d.addCallback(
            self._check_status_subpath_result, u"Tahoe-LAFS - File Upload Status"
        )
        return d
    ...
    def test_status_down_subpath(self):
        ...
        d.addCallback(
            self._check_status_subpath_result, u"Tahoe-LAFS - File Download Status"
        )
        return d
    ...
    def test_HEAD_FILEURL(self):
        ...
        def _got(res_and_status_and_headers):
            ...
            self.failUnlessReallyEqual(res, "")
            self.failUnlessReallyEqual(
                headers.getRawHeaders("content-length")[0], str(len(self.BAR_CONTENTS))
            )
            self.failUnlessReallyEqual(
    ...
    def test_PUT_NEWFILEURL(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessChildContentsAre(
                self._foo_node, u"new.txt", self.NEWFILE_CONTENTS
            )
        )
        return d
    ...
    def test_PUT_NEWFILEURL_range_bad(self):
        ...
        d.addCallback(lambda res: self.failIfNodeHasChild(self._foo_node, u"new.txt"))
        return d
    ...
    def test_PUT_NEWFILEURL_replace(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessChildContentsAre(
                self._foo_node, u"bar.txt", self.NEWFILE_CONTENTS
            )
        )
        return d
    ...
    def test_PUT_NEWFILEURL_mkdirs(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessChildContentsAre(
                fn, u"newdir/new.txt", self.NEWFILE_CONTENTS
            )
        )
        return d
    ...
    def test_CSS_FILE(self):
        ...
        def _check(res):
            CSS_STYLE = re.compile(
                "toolbar\s{.+text-align:\scenter.+toolbar-item.+display:\sinline",
                re.DOTALL,
            )
    ...
    def _check_upload_and_mkdir_forms(self, soup):
        ...
        for input_tag in soup.find_all(u"input"):
            if input_tag.get(u"id", u"") in desired_ids:
                ...
                if (
                    input_tag.get(u"name", u"") == u"t"
                    and input_tag.get(u"type", u"") == u"hidden"
                ):
    ...
    def test_GET_DIRURL(self):
        ...
        self.assertTrue(
            any(a.text == u"bar.txt" for a in soup.find_all(u"a", {u"href": bar_url}))
        )
        self.assertTrue(
            any(
        ...
        self.assertEqual(set(found), {u"unlink", u"rename/relink"})
    ...
    def test_GET_DIRURL_empty(self):
        ...
        mkdir_inputs = soup.find_all(
            u"input", {u"type": u"hidden", u"name": u"t", u"value": u"mkdir"}
        )
    ...
    def test_GET_DIRURL_literal(self):
        ...
        self.assertTrue(
            file_links[0][u"href"].endswith(
                u"/file/URI%3ALIT%3Akrugkidfnzsc4/@@named=/short"
            )
        )
    ...
    def test_PUT_NEWDIRURL(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessNodeHasChild(self._foo_node, u"newdir")
        )
        ...
        def _check(uri):
            ...
            d2.addCallback(
                lambda ign: self.failUnlessROChildURIIs(
                    n, u"child-imm", caps["filecap1"]
                )
            )
            d2.addCallback(
                lambda ign: self.failUnlessRWChildURIIs(
                    n, u"child-mutable", caps["filecap2"]
                )
            )
            d2.addCallback(
                lambda ign: self.failUnlessROChildURIIs(
                    n, u"child-mutable-ro", caps["filecap3"]
                )
            )
            d2.addCallback(
                lambda ign: self.failUnlessROChildURIIs(
                    n, u"unknownchild-ro", caps["unknown_rocap"]
                )
            )
            d2.addCallback(
                lambda ign: self.failUnlessRWChildURIIs(
                    n, u"unknownchild-rw", caps["unknown_rwcap"]
                )
            )
            d2.addCallback(
                lambda ign: self.failUnlessROChildURIIs(
                    n, u"unknownchild-imm", caps["unknown_immcap"]
                )
            )
            d2.addCallback(
                lambda ign: self.failUnlessRWChildURIIs(n, u"dirchild", caps["dircap"])
            )
            d2.addCallback(
                lambda ign: self.failUnlessROChildURIIs(
                    n, u"dirchild-lit", caps["litdircap"]
                )
            )
            d2.addCallback(
                lambda ign: self.failUnlessROChildURIIs(
                    n, u"dirchild-empty", caps["emptydircap"]
                )
            )
            return d2
    ...
    def test_POST_NEWDIRURL_initial_children_bad_format(self):
        ...
        yield self.assertHTTPError(
            url, 400, "Unknown format: foo", method="post", data=json.dumps(newkids)
        )
    ...
    def test_POST_NEWDIRURL_immutable(self):
        ...
        def _check(uri):
            ...
            d2.addCallback(
                lambda ign: self.failUnlessROChildURIIs(
                    n, u"dirchild-imm", caps["immdircap"]
                )
            )
            d2.addCallback(
                lambda ign: self.failUnlessROChildURIIs(
        ...
        d.addCallback(lambda res: self._foo_node.get(u"newdir"))
        d.addCallback(
            self.failUnlessROChildURIIs, u"unknownchild-imm", caps["unknown_immcap"]
        )
        d.addCallback(lambda res: self._foo_node.get(u"newdir"))
        ...
        d.addCallback(lambda res: self._foo_node.get(u"newdir"))
        d.addCallback(
            self.failUnlessROChildURIIs, u"dirchild-empty", caps["emptydircap"]
        )
    ...
    def test_PUT_NEWDIRURL_exists(self):
        ...
        d.addCallback(lambda res: self.failUnlessNodeHasChild(self._foo_node, u"sub"))
    ...
    def test_PUT_NEWDIRURL_mkdirs(self):
        ...
        d.addCallback(lambda res: self.failIfNodeHasChild(self._foo_node, u"newdir"))
        d.addCallback(
            lambda res: self.failUnlessNodeHasChild(self._foo_node, u"subdir")
        )
        d.addCallback(lambda res: self._foo_node.get_child_at_path(u"subdir/newdir"))
        d.addCallback(self.failUnlessNodeKeysAre, [])
        return d
    ...
    def test_PUT_NEWDIRURL_mkdirs_mdmf(self):
        ...
        d.addCallback(
            lambda ignored: self.failUnlessNodeHasChild(self._foo_node, u"subdir")
        )
        d.addCallback(
            lambda ignored: self.failIfNodeHasChild(self._foo_node, u"newdir")
        )
        d.addCallback(lambda ignored: self._foo_node.get_child_at_path(u"subdir"))
    ...
    def test_DELETE_DIRURL(self):
        ...
        d.addCallback(lambda res: self.failIfNodeHasChild(self.public_root, u"foo"))
        return d

    def test_DELETE_DIRURL_missing(self):
        d = self.DELETE(self.public_url + "/foo/missing")
        d.addBoth(self.should404, "test_DELETE_DIRURL_missing")
        d.addCallback(lambda res: self.failUnlessNodeHasChild(self.public_root, u"foo"))
        return d
    ...
    def test_POST_upload(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessChildContentsAre(
                fn, u"new.txt", self.NEWFILE_CONTENTS
            )
        )
        return d
    ...
    def test_POST_upload_mutable(self):
        ...
        def _check_page_json(res):
            ...
            children = dict(
                [
                    (unicode(name), value)
                    for (name, value) in parsed[1]["children"].iteritems()
                ]
            )
        ...
        d.addCallback(lambda res: self.GET("/uri/%s" % urllib.quote(self._mutable_uri)))
        ...
        def _got_headers(res_and_status_and_headers):
            ...
            self.failUnlessReallyEqual(res, "")
            self.failUnlessReallyEqual(
                headers.getRawHeaders("content-length")[0], str(len(NEW2_CONTENTS))
            )
            self.failUnlessReallyEqual(
                headers.getRawHeaders("content-type"), ["text/plain"]
            )
    ...
    def test_POST_upload_replace(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessChildContentsAre(
                fn, u"bar.txt", self.NEWFILE_CONTENTS
            )
        )
        return d
    ...
    def test_POST_upload_whendone(self):
        ...
        fn = self._foo_node
        yield self.failUnlessChildContentsAre(fn, u"new.txt", self.NEWFILE_CONTENTS)
    ...
    def test_POST_FILEURL_mdmf_check(self):
        ...
        d.addCallback(_check)
        quux_extension_url = "/uri/%s" % urllib.quote(
            "%s:3:131073" % self._quux_txt_uri
        )
    ...
    def test_POST_mkdirs_2(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessNodeHasChild(self._foo_node, u"bardir")
        )
    ...
    def test_POST_set_children(self, command_name="set_children"):
        ...
        def _then(res):
            ...
            self.failUnlessURIMatchesROChild(
                newuri10, self._foo_node, u"atomic_added_2"
            )
            self.failUnlessURIMatchesROChild(
                newuri11, self._foo_node, u"atomic_added_3"
            )
    ...
    def test_POST_link_uri(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessChildContentsAre(
                self._foo_node, u"new.txt", contents
            )
        )
        return d
    ...
    def test_POST_link_uri_replace(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessChildContentsAre(
                self._foo_node, u"bar.txt", contents
            )
        )
        return d
    ...
    def test_POST_link_uri_unknown_ro_good(self):
        ...
        d.addCallback(
            self.failUnlessURIMatchesROChild, self._foo_node, u"future-ro.txt"
        )
        return d
    ...
    def test_POST_link_uri_unknown_imm_good(self):
        ...
        d.addCallback(
            self.failUnlessURIMatchesROChild, self._foo_node, u"future-imm.txt"
        )
        return d
    ...
    def test_POST_rename_file(self):
        ...
        d.addCallback(lambda res: self.failIfNodeHasChild(self._foo_node, u"bar.txt"))
        d.addCallback(
            lambda res: self.failUnlessNodeHasChild(self._foo_node, u"wibble.txt")
        )
    ...
    def test_POST_rename_file_redundant(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessNodeHasChild(self._foo_node, u"bar.txt")
        )
        d.addCallback(lambda res: self.GET(self.public_url + "/foo/bar.txt"))
        d.addCallback(self.failUnlessIsBarDotTxt)
    ...
    def test_POST_rename_file_replace(self):
        ...
        d.addCallback(lambda res: self.failIfNodeHasChild(self._foo_node, u"bar.txt"))
        d.addCallback(lambda res: self.failUnlessNodeHasChild(self._foo_node, u"empty"))
    ...
    def test_POST_rename_dir(self):
        ...
        d.addCallback(lambda res: self.failIfNodeHasChild(self.public_root, u"foo"))
        d.addCallback(
            lambda res: self.failUnlessNodeHasChild(self.public_root, u"plunk")
        )
    ...
    def test_POST_relink_file(self):
        ...
        d.addCallback(lambda res: self.failIfNodeHasChild(self._foo_node, u"bar.txt"))
        d.addCallback(
            lambda res: self.failUnlessNodeHasChild(self._sub_node, u"bar.txt")
        )
    ...
    def test_POST_relink_file_new_name(self):
        ...
        d.addCallback(
            lambda res: self.failUnlessNodeHasChild(self._sub_node, u"wibble.txt")
        )
    ...
    def test_POST_relink_file_no_replace_explicitly_same_link(self):
        d = self.POST(
            ...
            from_name="bar.txt",
            to_name="bar.txt",
            to_dir=self.public_root.get_uri() + "/foo",
        )
        d.addCallback(
            lambda res: self.failUnlessNodeHasChild(self._foo_node, u"bar.txt")
        )
    ...
    def test_POST_relink_file_to_slash_fail(self):
        ...
        d.addCallback(
            lambda res: self.failIfNodeHasChild(self._sub_node, u"slash/fail.txt")
        )
        d.addCallback(
    ...
    def test_POST_relink_file_same_dir(self):
        ...
        d.addCallback(lambda res: self.failIfNodeHasChild(self._foo_node, u"bar.txt"))
        d.addCallback(
            lambda res: self.failUnlessNodeHasChild(self._sub_node, u"baz.txt")
        )
        d.addCallback(lambda res: self.GET(self.public_url + "/foo/baz.txt"))
        d.addCallback(self.failUnlessIsBarDotTxt)
        d.addCallback(lambda res: self.GET(self.public_url + "/foo/baz.txt?t=json"))
    ...
    def test_POST_relink_file_multi_level(self):
        ...
        d.addCallback(
            lambda res: self.POST(
                ...
                t="relink",
                from_name="bar.txt",
                to_dir=self.public_root.get_uri() + "/foo/sub/level2",
            )
        )
        d.addCallback(lambda res: self.failIfNodeHasChild(self._foo_node, u"bar.txt"))
        d.addCallback(lambda res: self.failIfNodeHasChild(self._sub_node, u"bar.txt"))
    ...
    def test_POST_relink_dir(self):
        ...
        d.addCallback(lambda res: self.failIfNodeHasChild(self._foo_node, u"empty"))
        d.addCallback(lambda res: self.failUnlessNodeHasChild(self._sub_node, u"empty"))
        d.addCallback(lambda res: self._sub_node.get_child_at_path(u"empty"))
        d.addCallback(lambda node: self.failUnlessNodeHasChild(node, u"bar.txt"))
    ...
    def test_GET_URI_form(self):
        ...
        data = yield self.GET("/uri?uri=%s&t=json" % self._foo_uri, followRedirect=True)
        ...
        def _made_dir(dn):
            ...
            d.addCallback(
                lambda res: self.failUnlessRWChildURIIs(
                    self.public_root, u"foo", new_uri
                )
            )
            return d
    ...
    def test_PUT_DIRURL_uri_noreplace(self):
        ...
        def _made_dir(dn):
            ...
            d.addCallback(
                lambda res: self.failUnlessRWChildURIIs(
                    self.public_root, u"foo", self._foo_uri
                )
            )
            return d
    ...
    def test_PUT_NEWFILEURL_uri_unknown_ro_good(self):
        ...
        d.addCallback(
            self.failUnlessURIMatchesROChild, self._foo_node, u"put-future-ro.txt"
        )
        return d
    ...
    def test_PUT_NEWFILEURL_uri_unknown_imm_good(self):
        ...
        d.addCallback(
            self.failUnlessURIMatchesROChild, self._foo_node, u"put-future-imm.txt"
        )
        return d
    ...
    def test_PUT_mkdir(self):
        ...
        def _check(uri):
            ...
            d2.addCallback(lambda res: self.GET("/uri/%s?t=json" % uri))
            return d2
    ...
    def test_PUT_update_at_offset(self):
        ...
        d.addCallback(
            lambda ignored: self.PUT(
                "/uri/%s?replace=True&offset=100" % self.filecap, "replaced and so on"
            )
        )
        ...
        d.addCallback(
            lambda ignored: self.PUT(
                "/uri/%s?offset=%d" % (self.filecap, len(self.new_data)),
                "puppies" * 100,
            )
        )
        d.addCallback(_get_data)
        d.addCallback(
            lambda results: self.failUnlessEqual(
                results, self.new_data + ("puppies" * 100)
            )
        )
        # and try replacing the beginning of the file
        d.addCallback(
            lambda ignored: self.PUT("/uri/%s?offset=0" % self.filecap, "begin")
        )
        d.addCallback(_get_data)
        d.addCallback(
            lambda results: self.failUnlessEqual(
                results, "begin" + self.new_data[len("begin") :] + ("puppies" * 100)
            )
        )
        return d
    ...
    def test_PUT_update_at_invalid_offset(self):
        ...
        yield self.assertHTTPError(url, 400, "Invalid offset", method="put", data="foo")
    ...
    def test_PUT_update_at_offset_immutable(self):
        ...
        yield self.assertHTTPError(url, 400, "immutable", method="put", data="foo")
