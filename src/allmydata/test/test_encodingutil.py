...
lumiere_nfc = "lumi\u00E8re"
Artonwall_nfc = "\u00C4rtonwall.mp3"
Artonwall_nfd = "A\u0308rtonwall.mp3"
...
from allmydata.util.encodingutil import (
    ...
    to_filepath,
    extend_filepath,
    unicode_from_filepath,
    unicode_segments_from,
    unicode_to_argv,
)
...
class EncodingUtilErrors(ReallyEqualMixin, unittest.TestCase):
    ...
    def test_argv_to_unicode(self):
        ...
        self.failUnlessRaises(
            usage.UsageError, argv_to_unicode, lumiere_nfc.encode("latin1")
        )
    ...
    def test_no_unicode_normalization(self):
        ...
        _reload()
        self.failUnlessReallyEqual(listdir_unicode("/dummy"), [Artonwall_nfd])
...
class EncodingUtilNonUnicodePlatform(unittest.TestCase):
    ...
    def test_listdir_unicode(self):
        ...
        _reload()
        self.failUnlessRaises(FilenameEncodingError, listdir_unicode, "/dummy")
        ...
        _reload()
        self.failUnlessRaises(FilenameEncodingError, listdir_unicode, "/" + lumiere_nfc)
...
class EncodingUtil(ReallyEqualMixin):
    ...
    def test_unicode_to_argv_py2(self):
        ...
        self.assertEqual(unicode_to_argv("abc"), "abc".encode(self.io_encoding))
    ...
    def test_listdir_unicode(self):
        ...
        try:
            "test".encode(self.filesystem_encoding)
        ...
        def call_os_listdir(path):
            if PY2:
                ...
                return [
                    d.decode(self.filesystem_encoding) if isinstance(d, bytes) else d
        ...
        _reload()
        filenames = listdir_unicode("/dummy")
...
class StdlibUnicode(unittest.TestCase):
    ...
    def test_mkdir_open_exists_abspath_listdir_expanduser(self):
        ...
        expanded = fileutil.expanduser("~/" + lumiere_nfc)
...
class QuoteOutput(ReallyEqualMixin, unittest.TestCase):
    ...
    def _check(self, inp, out, enc, optional_quotes, quote_newlines):
        ...
        self.failUnlessReallyEqual(
            quote_output(inp, encoding=enc, quote_newlines=quote_newlines), out
        )
        self.failUnlessReallyEqual(
            quote_output(
                inp, encoding=enc, quotemarks=False, quote_newlines=quote_newlines
            ),
            out2,
        )
        if out[0:2] == 'b"':
            ...
            try:
                ...
                return
            self.failUnlessReallyEqual(
                quote_output(unicode_inp, encoding=enc, quote_newlines=quote_newlines),
                out,
            )
            self.failUnlessReallyEqual(
                quote_output(
            ...
            try:
                ...
                return
            self.failUnlessReallyEqual(
                quote_output(bytes_inp, encoding=enc, quote_newlines=quote_newlines),
                out,
            )
            self.failUnlessReallyEqual(
                quote_output(
...
class QuotePaths(ReallyEqualMixin, unittest.TestCase):
    ...
    def test_quote_path(self):
        self.assertPathsEqual(quote_path(["foo", "bar"]), b"'foo/bar'")
        self.assertPathsEqual(quote_path(["foo", "bar"], quotemarks=True), b"'foo/bar'")
        self.assertPathsEqual(quote_path(["foo", "bar"], quotemarks=False), b"foo/bar")
        self.assertPathsEqual(quote_path(["foo", "\nbar"]), b'"foo/\\x0abar"')
        ...
        self.assertPathsEqual(
            quote_local_unicode_path("\\\\?\\C:\\foo"),
            win32_other(b"'C:\\foo'", b"'\\\\?\\C:\\foo'"),
        )
        self.assertPathsEqual(
            quote_local_unicode_path("\\\\?\\C:\\foo", quotemarks=True),
            win32_other(b"'C:\\foo'", b"'\\\\?\\C:\\foo'"),
        )
        self.assertPathsEqual(
            quote_local_unicode_path("\\\\?\\C:\\foo", quotemarks=False),
            win32_other(b"C:\\foo", b"\\\\?\\C:\\foo"),
        )
        self.assertPathsEqual(
            quote_local_unicode_path("\\\\?\\UNC\\foo\\bar"),
            win32_other(b"'\\\\foo\\bar'", b"'\\\\?\\UNC\\foo\\bar'"),
        )
        self.assertPathsEqual(
            quote_local_unicode_path("\\\\?\\UNC\\foo\\bar", quotemarks=True),
            win32_other(b"'\\\\foo\\bar'", b"'\\\\?\\UNC\\foo\\bar'"),
        )
        self.assertPathsEqual(
            quote_local_unicode_path("\\\\?\\UNC\\foo\\bar", quotemarks=False),
            win32_other(b"\\\\foo\\bar", b"\\\\?\\UNC\\foo\\bar"),
        )

    def test_quote_filepath(self):
        foo_bar_fp = FilePath(win32_other("C:\\foo\\bar", "/foo/bar"))
        self.assertPathsEqual(
            quote_filepath(foo_bar_fp), win32_other(b"'C:\\foo\\bar'", b"'/foo/bar'")
        )
        self.assertPathsEqual(
            quote_filepath(foo_bar_fp, quotemarks=True),
            win32_other(b"'C:\\foo\\bar'", b"'/foo/bar'"),
        )
        self.assertPathsEqual(
            quote_filepath(foo_bar_fp, quotemarks=False),
            win32_other(b"C:\\foo\\bar", b"/foo/bar"),
        )

        if sys.platform == "win32":
            foo_longfp = FilePath("\\\\?\\C:\\foo")
            self.assertPathsEqual(quote_filepath(foo_longfp), b"'C:\\foo'")
            self.assertPathsEqual(
                quote_filepath(foo_longfp, quotemarks=True), b"'C:\\foo'"
            )
            self.assertPathsEqual(
                quote_filepath(foo_longfp, quotemarks=False), b"C:\\foo"
            )


class FilePaths(ReallyEqualMixin, unittest.TestCase):
    def test_to_filepath(self):
        foo_u = win32_other("C:\\foo", "/foo")
    ...
    def test_extend_filepath(self):
        ...
        foo_ufp = FilePath(win32_other("C:\\foo", "/foo"))
        foo_bar_baz_u = win32_other("C:\\foo\\bar\\baz", "/foo/bar/baz")

        for foo_fp in (foo_bfp, foo_ufp):
            fp = extend_filepath(foo_fp, ["bar", "baz"])
    ...
    def test_unicode_segments_from(self):
        ...
        foo_bar_baz_ufp = FilePath(win32_other("C:\\foo\\bar\\baz", "/foo/bar/baz"))

        for foo_fp in (foo_bfp, foo_ufp):
            for foo_bar_baz_fp in (foo_bar_baz_bfp, foo_bar_baz_ufp):
                self.failUnlessReallyEqual(
                    unicode_segments_from(foo_bar_baz_fp, foo_fp), ["bar", "baz"]
                )
...
class Windows(EncodingUtil, unittest.TestCase):
    ...
    io_encoding = "utf-8"
    dirlist = ["Blah blah.txt", "test_file", "\xc4rtonwall.mp3"]
...
class MacOSXLeopard(EncodingUtil, unittest.TestCase):
    ...
    filesystem_encoding = "utf-8"
    io_encoding = "UTF-8"
    dirlist = ["A\u0308rtonwall.mp3", "Blah blah.txt", "test_file"]
...
class TestToFromStr(ReallyEqualMixin, unittest.TestCase):
    def test_to_bytes(self):
        ...
        self.failUnlessReallyEqual(
            to_bytes(b"\xFF"), b"\xFF"
        )  # passes through invalid UTF-8 -- is this what we want?
        self.failUnlessReallyEqual(to_bytes("lumi\u00E8re"), b"lumi\xc3\xa8re")
        self.failUnlessReallyEqual(to_bytes(None), None)

    def test_from_utf8_or_none(self):
        self.failUnlessRaises(AssertionError, from_utf8_or_none, "foo")
        self.failUnlessReallyEqual(from_utf8_or_none(b"lumi\xc3\xa8re"), "lumi\u00E8re")
