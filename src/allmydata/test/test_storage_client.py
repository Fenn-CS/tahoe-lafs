...
class PluginMatchedAnnouncement(SyncTestCase):
    ...
    def make_node(self, introducer_furl, storage_plugin, plugin_config):
        ...
        self.basedir.child("private").makedirs()
    ...
    def publish(self, server_id, announcement, introducer_client):
        for subscription in introducer_client.subscribed_to:
            if subscription.service_name == "storage":
                subscription.cb(
                    server_id, announcement, *subscription.args, **subscription.kwargs
                )
            ...
            "storage-options": [
                {
                    ...
                    "name": "tahoe-lafs-dummy-v2",
                    "storage-server-FURL": SOME_FURL,
                }
            ],
        }
            ...
            MatchesAll(
                ...
                MatchesStructure(
                    ...
                    announcement=Equals(
                        {
                            "name": plugin_name,
                            "storage-server-FURL": "pb://abcde@nowhere/fake",
                        }
                    ),
                ),
            ),
        ...
        self.assertTrue(
            verifyObject(
                ...
                _FoolscapStorage.from_announcement(
                    b"server-id",
                    SOME_FURL,
                    {"permutation-seed-base32": base32.b2a(b"permutationseed")},
                    NotStorageServer(),
                ),
            ),
        )
...
class StoragePluginWebPresence(AsyncTestCase):
    ...
    def test_plugin_resource_persistent_across_requests(self):
        ...
        values = {
            loads((yield do_http("get", url)))["value"],
            loads((yield do_http("get", url)))["value"],
        }
        self.assertThat(
            values,
...
""".format(
        ...
        self.failUnlessEqual(s.announcement, servers["storage"]["v0-1234-1"]["ann"])
        ...
        ann2 = {
            "service-name": "storage",
            "anonymous-storage-FURL": "pb://{}@nowhere/fake2".format(
                str(base32.b2a(b"1"), "utf-8")
            ),
    ...
    def test_static_permutation_seed_hashed(self):
        ...
        broker.set_static_servers({server_id.decode("ascii"): {"ann": ann}})
        s = broker.servers[server_id]
        self.assertEqual(s.get_permutation_seed(), hashlib.sha256(server_id).digest())
    ...
    def test_threshold_reached(self):
        ...
        def add_one_server(x):
            data["anonymous-storage-FURL"] = "pb://%s@spy:nowhere/fake" % (
                str(base32.b2a(b"%d" % x), "ascii"),
            )
            tub = new_tub()
            connects = []
