...
class Base62(unittest.TestCase):
    def _test_num_octets_that_encode_to_this_many_chars(self, chars, octets):
        assert (
            base62.num_octets_that_encode_to_this_many_chars(chars) == octets
        ), "%s != %s <- %s" % (
            octets,
            base62.num_octets_that_encode_to_this_many_chars(chars),
            chars,
        )
    ...
    def test_odd_sizes(self):
        for j in range(2 ** 6):
            ...
            assert len(asl) == base62.num_chars_that_this_many_octets_encode_to(
                numos
