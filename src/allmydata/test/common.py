...
class MemoryIntroducerClient(object):
    ...
    def publish(self, service_name, ann, signing_key):
        self.published_announcements.append(
            Announcement(
                service_name,
                ann,
...
def get_published_announcements(client):
    ...
    return list(
        announcement
        for introducer_client in client.introducer_clients
        for announcement in introducer_client.published_announcements
    )
...
class UseNode(object):
    ...
    introducer_furl = attr.ib(
        validator=attr.validators.instance_of(native_str), converter=six.ensure_str
    )
    ...
    def setUp(self):
        def format_config_items(config):
            return "\n".join(
                " = ".join((key, value)) for (key, value) in list(config.items())
            )
...
class FakeMutableFileNode(object):  # type: ignore # incomplete implementation
    ...
    def create(self, contents, key_generator=None, keysize=None, version=SDMF_VERSION):
        if version == MDMF_VERSION and isinstance(
            self.my_uri, (uri.ReadonlySSKFileURI, uri.WriteableSSKFileURI)
        ):
    ...
    def init_from_cap(self, filecap):
        ...
        self.my_uri = filecap
        self.storage_index = self.my_uri.get_storage_index()
        if isinstance(filecap, (uri.WriteableMDMFFileURI, uri.ReadonlyMDMFFileURI)):
...
def make_mutable_file_cap():
    return uri.WriteableSSKFileURI(writekey=os.urandom(16), fingerprint=os.urandom(32))


def make_mdmf_mutable_file_cap():
    return uri.WriteableMDMFFileURI(writekey=os.urandom(16), fingerprint=os.urandom(32))
...
def make_verifier_uri():
    return uri.SSKVerifierURI(
        storage_index=os.urandom(16), fingerprint=os.urandom(32)
    ).to_string()
...
def create_mutable_filenode(contents, mdmf=False, all_contents=None):
    ...
    filenode = FakeMutableFileNode(None, None, encoding_params, None, all_contents)
...
class WebErrorMixin(object):
        ...
        def _validate(f):
            ...
            if substring:
                ...
                self.failUnless(
                    ...
                    "%s: substring '%s' not in '%s'" % (which, substring, code_string),
                )
    ...
    @inlineCallbacks
    def assertHTTPError(
        self, url, code, response_substring, method="get", persistent=False, **args
    ):
        response = yield treq.request(method, url, persistent=persistent, **args)
...
def corrupt_field(data, offset, size, debug=False):
    if random.random() < 0.5:
        ...
        if debug:
            log.msg(
                "testing: corrupting offset %d, size %d flipping one bit orig: %r, newdata: %r"
                % (
                    offset,
                    size,
                    data[offset : offset + size],
                    newdata[offset : offset + size],
                )
            )
        return newdata
    else:
        ...
        if debug:
            log.msg(
                "testing: corrupting offset %d, size %d randomizing field, orig: %r, newval: %r"
                % (offset, size, data[offset : offset + size], newval)
            )
...
def _corrupt_offset_of_block_hashes_to_truncate_crypttext_hashes(data, debug=False):
    ...
    if sharevernum == 1:
        ...
        newval = (
            random.randrange(0, max(1, (curval // hashutil.CRYPTO_VAL_SIZE) // 2))
...
def _corrupt_offset_of_uri_extension_to_force_short_read(data, debug=False):
    ...
    if sharevernum == 1:
        if debug:
            log.msg(
                "testing: corrupting offset %d, size %d, changing %d to %d (len(data) == %d)"
                % (
                    0x2C,
                    4,
                    struct.unpack(">L", data[0x2C : 0x2C + 4])[0],
                    len(data) - 0x0C - 3,
                    len(data),
                )
            )
        ...
        if debug:
            log.msg(
                ...
                % (
                    0x48,
                    8,
                    struct.unpack(">Q", data[0x48 : 0x48 + 8])[0],
                    len(data) - 0x0C - 3,
                    len(data),
                )
            )
...
def _corrupt_mutable_share_data(data, debug=False):
    prefix = data[:32]
    assert prefix == MutableShareFile.MAGIC, (
        "This function is designed to corrupt mutable shares of v1, and the magic number doesn't look right: %r vs %r"
        % (prefix, MutableShareFile.MAGIC)
    )
    ...
    (
        ...
        ig_datalen,
        offsets,
    ) = unpack_header(data[data_offset:])
...
def _corrupt_share_data(data, debug=False):
    ...
    sharevernum = struct.unpack(">L", data[0x0C : 0x0C + 4])[0]
    assert sharevernum in (1, 2), (
        "This test is designed to corrupt immutable shares of v1 or v2 in specific ways, not v%d."
        % sharevernum
    )
    if sharevernum == 1:
...
def _corrupt_share_data_last_byte(data, debug=False):
    ...
    newdata = (
        data[:offset]
        + byteschr(ord(data[offset : offset + 1]) ^ 0xFF)
        + data[offset + 1 :]
    )
    if debug:
        log.msg(
...
def _corrupt_crypttext_hash_tree(data, debug=False):
    ...
    if sharevernum == 1:
        crypttexthashtreeoffset = struct.unpack(
            ">L", data[0x0C + 0x14 : 0x0C + 0x14 + 4]
        )[0]
        blockhashesoffset = struct.unpack(">L", data[0x0C + 0x18 : 0x0C + 0x18 + 4])[0]
    else:
        crypttexthashtreeoffset = struct.unpack(
            ">Q", data[0x0C + 0x24 : 0x0C + 0x24 + 8]
        )[0]
        blockhashesoffset = struct.unpack(">Q", data[0x0C + 0x2C : 0x0C + 0x2C + 8])[0]

    return corrupt_field(
        data,
        0x0C + crypttexthashtreeoffset,
        blockhashesoffset - crypttexthashtreeoffset,
        debug=debug,
    )
...
def _corrupt_crypttext_hash_tree_byte_x221(data, debug=False):
    ...
    return (
        data[: 0x0C + 0x221]
        + byteschr(ord(data[0x0C + 0x221 : 0x0C + 0x221 + 1]) ^ 0x02)
        + data[0x0C + 0x2210 + 1 :]
    )
...
def _corrupt_block_hashes(data, debug=False):
    ...
    return corrupt_field(
        data, 0x0C + blockhashesoffset, sharehashesoffset - blockhashesoffset
    )
...
def _corrupt_share_hashes(data, debug=False):
    ...
    return corrupt_field(
        data, 0x0C + sharehashesoffset, uriextoffset - sharehashesoffset
    )
...
def _corrupt_uri_extension(data, debug=False):
    ...
    if sharevernum == 1:
        uriextoffset = struct.unpack(">L", data[0x0C + 0x20 : 0x0C + 0x20 + 4])[0]
        uriextlen = struct.unpack(
            ">L", data[0x0C + uriextoffset : 0x0C + uriextoffset + 4]
        )[0]
    else:
        uriextoffset = struct.unpack(">Q", data[0x0C + 0x3C : 0x0C + 0x3C + 8])[0]
        uriextlen = struct.unpack(
            ">Q", data[0x0C + uriextoffset : 0x0C + uriextoffset + 8]
        )[0]
