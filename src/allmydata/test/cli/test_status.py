...
class Integration(GridTestMixin, CLITestMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        yield do_http(
            "get", "http://127.0.0.1:{}/status?t=json".format(self.client_webports[0])
        )
    ...
    def test_simple(self, http):
        ...
        values = [
            StringIO(
                json.dumps(
                    {
                        "active": list(marshal_json(item) for item in active_items),
                        "recent": list(marshal_json(item) for item in recent_items),
                    }
                )
            ),
            StringIO(
