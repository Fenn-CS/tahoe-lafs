...
class Check(GridTestMixin, CLITestMixin, unittest.TestCase):
    def test_check(self):
        ...
        def _check3(args):
            ...
            self.failUnless(
                "Summary: Unhealthy: 8 shares (enc 3-of-10)" in summary, summary
            )
    ...
    def test_deep_check(self):
        ...
        def _stash_root_and_create_file(n):
            ...
            return n.add_file("g\u00F6\u00F6d", upload.Data(DATA, convergence=b""))
        ...
        d.addCallback(lambda ign: c0.create_mutable_file(MutableData(DATA + b"1")))
        ...
        def _check1(args):
            ...
            lines = out.splitlines()
            self.failUnless(
                "done: 4 objects checked, 4 healthy, 0 unhealthy" in lines, out
            )
        ...
        d.addCallback(lambda ign: self.do_cli("deep-check", "--verbose", self.rooturi))
        ...
        def _clobber_shares(ignored):
            shares = self.find_uri_shares(self.uris["g\u00F6\u00F6d"])
            self.failUnlessReallyEqual(len(shares), 10)
            os.unlink(shares[0][2])
        ...
        def _check3(args):
            ...
            self.failUnless(
                (quoted_good + ": Not Healthy: 9 shares (enc 3-of-10)") in lines, out
            )
            self.failIf(self._corrupt_share_line in lines, out)
            self.failUnless(
                "done: 4 objects checked, 3 healthy, 1 unhealthy" in lines, out
            )

        d.addCallback(_check3)

        d.addCallback(
            lambda ign: self.do_cli("deep-check", "--verbose", "--verify", self.rooturi)
        )
        ...
        def _check4(args):
            ...
            self.failUnless(
                mutable.startswith("'mutable': Unhealthy: 9 shares (enc 3-of-10)"),
                mutable,
            )
            self.failUnless(self._corrupt_share_line in lines, out)
            self.failUnless(
            ...
            self.failUnless(
                "done: 4 objects checked, 2 healthy, 2 unhealthy" in lines, out
            )

        d.addCallback(_check4)

        d.addCallback(lambda ign: self.do_cli("deep-check", "--raw", self.rooturi))
        ...
        d.addCallback(lambda fn: fn.add_file("subfile", upload.Data(DATA + b"2", b"")))
        d.addCallback(
            lambda ign: self.delete_shares_numbered(
                self.uris["subdir"], list(range(10))
            )
        )
    ...
    def test_check_with_multiple_aliases(self):
        ...
        d.addCallback(
            lambda ign: self.do_cli("check", self.uriList[0], self.uriList[1])
        )
