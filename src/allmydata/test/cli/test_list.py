...
class List(GridTestMixin, CLITestMixin, unittest.TestCase):
    def test_list(self):
        ...
        def _stash_root_and_create_file(n):
            ...
            return n.add_file("g\u00F6\u00F6d", upload.Data(small, convergence=b""))
        ...
        d.addCallback(lambda ign: self.rootnode.create_subdirectory("1share"))
        d.addCallback(
            lambda n: self.delete_shares_numbered(n.get_uri(), list(range(1, 10)))
        )
        d.addCallback(lambda ign: self.rootnode.create_subdirectory("0share"))
        d.addCallback(
            lambda n: self.delete_shares_numbered(n.get_uri(), list(range(0, 10)))
        )
        d.addCallback(lambda ign: self.do_cli("add-alias", "tahoe", self.rooturi))
        ...
        def _check1(args):
            ...
            expected = sorted(
                [ensure_str("0share"), ensure_str("1share"), good_out_encoded]
            )
        ...
        def _check3(args):
            ...
            self.failUnlessIn(
                "could not be retrieved, because there were "
                "insufficient good shares.",
                err,
            )
            self.assertEqual(len(out), 0, out)
        ...
        def _check4(args):
            ...
            if good_out is None:
                ...
                self.failUnlessIn(quote_output("g\u00F6\u00F6d"), err)
                self.assertEqual(len(out), 0, out)
            else:
        ...
        if good_arg is not None:
            ...
            d.addCallback(
                lambda ign: self.do_cli("ls", "-l", self.rooturi + "/" + good_arg)
            )
            d.addCallback(_check4)
        ...
        d.addCallback(
            lambda ign: self.rootnode.move_child_to(
                "g\u00F6\u00F6d", self.rootnode, "good"
            )
        )
        ...
        def _check1_ascii(args):
            ...
            self.failUnlessReallyEqual(
                sorted(out.splitlines()), sorted(["0share", "1share", "good"])
            )
        ...
        def _check4_ascii(args):
            ...
            self.failIfIn(
                "-r-- %d -" % len(small), out, "trailing hyphen means unknown date"
            )
        ...
        def _create_unknown(ign):
            nm = c0.nodemaker
            kids = {"unknownchild-imm": (nm.create_from_cap(unknown_immcap), {})}
            return self.rootnode.create_subdirectory(
                "unknown", initial_children=kids, mutable=False
            )
    ...
    def _create_directory_structure(self):
        ...
        def _got_rootnode(n):
            ...
            # with add_file.
            immutable_data = upload.Data(b"immutable data" * 100000, convergence=b"")
            d3 = n.add_file("immutable", immutable_data)
            ds = [d1, d2, d3]
            ...
            def _made_files(args):
                ...
                d1 = n.set_node("mdmf", mdmf_node)
                d2 = n.set_node("sdmf", sdmf_node)
    ...
    def test_list_mdmf(self):
        ...
        d.addCallback(lambda ignored: self.do_cli("ls", self._dircap))
    ...
    def test_list_mdmf_json(self):
        ...
        self.set_up_grid(oneshare=True)
        d = self._create_directory_structure()
        d.addCallback(lambda ignored: self.do_cli("ls", "--json", self._dircap))
