...
class List(GridTestMixin, CLITestMixin, unittest.TestCase):
    def test_list(self):
        ...
        d.addCallback(lambda n: self.delete_shares_numbered(n.get_uri(), range(1, 10)))
        d.addCallback(lambda ign: self.rootnode.create_subdirectory(u"0share"))
        d.addCallback(lambda n: self.delete_shares_numbered(n.get_uri(), range(0, 10)))
        ...
        def _check1(args):
            ...
            if good_out is None:
                ...
                self.failUnlessReallyEqual(
                    sorted(out.splitlines()), sorted(["0share", "1share"])
                )
            else:
                self.failUnlessReallyEqual(rc, 0)
                self.failUnlessReallyEqual(err, "")
                self.failUnlessReallyEqual(
                    sorted(out.splitlines()), sorted(["0share", "1share", good_out])
                )
        ...
        def _create_unknown(ign):
            ...
            return self.rootnode.create_subdirectory(
                u"unknown", initial_children=kids, mutable=False
            )
    ...
    def _create_directory_structure(self):
        ...
        def _got_rootnode(n):
            ...
            # with add_file.
            immutable_data = upload.Data("immutable data" * 100000, convergence="")
