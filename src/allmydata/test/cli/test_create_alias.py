...
class CreateAlias(GridTestMixin, CLITestMixin, unittest.TestCase):
    def _test_webopen(self, args, expected_url):
        o = runner.Options()
        o.parseOptions(
            ["--node-directory", self.get_clientdir(), "webopen"] + list(args)
        )
        urls = []
    ...
    def test_create(self):
        ...
        for invalid in ["foo:bar", "foo bar", "foobar::"]:
            d.addCallback(
                lambda res, invalid=invalid: self.do_cli("create-alias", invalid)
            )
            d.addCallback(_check_invalid)
            d.addCallback(
                lambda res, invalid=invalid: self.do_cli(
                    "add-alias", invalid, self.two_uri
                )
            )
            d.addCallback(_check_invalid)
        ...
        def _test_urls(junk):
            ...
            self._test_webopen(
                ["-i", "tahoe:subdir"], self.tahoe_subdir_url + "?t=info"
            )
            self._test_webopen(["tahoe:subdir/"], self.tahoe_subdir_url + "/")
            self._test_webopen(["tahoe:subdir/file"], self.tahoe_subdir_url + "/file")
            self._test_webopen(
                ["--info", "tahoe:subdir/file"], self.tahoe_subdir_url + "/file?t=info"
            )
    ...
    def test_create_unicode(self):
        ...
        def _check_create_unicode(args):
            ...
            self.failUnlessIn("Alias %s created" % (quote_output_u(etudes_arg),), out)

            aliases = get_aliases(self.get_clientdir())
            self.failUnless(aliases["\u00E9tudes"].startswith(b"URI:DIR2:"))
        ...
        d.addCallback(
            lambda res: self.do_cli(
                "get",
                str(get_aliases(self.get_clientdir())["\u00E9tudes"], "ascii")
                + "/"
                + lumiere_arg,
                return_bytes=True,
            )
