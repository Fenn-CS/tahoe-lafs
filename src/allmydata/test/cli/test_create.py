...
class Config(unittest.TestCase):
    ...
    def test_client_hide_ip_no_i2p_txtorcon(self):
        ...
        txi2p = mock.patch(
            "allmydata.util.i2p_provider._import_txi2p", return_value=None
        )
        txtorcon = mock.patch(
            "allmydata.util.tor_provider._import_txtorcon", return_value=None
        )
        with txi2p, txtorcon:
            basedir = self.mktemp()
    ...
    def test_client_i2p_option_no_txi2p(self):
        ...
        with txi2p:
            basedir = self.mktemp()
            rc, out, err = yield run_cli(
                "create-node", "--listen=i2p", "--i2p-launch", basedir
            )
            self.assertTrue(rc != 0)
            self.assertTrue(
                "Specifying any I2P options requires the 'txi2p' module" in out
            )
    ...
    def test_client_tor_option_no_txtorcon(self):
        ...
        with txtorcon:
            basedir = self.mktemp()
            rc, out, err = yield run_cli(
                "create-node", "--listen=tor", "--tor-launch", basedir
            )
            self.assertTrue(rc != 0)
            self.assertTrue(
                "Specifying any Tor options requires the 'txtorcon' module" in out
            )
    ...
    def test_node_hide_ip(self):
        basedir = self.mktemp()
        rc, out, err = yield run_cli(
            "create-node", "--hide-ip", "--hostname=foo", basedir
        )
        cfg = read_config(basedir)
        self.assertEqual(cfg.getboolean("node", "reveal-IP-address"), False)
        self.assertEqual(cfg.get("connections", "tcp"), "tor")
    ...
    def test_node_hostname_port_location(self):
        ...
        self.assertEqual(str(e), "--hostname cannot be used with --location/--port")

    def test_node_listen_tcp_no_hostname(self):
        basedir = self.mktemp()
        e = self.assertRaises(
            usage.UsageError, parse_cli, "create-node", "--listen=tcp", basedir
        )
    ...
    def test_node_listen_none_errors(self):
        ...
        e = self.assertRaises(
            usage.UsageError, parse_cli, "create-node", "--listen=tcp,none", basedir
        )
        self.assertEqual(
    ...
    def test_node_listen_bad(self):
        basedir = self.mktemp()
        e = self.assertRaises(
            usage.UsageError, parse_cli, "create-node", "--listen=XYZZY,tcp", basedir
        )
        self.assertEqual(
            str(e), "--listen= must be none, or one/some of: tcp, tor, i2p"
        )
    ...
    def test_node_listen_tor_hostname(self):
        e = self.assertRaises(
            usage.UsageError, parse_cli, "create-node", "--listen=tor", "--hostname=foo"
        )
        self.assertEqual(str(e), "--listen= must be tcp to use --hostname")

    def test_node_port_only(self):
        e = self.assertRaises(
            usage.UsageError, parse_cli, "create-node", "--port=unix:/var/tahoe/socket"
        )
    ...
    def test_node_slow_tor(self):
        basedir = self.mktemp()
        d = defer.Deferred()
        with mock.patch("allmydata.util.tor_provider.create_config", return_value=d):
    ...
    def test_node_slow_i2p(self):
        basedir = self.mktemp()
        d = defer.Deferred()
        with mock.patch("allmydata.util.i2p_provider.create_config", return_value=d):
    ...
    def test_introducer_no_hostname(self):
        basedir = self.mktemp()
        e = self.assertRaises(usage.UsageError, parse_cli, "create-introducer", basedir)
    ...
    def test_introducer_hide_ip(self):
        basedir = self.mktemp()
        rc, out, err = yield run_cli(
            "create-introducer", "--hide-ip", "--hostname=foo", basedir
        )
        cfg = read_config(basedir)
        self.assertEqual(cfg.getboolean("node", "reveal-IP-address"), False)
    ...
    def test_introducer_hostname(self):
        basedir = self.mktemp()
        rc, out, err = yield run_cli("create-introducer", "--hostname=foo", basedir)
        cfg = read_config(basedir)
...
class Tor(unittest.TestCase):
    def test_default(self):
        ...
        with mock.patch(
            "allmydata.util.tor_provider.create_config", return_value=config_d
        ) as co:
    ...
    def test_launch(self):
        ...
        with mock.patch(
            ...
            rc, out, err = self.successResultOf(
                run_cli("create-node", "--listen=tor", "--tor-launch", basedir)
            )
        args = co.mock_calls[0][1]
        self.assertEqual(args[1]["listen"], "tor")
    ...
    def test_control_port(self):
        ...
        with mock.patch(
            ...
            rc, out, err = self.successResultOf(
                run_cli(
                    "create-node", "--listen=tor", "--tor-control-port=mno", basedir
                )
            )
        args = co.mock_calls[0][1]
        self.assertEqual(args[1]["listen"], "tor")
    ...
    def test_not_both(self):
        ...
        self.assertEqual(
            str(e), "use either --tor-launch or" " --tor-control-port=, not both"
        )

    def test_launch_without_listen(self):
        e = self.assertRaises(
            usage.UsageError, parse_cli, "create-node", "--listen=none", "--tor-launch"
        )
...
class I2P(unittest.TestCase):
    def test_default(self):
        ...
        with mock.patch(
            "allmydata.util.i2p_provider.create_config", return_value=dest_d
        ) as co:
            rc, out, err = self.successResultOf(
    ...
    def test_launch(self):
        e = self.assertRaises(
            usage.UsageError, parse_cli, "create-node", "--listen=i2p", "--i2p-launch"
        )
    ...
    def test_sam_port(self):
        ...
        with mock.patch(
            ...
            rc, out, err = self.successResultOf(
                run_cli("create-node", "--listen=i2p", "--i2p-sam-port=mno", basedir)
            )
        args = co.mock_calls[0][1]
        self.assertEqual(args[1]["listen"], "i2p")
    ...
    def test_not_both(self):
        ...
        self.assertEqual(
            str(e), "use either --i2p-launch or" " --i2p-sam-port=, not both"
        )

    def test_launch_without_listen(self):
        e = self.assertRaises(
            usage.UsageError, parse_cli, "create-node", "--listen=none", "--i2p-launch"
        )
