...
class Update(GridTestMixin, unittest.TestCase, testutil.ShouldFailMixin):
    ...
    def do_upload_mdmf(self):
        d = self.nm.create_mutable_file(MutableData(self.data), version=MDMF_VERSION)
    ...
    def _check_differences(self, got, expected):
        ...
        gotspans = [
            "%d:%d=%r" % (start, end, got[start:end]) for (start, end) in gotmods
        ]
        expspans = [
            "%d:%d=%r" % (start, end, expected[start:end]) for (start, end) in expmods
        ]
    ...
    def test_append_power_of_two(self):
        ...
        def _run(ign):
            ...
            d.addCallback(lambda ign: self.mdmf_node.get_best_mutable_version())
            d.addCallback(
                lambda mv: mv.update(MutableData(segment * 2), len(self.data))
            )
            d.addCallback(lambda ign: self.mdmf_node.download_best_version())
    ...
    def test_update_sdmf(self):
        ...
        def _run(ign):
            ...
            d.addCallback(
                lambda mv: mv.update(MutableData(b"appended"), len(self.small_data))
            )
    ...
    def test_replace_in_last_segment(self):
        ...
        def _run(ign):
            ...
            d.addCallback(lambda ign: self.mdmf_node.get_best_mutable_version())
            d.addCallback(
                lambda mv: mv.update(MutableData(b"replaced"), replace_offset)
            )
            d.addCallback(lambda ign: self.mdmf_node.download_best_version())
            d.addCallback(lambda results: self.failUnlessEqual(results, new_data))
            return d
    ...
    def test_multiple_segment_replace(self):
        ...
        def _run(ign):
            ...
            d.addCallback(
                lambda mv: mv.update(
                    MutableData((2 * new_segment) + b"replaced"), replace_offset
                )
            )
            d.addCallback(lambda ignored: self.mdmf_node.download_best_version())
            d.addCallback(lambda results: self.failUnlessEqual(results, new_data))
