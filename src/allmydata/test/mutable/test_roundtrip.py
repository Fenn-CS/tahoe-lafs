...
class Roundtrip(unittest.TestCase, testutil.ShouldFailMixin, PublishMixin):
    ...
    def dump_servermap(self, servermap):
        ...
        print(
            "RECOVERABLE",
            [self.abbrev_verinfo(v) for v in servermap.recoverable_versions()],
        )
    ...
    def _test_corrupt_all(
        ...
        def _do_retrieve(servermap):
            ...
            if ver is None and not should_succeed:
                ...
                return servermap
            if should_succeed:
                d1 = self._fn.download_version(servermap, ver, fetch_privkey)
                d1.addCallback(
                    lambda new_contents: self.failUnlessEqual(
    ...
    def test_corrupt_all_pubkey(self):
        ...
        # to update it.
        self._fn._pubkey = None
        return self._test_corrupt_all("pubkey", "pubkey doesn't match fingerprint")
    ...
    def test_corrupt_all_block_hash_tree(self):
        return self._test_corrupt_all("block_hash_tree", "block hash tree failure")
    ...
    def OFF_test_corrupt_all_seqnum_late(self):
        ...
        return self._test_corrupt_all(
            1, "ran out of servers", corrupt_early=False, failure_checker=_check
        )
    ...
    def test_basic_pubkey_at_end(self):
        ...
        d = defer.succeed(None)
        d.addCallback(
            corrupt, self._storage, "pubkey", shnums_to_corrupt=list(range(0, N - k))
        )
        d.addCallback(lambda res: self.make_servermap())
        ...
        def _do_retrieve(servermap):
            ...
            self.failUnless(
                "pubkey doesn't match fingerprint" in str(servermap.get_problems()[0])
            )
            ver = servermap.best_recoverable_version()
        ...
        d.addCallback(lambda mc: b"".join(mc.chunks))
        d.addCallback(
            lambda new_contents: self.failUnlessEqual(new_contents, self.CONTENTS)
        )
        return d
    ...
    def _test_corrupt_some(self, offset, mdmf=False):
        if mdmf:
            ...
            d = defer.succeed(None)
        d.addCallback(
            lambda ignored: corrupt(None, self._storage, offset, list(range(5)))
        )
        d.addCallback(lambda ignored: self.make_servermap())
    ...
    def test_corrupt_mdmf_block_hash_tree(self):
        d = self.publish_mdmf()
        d.addCallback(
            lambda ignored: self._test_corrupt_all(
                ("block_hash_tree", 12 * 32),
                "block hash tree failure",
                corrupt_early=True,
                should_succeed=False,
            )
    ...
    def test_corrupt_some_mdmf(self):
        return self._test_corrupt_some(("share_data", 12 * 40), mdmf=True)
