...
class Version(GridTestMixin, unittest.TestCase, testutil.ShouldFailMixin, PublishMixin):
    def setUp(self):
    ...
    def do_upload_mdmf(self, data=None):
        if data is None:
            data = self.data
        d = self.nm.create_mutable_file(MutableData(data), version=MDMF_VERSION)
    ...
    def test_debug(self):
        ...
        def _debug(n):
            ...
            fso.si_s = str(
                storage_index, "utf-8"
            )  # command-line options are unicode on Python 3
            fso.nodedirs = [
                os.path.dirname(abspath_expanduser_unicode(str(storedir)))
    ...
    def test_get_sequence_number(self):
        ...
        d.addCallback(lambda bv: self.failUnlessEqual(bv.get_sequence_number(), 1))
        d.addCallback(lambda ignored: self.sdmf_node.get_best_readable_version())
        d.addCallback(lambda bv: self.failUnlessEqual(bv.get_sequence_number(), 1))
        ...
        d.addCallback(lambda ignored: self.mdmf_node.get_best_readable_version())
        d.addCallback(lambda bv: self.failUnlessEqual(bv.get_sequence_number(), 2))
        d.addCallback(lambda ignored: self.sdmf_node.get_best_readable_version())
        d.addCallback(lambda bv: self.failUnlessEqual(bv.get_sequence_number(), 2))
        return d
    ...
    def test_toplevel_overwrite(self):
        ...
        d.addCallback(lambda data: self.failUnlessEqual(data, b"foo bar baz" * 100000))
        ...
        d.addCallback(lambda data: self.failUnlessEqual(data, b"foo bar baz" * 10))
        return d
    ...
    def test_toplevel_modify(self):
        ...
        d.addCallback(lambda ignored: self.mdmf_node.download_best_version())
        d.addCallback(lambda data: self.failUnlessIn(b"modified", data))
        d.addCallback(lambda ignored: self.sdmf_node.modify(modifier))
        d.addCallback(lambda ignored: self.sdmf_node.download_best_version())
        d.addCallback(lambda data: self.failUnlessIn(b"modified", data))
        return d
    ...
    def test_download_version(self):
        ...
        d.addCallback(
            lambda res: self._set_versions(
                {0: 0, 2: 0, 4: 0, 6: 0, 8: 0, 1: 1, 3: 1, 5: 1, 7: 1, 9: 1}
            )
        )
        # Now try to download each version. We should get the plaintext
        # associated with that version.
        d.addCallback(lambda ignored: self._fn.get_servermap(mode=MODE_READ))
        ...
        d.addCallback(
            lambda ignored: self._fn.download_version(self.servermap, self.version1)
        )
        d.addCallback(
            lambda results: self.failUnlessEqual(
                self.CONTENTS[self.version1_index], results
            )
        )
        d.addCallback(
            lambda ignored: self._fn.download_version(self.servermap, self.version2)
        )
        d.addCallback(
            lambda results: self.failUnlessEqual(
                self.CONTENTS[self.version2_index], results
            )
        )
        return d
    ...
    def test_download_nonexistent_version(self):
        ...
        d.addCallback(
            lambda ignored: self.shouldFail(
                ...
                "nonexistent version",
                None,
                self.mdmf_node.download_version,
                self.servermap,
                "not a version",
            )
        )
    ...
    def _test_partial_read(self, node, expected, modes, step):
        ...
        def _read_data(version):
            ...
            d2.addCallback(
                lambda ignored: self.failUnlessEqual(expected, b"".join(c.chunks))
            )
            return d2
    ...
    def _test_read_and_download(self, node, expected):
        ...
        def _read_data(version):
            ...
            d2.addCallback(
                lambda ignored: version.read(c2, offset=0, size=len(expected))
            )
            d2.addCallback(
                lambda ignored: self.failUnlessEqual(expected, b"".join(c2.chunks))
            )
            return d2
