...
class T(unittest.TestCase):
    def do_test(self, size, required_shares, max_shares, fewer_shares=None):
        data0s = [
            os.urandom(mathutil.div_ceil(size, required_shares))
        ...
        def _check_data(decoded_shares):
            ...
            self.failUnless(
                b"".join(decoded_shares) == b"".join(data0s), "%s" % ("???",)
            )
        ...
        def _decode_multiple(res):
            ...
            sharesl1 = random.sample(
                list(zip(self.shares, self.shareids)), required_shares
            )
            shares1 = [x[0] for x in sharesl1]
            shareids1 = [x[1] for x in sharesl1]
            sharesl2 = random.sample(
                list(zip(self.shares, self.shareids)), required_shares
            )
