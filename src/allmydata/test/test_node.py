...
class TestCase(testutil.SignalMixin, unittest.TestCase):
    ...
    def _test_location(
        ...
        if local_addresses is not None:
            self.patch(iputil, "get_local_addresses_sync", lambda: local_addresses)
    ...
    def test_location1(self):
        return self._test_location(
            expected_addresses=["192.0.2.0:1234"], tub_location="192.0.2.0:1234"
        )
    ...
    def test_config_required(self):
        ...
        """
        basedir = "test_node/test_config_required"
    ...
    def test_config_items(self):
        ...
        """
        basedir = "test_node/test_config_items"
        create_node_dir(basedir, "testing")
    ...
    def test_private_config_unreadable(self):
        ...
        """
        basedir = "test_node/test_private_config_unreadable"
        create_node_dir(basedir, "testing")
        config = read_config(basedir, "portnum")
    ...
    def test_private_config_unreadable_preexisting(self):
        ...
        """
        basedir = "test_node/test_private_config_unreadable_preexisting"
        create_node_dir(basedir, "testing")
        config = read_config(basedir, "portnum")
        fname = os.path.join(basedir, "private", "foo")
    ...
    def test_private_config_missing(self):
        ...
        """
        basedir = "test_node/test_private_config_missing"
        create_node_dir(basedir, "testing")
        config = read_config(basedir, "portnum")
    ...
    def test_set_config_new_section(self):
        ...
        """
        basedir = self.mktemp()
        new_config = config_from_string(
            basedir, "", "", ValidConfiguration.everything()
        )
...
class TestMissingPorts(unittest.TestCase):
    ...
    def test_disabled_port_not_tub(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            _tub_portlocation(config)
        self.assertIn("tub.port is disabled, but not tub.location", str(ctx.exception))
    ...
    def test_disabled_tub_not_port(self):
        ...
        with self.assertRaises(ValueError) as ctx:
            _tub_portlocation(config)
        self.assertIn("tub.location is disabled, but not tub.port", str(ctx.exception))
...
class Listeners(unittest.TestCase):
    def test_listen_on_zero(self):
        ...
        with mock.patch("allmydata.node.Tub", return_value=t):
            with self.assertRaises(ValueError) as ctx:
                create_main_tub(
                    config, tub_options, dfh, fch, i2p_provider, tor_provider
                )
        self.assertIn(
            "you must choose",
            str(ctx.exception),
    ...
    def test_multiple_ports(self, ports):
        ...
        port = "tcp:%d:interface=127.0.0.1,tcp:%d:interface=127.0.0.1" % (port1, port2)
