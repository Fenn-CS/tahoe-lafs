    ...
    def test_random(self):
        ...
        def _create(subseed):
            ...
            for i in range(10):
                ...
                ns1.add(start, length)
                ns2.add(start, length)
            return ns1, ns2
        ...
        for i in range(1000):
            ...
            if op in b"0":
                ...
                s1.remove(start, length)
                s2.remove(start, length)
            elif op in b"78":
...
class SimpleDataSpans(object):
    ...
    def get_spans(self):
        return SimpleSpans([(start, len(data)) for (start, data) in self.get_chunks()])
    ...
    def remove(self, start, length):
        self.missing = replace(
            extend(self.missing, start, length, "1"), start, "1" * length
        )

    def add(self, start, data):
        self.missing = replace(
            extend(self.missing, start, len(data), "1"), start, "0" * len(data)
        )
        self.data = replace(extend(self.data, start, len(data), b" "), start, data)
                ...
                for t_start in range(l):
                    for t_len in range(1, 4):
                        ...
                        if d is not None:
                            ...
                            self.failUnlessEqual(
                                d, S[t_start : t_start + t_len], which2
                            )
                        ...
                        for i in range(l):
                            exp = ((i in p_elements) or (i in p_added)) and (
                                i not in removed
                            )
                            which2 = "%s-(%d-%d)" % (
                                which,
                            ...
                            self.failUnlessEqual(
                                bool(b2.get(i, 1)), exp, which2 + " %d" % i
                            )
    ...
    def test_random(self):
        ...
        for i in range(1000):
            ...
            if op in b"0":
                ...
                d1 = s1.pop(start, length)
                d2 = s2.pop(start, length)
            ...
            for j in range(100):
                ...
                start = int(what[2:4], 16)
                length = max(1, int(what[5:6], 16))
                d1 = s1.get(start, length)
                d2 = s2.get(start, length)
