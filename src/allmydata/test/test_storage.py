...
class Bucket(unittest.TestCase):
    ...
    def make_lease(self):
        ...
        return LeaseInfo(
            owner_num, renew_secret, cancel_secret, expiration_time, b"\x00" * 20
        )

    def test_create(self):
        incoming, final = self.make_workdir("test_create")
        bw = BucketWriter(self, incoming, final, 200, self.make_lease(), FakeCanary())
    ...
    def test_read_past_end_of_share_data(self):
        ...
        read_length = (
            len(share_data) + len(ownernumber) + len(renewsecret) + len(cancelsecret)
        )
...
class BucketProxy(unittest.TestCase):
    def make_bucket(self, name, size):
        ...
        fileutil.make_dirs(basedir)
        fileutil.make_dirs(os.path.join(basedir, "tmp"))
        bw = BucketWriter(self, incoming, final, size, self.make_lease(), FakeCanary())
        rb = RemoteBucket(bw)
        return bw, rb, final
    ...
    def _do_test_readwrite(self, name, header_size, wbp_class, rbp_class):
        ...
        sharesize = (
            header_size + 100 + 7 * 32 + 7 * 32 + 7 * 32 + 3 * (2 + 32) + 4 + 500
        )
        ...
        share_hashes = [
            (i, hashutil.tagged_hash(b"share", b"bar%d" % i)) for i in (1, 9, 13)
        ]
        ...
        def _start_reading(res):
            ...
            d1.addCallback(lambda res: self.failUnlessEqual(res, crypttext_hashes))
            ...
            d1.addCallback(lambda res: self.failUnlessEqual(res, uri_extension))
    ...
    def test_readwrite_v1(self):
        return self._do_test_readwrite(
            "test_readwrite_v1", 0x24, WriteBucketProxy, ReadBucketProxy
        )

    def test_readwrite_v2(self):
        return self._do_test_readwrite(
            "test_readwrite_v2", 0x44, WriteBucketProxy_v2, ReadBucketProxy
        )
...
class Server(unittest.TestCase):
    ...
    def allocate(self, ss, storage_index, sharenums, size, canary=None):
        ...
        if not canary:
            canary = FakeCanary()
        return ss.remote_allocate_buckets(
            storage_index, renew_secret, cancel_secret, sharenums, size, canary
        )
    ...
    def test_large_share(self):
        ...
        if "cygwin" in syslow or "windows" in syslow or "darwin" in syslow:
            raise unittest.SkipTest(
                "If your filesystem doesn't support efficient sparse files then it is very expensive (Mac OS X and Windows don't support efficient sparse files)."
            )

        avail = fileutil.get_available_space(".", 512 * 2 ** 20)
        if avail <= 4 * 2 ** 30:
            raise unittest.SkipTest(
                "This test will spuriously fail if you have less than 4 GiB free on your filesystem."
            )
    ...
    def test_dont_overfill_dirs(self):
        ...
        for i, wb in writers.items():
            ...
            wb.remote_close()
        storedir = os.path.join(self.workdir("test_dont_overfill_dirs"), "shares")
    ...
    def test_bad_container_version(self):
        ...
        e = self.failUnlessRaises(
            UnknownImmutableContainerVersionError, ss.remote_get_buckets, b"si1"
        )
    ...
    def test_leases(self):
        ...
        already, writers = ss.remote_allocate_buckets(
            b"si0", rs0, cs0, sharenums, size, canary
        )
        ...
        already, writers = ss.remote_allocate_buckets(
            b"si1", rs1, cs1, sharenums, size, canary
        )
        for wb in writers.values():
            wb.remote_close()
        ...
        already, writers = ss.remote_allocate_buckets(
            b"si1", rs2, cs2, sharenums, size, canary
        )
        ...
        self.failUnlessEqual(
            set([l.renew_secret for l in leases]), set([rs1, rs2, rs2a])
        )
        ...
        # ticket #1528.
        self.failIf(
            hasattr(ss, "remote_cancel_lease"),
        ...
        already, writers = ss.remote_allocate_buckets(
            b"si3", rs3, cs3, sharenums, size, canary
        )
        self.failUnlessEqual(len(already), 0)
        self.failUnlessEqual(len(writers), 5)
        already2, writers2 = ss.remote_allocate_buckets(
            b"si3", rs4, cs4, sharenums, size, canary
        )
    ...
    def test_advise_corruption(self):
        ...
        ss.remote_advise_corrupt_share(
            b"immutable", b"si0", 0, b"This share smells funny.\n"
        )
...
class MutableServer(unittest.TestCase):
    ...
    def allocate(self, ss, storage_index, we_tag, lease_tag, sharenums, size):
        ...
        testandwritev = dict([(shnum, ([], [], None)) for shnum in sharenums])
        readv = []
        rc = rstaraw(
            storage_index,
    ...
    def test_bad_magic(self):
        ...
        e = self.failUnlessRaises(
            UnknownMutableContainerVersionError, read, b"si1", [0], [(0, 10)]
        )
    ...
    def test_container_size(self):
        ...
        self.allocate(ss, b"si1", b"we1", next(self._lease_secret), set([0, 1, 2]), 100)
        read = ss.remote_slot_readv
        rstaraw = ss.remote_slot_testv_and_readv_and_writev
        secrets = (
            self.write_enabler(b"we1"),
            self.renew_secret(b"we1"),
            self.cancel_secret(b"we1"),
        )
        data = b"".join([(b"%d" % i) * 10 for i in range(10)])
        answer = rstaraw(b"si1", secrets, {0: ([], [(0, data)], len(data) + 12)}, [])
        ...
        answer = rstaraw(b"si1", secrets, {0: ([], [(0, data)], None)}, [])
        self.failUnlessEqual(answer, (True, {0: [], 1: [], 2: []}))
        ...
        pattern = "".join([chr(i) for i in range(100)]).encode("utf-8")
        answer = rstaraw(b"si1", secrets, {0: ([], [(0, pattern)], None)}, [])
        assert answer == (True, {0: [], 1: [], 2: []})
        ...
        answer = rstaraw(b"si1", secrets, {0: ([], [(0, data)], 0)}, [])
        self.failUnlessEqual(answer, (True, {0: [], 1: [], 2: []}))
    ...
    def test_allocate(self):
        ...
        read = ss.remote_slot_readv
        self.failUnlessEqual(read(b"si1", [0], [(0, 10)]), {0: [b""]})
        self.failUnlessEqual(
            read(b"si1", [], [(0, 10)]), {0: [b""], 1: [b""], 2: [b""]}
        )
        self.failUnlessEqual(read(b"si1", [0], [(100, 10)]), {0: [b""]})
        ...
        answer = write(b"si1", secrets, {0: ([], [(0, data)], None)}, [])
        self.failUnlessEqual(answer, (True, {0: [], 1: [], 2: []}))

        self.failUnlessEqual(
            read(b"si1", [0], [(0, 20)]), {0: [b"00000000001111111111"]}
        )
        self.failUnlessEqual(read(b"si1", [0], [(95, 10)]), {0: [b"99999"]})
        # self.failUnlessEqual(s0.remote_get_length(), 100)

        bad_secrets = (b"bad write enabler", secrets[1], secrets[2])
        f = self.failUnlessRaises(
            BadWriteEnablerError, write, b"si1", bad_secrets, {}, []
        )
        self.failUnlessIn(
            "The write enabler was recorded by nodeid 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'.",
            str(f),
        )
    ...
    def test_operators(self):
        ...
        def reset():
            write(b"si1", secrets, {0: ([], [(0, data)], None)}, [])
    ...
    def test_readv(self):
        ...
        self.failUnlessEqual(answer, {0: [b"0" * 10], 1: [b"1" * 10], 2: [b"2" * 10]})
    ...
    def test_leases(self):
        ...
        bucket_dir = os.path.join(
            self.workdir("test_leases"), "shares", storage_index_to_dir(b"si1")
        )
        ...
        e = self.failUnlessRaises(
            IndexError, ss.remote_renew_lease, b"si1", secrets(20)[1]
        )
        e_s = str(e)
        ...
        write(b"si1", secrets(0), {0: ([], [(200, b"make me bigger")], None)}, [])
        self.compare_leases_without_timestamps(all_leases, list(s0.get_leases()))

        write(
            b"si1", secrets(0), {0: ([], [(500, b"make me really bigger")], None)}, []
        )
        self.compare_leases_without_timestamps(all_leases, list(s0.get_leases()))
...
class MDMFProxies(unittest.TestCase, ShouldFailMixin):
    def setUp(self):
        ...
        self.secrets = (
            self.write_enabler(b"we_secret"),
    ...
    def build_test_mdmf_share(self, tail_segment=False, empty=False):
        ...
        data = struct.pack(">BQ32s", 1, 0, self.root_hash)
        self.checkstring = data
        # Next, the encoding parameters
        if tail_segment:
            data += struct.pack(">BBQQ", 3, 10, 6, 33)
        elif empty:
            data += struct.pack(">BBQQ", 3, 10, 0, 0)
        else:
            data += struct.pack(">BBQQ", 3, 10, 6, 36)
        ...
        sharehashes_offset = encrypted_private_key_offset + len(self.encprivkey)
        ...
        verification_key_end = verification_key_offset + len(self.verification_key)
    ...
    def write_test_share_to_server(
        self, storage_index, tail_segment=False, empty=False
    ):
        """
    ...
    def write_sdmf_share_to_server(self, storage_index, empty=False):
    ...
    def test_read(self):
        ...
        for i in range(6):
            d.addCallback(lambda ignored, i=i: mr.get_block_and_salt(i))
            d.addCallback(_check_block_and_salt)

        d.addCallback(lambda ignored: mr.get_encprivkey())
        d.addCallback(
            lambda encprivkey: self.failUnlessEqual(self.encprivkey, encprivkey)
        )

        d.addCallback(lambda ignored: mr.get_blockhashes())
        d.addCallback(
            lambda blockhashes: self.failUnlessEqual(self.block_hash_tree, blockhashes)
        )

        d.addCallback(lambda ignored: mr.get_sharehashes())
        d.addCallback(
            lambda sharehashes: self.failUnlessEqual(self.share_hash_chain, sharehashes)
        )

        d.addCallback(lambda ignored: mr.get_signature())
        d.addCallback(lambda signature: self.failUnlessEqual(signature, self.signature))

        d.addCallback(lambda ignored: mr.get_verification_key())
        d.addCallback(
            lambda verification_key: self.failUnlessEqual(
                verification_key, self.verification_key
            )
        )

        d.addCallback(lambda ignored: mr.get_seqnum())
        d.addCallback(lambda seqnum: self.failUnlessEqual(seqnum, 0))

        d.addCallback(lambda ignored: mr.get_root_hash())
        d.addCallback(lambda root_hash: self.failUnlessEqual(self.root_hash, root_hash))

        d.addCallback(lambda ignored: mr.get_seqnum())
        d.addCallback(lambda seqnum: self.failUnlessEqual(0, seqnum))

        d.addCallback(lambda ignored: mr.get_encoding_parameters())
        ...
        d.addCallback(lambda ignored: mr.get_checkstring())
        d.addCallback(
            lambda checkstring: self.failUnlessEqual(checkstring, checkstring)
        )
        return d
    ...
    def test_get_block_with_invalid_segnum(self):
        ...
        d.addCallback(
            lambda ignored: self.shouldFail(
                LayoutInvalid, "test invalid segnum", None, mr.get_block_and_salt, 7
            )
        )
        return d
    ...
    def test_get_root_hash_first(self):
        ...
        mr = MDMFSlotReadProxy(self.storage_server, b"si1", 0)
        d = mr.get_root_hash()
        d.addCallback(lambda root_hash: self.failUnlessEqual(root_hash, self.root_hash))
        return d
    ...
    def test_get_checkstring_first(self):
        ...
        d.addCallback(
            lambda checkstring: self.failUnlessEqual(checkstring, self.checkstring)
        )
        return d
    ...
    def test_write_read_vectors(self):
        ...
        d.addCallback(_then)
        d.addCallback(lambda ignored: mw.finish_publishing())
    ...
    def test_private_key_after_share_hash_chain(self):
        ...
        d = defer.succeed(None)
        for i in range(6):
            d.addCallback(lambda ignored, i=i: mw.put_block(self.block, i, self.salt))
        d.addCallback(lambda ignored: mw.put_encprivkey(self.encprivkey))
        d.addCallback(lambda ignored: mw.put_sharehashes(self.share_hash_chain))

        # Now try to put the private key again.
        d.addCallback(
            lambda ignored: self.shouldFail(
                LayoutInvalid,
                "test repeat private key",
                None,
                mw.put_encprivkey,
                self.encprivkey,
            )
    ...
    def test_signature_after_verification_key(self):
        ...
        for i in range(6):
            d.addCallback(lambda ignored, i=i: mw.put_block(self.block, i, self.salt))
        d.addCallback(lambda ignored: mw.put_encprivkey(self.encprivkey))
        d.addCallback(lambda ignored: mw.put_blockhashes(self.block_hash_tree))
        d.addCallback(lambda ignored: mw.put_sharehashes(self.share_hash_chain))
        d.addCallback(lambda ignored: mw.put_root_hash(self.root_hash))
        d.addCallback(lambda ignored: mw.put_signature(self.signature))
        d.addCallback(lambda ignored: mw.put_verification_key(self.verification_key))
    ...
    def test_uncoordinated_write(self):
        ...
        d.addCallback(lambda ignored: _write_share(mw2))
    ...
    def test_write_test_vectors(self):
        ...
        d = mw.finish_publishing()
        d.addCallback(_check_failure)
        d.addCallback(lambda ignored: mw.set_checkstring(b""))
        d.addCallback(lambda ignored: mw.finish_publishing())
        d.addCallback(_check_success)
        return d
    ...
    def test_write(self):
        ...
        def _check_publish(results):
            ...
            self.failUnlessEqual(
                read(b"si1", [0], [(expected_private_key_offset, 7)]),
                {0: [self.encprivkey]},
            )
            ...
            self.failUnlessEqual(
                read(b"si1", [0], [(expected_block_hash_offset, 32 * 6)]),
                {0: [self.block_hash_tree_s]},
            )

            expected_share_hash_offset = expected_private_key_offset + len(
                self.encprivkey
            )
            self.failUnlessEqual(
                read(b"si1", [0], [(expected_share_hash_offset, (32 + 2) * 6)]),
                {0: [self.share_hash_chain_s]},
            )

            self.failUnlessEqual(read(b"si1", [0], [(9, 32)]), {0: [self.root_hash]})
            expected_signature_offset = expected_share_hash_offset + len(
                self.share_hash_chain_s
            )
            self.failUnlessEqual(len(self.signature), 9)
            self.failUnlessEqual(
                read(b"si1", [0], [(expected_signature_offset, 9)]),
                {0: [self.signature]},
            )

            expected_verification_key_offset = expected_signature_offset + len(
                self.signature
            )
            self.failUnlessEqual(len(self.verification_key), 6)
            self.failUnlessEqual(
                read(b"si1", [0], [(expected_verification_key_offset, 6)]),
                {0: [self.verification_key]},
            )

            signable = mw.get_signable()
            verno, seq, roothash, k, n, segsize, datalen = struct.unpack(
                ">BQ32sBBQQ", signable
            )
            ...
            self.failUnlessEqual(segsize, 6)
            self.failUnlessEqual(datalen, 36)
            expected_eof_offset = expected_block_hash_offset + len(
                self.block_hash_tree_s
            )
            ...
            self.failUnlessEqual(read(b"si1", [0], [(41, 1)]), {0: [expected_k]})
            expected_n = struct.pack(">B", 10)
            self.failUnlessEqual(read(b"si1", [0], [(42, 1)]), {0: [expected_n]})
            expected_segment_size = struct.pack(">Q", 6)
            self.failUnlessEqual(
                read(b"si1", [0], [(43, 8)]), {0: [expected_segment_size]}
            )
            expected_data_length = struct.pack(">Q", 36)
            self.failUnlessEqual(
                read(b"si1", [0], [(51, 8)]), {0: [expected_data_length]}
            )
            expected_offset = struct.pack(">Q", expected_private_key_offset)
            self.failUnlessEqual(read(b"si1", [0], [(59, 8)]), {0: [expected_offset]})
            expected_offset = struct.pack(">Q", expected_share_hash_offset)
            self.failUnlessEqual(read(b"si1", [0], [(67, 8)]), {0: [expected_offset]})
            expected_offset = struct.pack(">Q", expected_signature_offset)
            self.failUnlessEqual(read(b"si1", [0], [(75, 8)]), {0: [expected_offset]})
            expected_offset = struct.pack(">Q", expected_verification_key_offset)
            self.failUnlessEqual(read(b"si1", [0], [(83, 8)]), {0: [expected_offset]})
            expected_offset = struct.pack(
                ">Q", expected_verification_key_offset + len(self.verification_key)
            )
            self.failUnlessEqual(read(b"si1", [0], [(91, 8)]), {0: [expected_offset]})
            expected_offset = struct.pack(">Q", expected_sharedata_offset)
            self.failUnlessEqual(read(b"si1", [0], [(99, 8)]), {0: [expected_offset]})
            expected_offset = struct.pack(">Q", expected_block_hash_offset)
            self.failUnlessEqual(read(b"si1", [0], [(107, 8)]), {0: [expected_offset]})
            expected_offset = struct.pack(">Q", expected_eof_offset)
            self.failUnlessEqual(read(b"si1", [0], [(115, 8)]), {0: [expected_offset]})
    ...
    def _make_new_mw(self, si, share, datalength=36):
        ...
        # parameter is 3.
        mw = MDMFSlotWriteProxy(
            share, self.storage_server, si, self.secrets, 0, 3, 10, 6, datalength
        )
        return mw
    ...
    def test_write_rejected_with_invalid_blocksize(self):
        ...
        d = defer.succeed(None)
        d.addCallback(
            lambda ignored, invalid_block=invalid_block: self.shouldFail(
                LayoutInvalid,
                "test blocksize too small",
                None,
                mw.put_block,
                invalid_block,
                0,
        ...
        valid_block = b"a"
        d.addCallback(lambda ignored: mw.put_block(valid_block, 5, self.salt))
        return d
    ...
    def test_write_enforces_order_constraints(self):
        ...
        d = defer.succeed(None)
        for i in range(6):
            d.addCallback(lambda ignored, i=i: mw0.put_block(self.block, i, self.salt))
        ...
        d.addCallback(
            lambda ignored: self.shouldFail(
                LayoutInvalid,
                "share hash chain before " "private key",
                None,
                mw0.put_sharehashes,
        ...
        d.addCallback(lambda ignored: mw0.put_encprivkey(self.encprivkey))

        # Now write the block hashes and try again
        d.addCallback(lambda ignored: mw0.put_blockhashes(self.block_hash_tree))
        ...
        d.addCallback(
            lambda ignored: self.failUnlessRaises(LayoutInvalid, mw0.get_signable)
        )
        ...
        d.addCallback(
            lambda ignored: self.shouldFail(
                LayoutInvalid,
                "key before signature",
                None,
                mw0.put_verification_key,
                self.verification_key,
            )
        )

        # Now write the share hashes.
        d.addCallback(lambda ignored: mw0.put_sharehashes(self.share_hash_chain))
        # We should be able to write the root hash now too
        d.addCallback(lambda ignored: mw0.put_root_hash(self.root_hash))
        ...
        d.addCallback(lambda ignored: mw0.put_signature(self.signature))
        ...
        d.addCallback(
            lambda ignored: self.shouldFail(
                LayoutInvalid,
                "offsets before verification key",
                None,
                mw0.finish_publishing,
            )
        )

        d.addCallback(lambda ignored: mw0.put_verification_key(self.verification_key))
        return d
    ...
    def test_end_to_end(self):
        ...
        d.addCallback(lambda ignored: mr.get_checkstring())
        d.addCallback(
            lambda checkstring: self.failUnlessEqual(checkstring, mw.get_checkstring())
        )
        return d
    ...
    def test_reads_sdmf(self):
        ...
        mr = MDMFSlotReadProxy(self.storage_server, b"si1", 0)
        d = defer.succeed(None)
        d.addCallback(lambda ignored: mr.is_sdmf())
        d.addCallback(lambda issdmf: self.failUnless(issdmf))

        # What do we need to read?
        #  - The sharedata
        #  - The salt
        d.addCallback(lambda ignored: mr.get_block_and_salt(0))
        ...
        d.addCallback(lambda ignored: mr.get_blockhashes())
        d.addCallback(
            lambda blockhashes: self.failUnlessEqual(
                self.block_hash_tree, blockhashes, blockhashes
            )
        )
        #  - The sharehashes
        d.addCallback(lambda ignored: mr.get_sharehashes())
        ...
        d.addCallback(lambda ignored: mr.get_encprivkey())
        d.addCallback(
            lambda encprivkey: self.failUnlessEqual(
                encprivkey, self.encprivkey, encprivkey
            )
        )
        d.addCallback(lambda ignored: mr.get_verification_key())
        d.addCallback(
            lambda verification_key: self.failUnlessEqual(
                verification_key, self.verification_key, verification_key
            )
        )
        #  - The signature
        d.addCallback(lambda ignored: mr.get_signature())
        d.addCallback(
            lambda signature: self.failUnlessEqual(signature, self.signature, signature)
        )

        #  - The sequence number
        d.addCallback(lambda ignored: mr.get_seqnum())
        d.addCallback(lambda seqnum: self.failUnlessEqual(seqnum, 0, seqnum))

        #  - The root hash
        d.addCallback(lambda ignored: mr.get_root_hash())
        d.addCallback(
            lambda root_hash: self.failUnlessEqual(root_hash, self.root_hash, root_hash)
        )
        return d
    ...
    def test_only_reads_one_segment_sdmf(self):
        ...
        d.addCallback(
            lambda ignored: self.shouldFail(
                LayoutInvalid, "test bad segment", None, mr.get_block_and_salt, 1
            )
        )
        return d
    ...
    def test_read_with_prefetched_mdmf_data(self):
        ...
        d.addCallback(lambda mr: mr.get_verinfo())
        ...
        def _check_verinfo(verinfo):
            ...
            expected_prefix = struct.pack(
                MDMFSIGNABLEHEADER, 1, seqnum, root_hash, k, n, segsize, datalen
            )
        ...
        d.addCallback(_make_mr, 123)
        d.addCallback(lambda mr: mr.get_block_and_salt(0))
    ...
    def test_read_with_prefetched_sdmf_data(self):
        ...
        def _check_verinfo(verinfo):
            self.failUnless(verinfo)
            self.failUnlessEqual(len(verinfo), 9)
            (seqnum, root_hash, salt, segsize, datalen, k, n, prefix, offsets) = verinfo
            self.failUnlessEqual(seqnum, 0)
            self.failUnlessEqual(root_hash, self.root_hash)
            self.failUnlessEqual(salt, self.salt)
            ...
            self.failUnlessEqual(n, 10)
            expected_prefix = struct.pack(
                SIGNED_PREFIX, 0, seqnum, root_hash, salt, k, n, segsize, datalen
            )
            self.failUnlessEqual(expected_prefix, prefix)
            self.failUnlessEqual(self.rref.read_count, 0)
    ...
    def test_read_with_empty_mdmf_file(self):
        ...
        d.addCallback(
            lambda ignored: self.shouldFail(
                LayoutInvalid, "get block on empty file", None, mr.get_block_and_salt, 0
            )
        )
        return d
    ...
    def test_verinfo_with_sdmf_file(self):
        ...
        d = defer.succeed(None)
        d.addCallback(lambda ignored: mr.get_verinfo())
        ...
        def _check_verinfo(verinfo):
            ...
            self.failUnlessEqual(n, 10)
            expected_prefix = struct.pack(
                ">BQ32s16s BBQQ", 0, seqnum, root_hash, salt, k, n, segsize, datalen
            )
    ...
    def test_verinfo_with_mdmf_file(self):
        ...
        def _check_verinfo(verinfo):
            self.failUnless(verinfo)
            self.failUnlessEqual(len(verinfo), 9)
            (seqnum, root_hash, IV, segsize, datalen, k, n, prefix, offsets) = verinfo
            self.failUnlessEqual(seqnum, 0)
            self.failUnlessEqual(root_hash, self.root_hash)
            self.failIf(IV)
            self.failUnlessEqual(segsize, 6)
            ...
            self.failUnlessEqual(n, 10)
            expected_prefix = struct.pack(
                ">BQ32s BBQQ", 1, seqnum, root_hash, k, n, segsize, datalen
            )
            self.failUnlessEqual(prefix, expected_prefix)
            self.failUnlessEqual(offsets, self.offsets)
    ...
    def test_sdmf_writer(self):
        ...
        sdmfr = SDMFSlotWriteProxy(
            0, self.storage_server, b"si1", self.secrets, 0, 3, 10, 36, 36
        )
        ...
        def _then(ignored):
            ...
            read = self.ss.remote_slot_readv
            self.failUnlessEqual(read(b"si1", [0], [(0, len(data))]), {0: [data]})
    ...
    def test_sdmf_writer_preexisting_share(self):
        ...
        sdmfw = SDMFSlotWriteProxy(
            0, self.storage_server, b"si1", self.secrets, 1, 3, 10, 36, 36
        )
        ...
        d.addCallback(lambda ignored: sdmfw.get_checkstring())
        d.addCallback(
            lambda checkstring: self.failUnlessEqual(
                checkstring, self._expected_checkstring
            )
        )
        d.addCallback(lambda ignored: sdmfw.finish_publishing())

        def _then_again(results):
            self.failUnless(results[0])
            read = self.ss.remote_slot_readv
            self.failUnlessEqual(
                read(b"si1", [0], [(1, 8)]), {0: [struct.pack(">Q", 1)]}
            )
            self.failUnlessEqual(
                read(b"si1", [0], [(9, len(data) - 9)]), {0: [data[9:]]}
            )
