...
class Handler(GridTestMixin, ShouldFailMixin, ReallyEqualMixin, unittest.TestCase):
    ...
    def shouldFailWithSFTPError(self, expected_code, which, callable, *args, **kwargs):
        ...
        def _done(res):
            if isinstance(res, Failure):
                ...
                print("@" + "@".join(s))
                self.fail(
                    "%s was supposed to raise SFTPError(%r), not get %r"
    ...
    def _set_up(self, basedir, num_clients=1, num_servers=10):
        ...
        self.set_up_grid(
            num_clients=num_clients, num_servers=num_servers, oneshare=True
        )
    ...
    def _set_up_tree(self):
        ...
        d.addCallback(lambda node: self.root.set_node("mutable", node))
        ...
        d.addCallback(
            lambda ign: self.root._create_and_validate_node(
                None, self.mutable.get_readonly_uri(), name="readonly"
            )
        )
        d.addCallback(lambda node: self.root.set_node("readonly", node))
        ...
        d.addCallback(lambda ign: self.root.add_file("gro\u00DF", gross))
        ...
        d.addCallback(
            lambda ign: self.root._create_and_validate_node(
                None, empty_litdir_uri, name="empty_lit_dir"
            )
        )
        ...
        d.addCallback(
            lambda ign: self.root._create_and_validate_node(
                None, tiny_litdir_uri, name="tiny_lit_dir"
            )
        )
        ...
        def _created_unknown(n):
            ...
            self.root.set_node("unknown", n)

        d.addCallback(_created_unknown)

        fall_of_the_Berlin_wall = calendar.timegm(
            time.strptime("1989-11-09 20:00:00 UTC", "%Y-%m-%d %H:%M:%S %Z")
        )
        md = {
            "mtime": fall_of_the_Berlin_wall,
            "tahoe": {"linkmotime": fall_of_the_Berlin_wall},
        }
        d.addCallback(lambda ign: self.root.set_node("loop", self.root, metadata=md))
        return d
    ...
    def test_basic(self):
        ...
        def _check(ign):
            ...
            self.failUnlessReallyEqual(self.handler._path_from_string(b"/."), [])
            self.failUnlessReallyEqual(self.handler._path_from_string(b"/./"), [])
            self.failUnlessReallyEqual(self.handler._path_from_string(b"foo"), ["foo"])
            self.failUnlessReallyEqual(self.handler._path_from_string(b"/foo"), ["foo"])
            self.failUnlessReallyEqual(self.handler._path_from_string(b"foo/"), ["foo"])
            self.failUnlessReallyEqual(
                self.handler._path_from_string(b"/foo/"), ["foo"]
            )
            self.failUnlessReallyEqual(
                self.handler._path_from_string(b"foo/bar"), ["foo", "bar"]
            )
            self.failUnlessReallyEqual(
                self.handler._path_from_string(b"/foo/bar"), ["foo", "bar"]
            )
            ...
            self.failUnlessReallyEqual(
                self.handler._path_from_string(b"foo/../bar"), ["bar"]
            )
            self.failUnlessReallyEqual(
                self.handler._path_from_string(b"/foo/../bar"), ["bar"]
            )
            self.failUnlessReallyEqual(
                self.handler._path_from_string(b"../bar"), ["bar"]
            )
            self.failUnlessReallyEqual(
                self.handler._path_from_string(b"/../bar"), ["bar"]
            )
            ...
            self.failUnlessReallyEqual(self.handler.realPath(b"foo/bar//"), b"/foo/bar")
            self.failUnlessReallyEqual(
                self.handler.realPath(b"/foo/bar//"), b"/foo/bar"
            )
            self.failUnlessReallyEqual(self.handler.realPath(b"foo/./bar"), b"/foo/bar")
            self.failUnlessReallyEqual(
                self.handler.realPath(b"./foo/./bar"), b"/foo/bar"
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "_path_from_string invalid UTF-8",
                self.handler._path_from_string,
                b"\xFF",
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "realPath invalid UTF-8",
                self.handler.realPath,
                b"\xFF",
            )
        )
    ...
    def test_convert_error(self):
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_FAILURE,
                "_convert_error SFTPError",
                sftpd._convert_error,
                Failure(sftp.SFTPError(sftp.FX_FAILURE, "foo")),
                "request",
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "_convert_error NoSuchChildError",
                sftpd._convert_error,
                Failure(NoSuchChildError("foo")),
                "request",
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_FAILURE,
                "_convert_error ExistingChildError",
                sftpd._convert_error,
                Failure(ExistingChildError("foo")),
                "request",
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "_convert_error NotWriteableError",
                sftpd._convert_error,
                Failure(NotWriteableError("foo")),
                "request",
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_OP_UNSUPPORTED,
                "_convert_error NotImplementedError",
                sftpd._convert_error,
                Failure(NotImplementedError("foo")),
                "request",
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_EOF,
                "_convert_error EOFError",
                sftpd._convert_error,
                Failure(EOFError("foo")),
                "request",
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_EOF,
                "_convert_error defer.FirstError",
                sftpd._convert_error,
                Failure(
                    defer.FirstError(Failure(sftp.SFTPError(sftp.FX_EOF, "foo")), 0)
                ),
                "request",
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_FAILURE,
                "_convert_error AssertionError",
                sftpd._convert_error,
                Failure(AssertionError("foo")),
                "request",
            )
        )
    ...
    def test_not_implemented(self):
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_OP_UNSUPPORTED, "readLink link", self.handler.readLink, b"link"
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
    ...
    def _compareDirLists(self, actual, expected):
        ...
        for (a, b) in zip(actual_list, expected_list):
            ...
            self.failUnless(
                re.match(expected_text_re, text),
    ...
    def test_openDirectory_and_attrs(self):
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "openDirectory nodir/nodir",
                self.handler.openDirectory,
                b"nodir/nodir",
            )
        )

        gross = "gro\u00DF".encode("utf-8")
        expected_root = [
            (
                b"empty_lit_dir",
                br"dr-xr-xr-x .* 0 .* empty_lit_dir$",
                {"permissions": S_IFDIR | 0o555},
            ),
            (
                gross,
                br"-rw-rw-rw- .* 1010 .* " + gross + b"$",
                {"permissions": S_IFREG | 0o666, "size": 1010},
            ),
            ...
            (
                b"mutable",
                br"-rw-rw-rw- .* 0 .* mutable$",
                {"permissions": S_IFREG | 0o666},
            ),
            (
                b"readonly",
                br"-r--r--r-- .* 0 .* readonly$",
                {"permissions": S_IFREG | 0o444},
            ),
            (
                b"small",
                br"-rw-rw-rw- .* 10 .* small$",
                {"permissions": S_IFREG | 0o666, "size": 10},
            ),
            (
                b"small2",
                br"-rw-rw-rw- .* 26 .* small2$",
                {"permissions": S_IFREG | 0o666, "size": 26},
            ),
            (
                b"tiny_lit_dir",
                br"dr-xr-xr-x .* 0 .* tiny_lit_dir$",
                {"permissions": S_IFDIR | 0o555},
            ),
        ...
        expected_tiny_lit = [
            (
                b"short",
                br"-r--r--r-- .* 8 (Jan 01  1970|Dec 31  1969) short$",
                {"permissions": S_IFREG | 0o444, "size": 8},
            ),
        ]
        ...
        d.addCallback(
            lambda attrs: self._compareAttributes(
                attrs, {"permissions": S_IFREG | 0o666, "size": 10}
            )
        )
    ...
    def test_openFile_read(self):
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "openFile nodir/file READ nosuch",
                self.handler.openFile,
                b"nodir/file",
                sftp.FXF_READ,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile unknown/file READ denied",
                self.handler.openFile,
                b"unknown/file",
                sftp.FXF_READ,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile tiny_lit_dir READ denied",
                self.handler.openFile,
                b"tiny_lit_dir",
                sftp.FXF_READ,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile unknown uri READ denied",
                self.handler.openFile,
                b"uri/" + self.unknown_uri,
                sftp.FXF_READ,
                {},
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile tiny_lit_dir uri READ denied",
                self.handler.openFile,
                b"uri/" + self.tiny_lit_dir_uri,
                sftp.FXF_READ,
                {},
            )
        )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile noexist uri READ denied",
                self.handler.openFile,
                b"uri/URI:noexist",
                sftp.FXF_READ,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "openFile invalid UTF-8 uri READ denied",
                self.handler.openFile,
                b"uri/URI:\xFF",
                sftp.FXF_READ,
                {},
        ...
        def _read_small(rf):
            ...
            d2.addCallback(
                lambda ign: rf.readChunk(8, 4)
            )  # read that starts before EOF is OK
            d2.addCallback(lambda data: self.failUnlessReallyEqual(data, b"89"))

            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_EOF,
                    "readChunk starting at EOF (0-byte)",
                    rf.readChunk,
                    10,
                    0,
                )
            ...
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_EOF, "readChunk starting at EOF", rf.readChunk, 10, 1
                )
            )
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_EOF, "readChunk starting after EOF", rf.readChunk, 11, 1
                )
            )
            ...
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_PERMISSION_DENIED,
                    "writeChunk on read-only handle denied",
                    rf.writeChunk,
                    0,
                    b"a",
                )
            ...
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_PERMISSION_DENIED,
                    "setAttrs on read-only handle denied",
                    rf.setAttrs,
                    {},
                )
            )
            ...
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_BAD_MESSAGE, "getAttrs on closed file bad", rf.getAttrs
                )
            )
        ...
        def _read_gross(rf):
            ...
            d2.addCallback(
                lambda ign: rf.readChunk(1008, 4)
            )  # read that starts before EOF is OK
            d2.addCallback(lambda data: self.failUnlessReallyEqual(data, b"89"))
            ...
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_EOF, "readChunk starting at EOF", rf.readChunk, 1010, 1
                )
            )
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_EOF, "readChunk starting after EOF", rf.readChunk, 1011, 1
                )
            )
            ...
            d2.addCallback(
                lambda attrs: self._compareAttributes(
                    attrs, {"permissions": S_IFREG | 0o666, "size": 1010}
                )
            )
            ...
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_BAD_MESSAGE, "readChunk on closed file", rf.readChunk, 0, 1
                )
            )
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_BAD_MESSAGE, "getAttrs on closed file", rf.getAttrs
                )
            )
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"uri/" + self.small_uri, sftp.FXF_READ, {}
            )
        )
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"uri/" + self.gross_uri, sftp.FXF_READ, {}
            )
        )
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"uri/" + self.mutable_uri, sftp.FXF_READ, {}
            )
        )
        ...
        def _read_mutable_uri(rf):
            ...
            d2.addCallback(
                lambda data: self.failUnlessReallyEqual(data, b"mutable file contents")
            )
            d2.addCallback(lambda ign: rf.close())
            return d2
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"uri/" + self.tiny_lit_dir_uri + b"/short", sftp.FXF_READ, {}
            )
        )
        ...
        def _read_broken(rf):
            ...
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_FAILURE, "read broken", rf.readChunk, 0, 100
                )
            )
    ...
    def test_openFile_read_error(self):
        ...
        d.addCallback(lambda ign: self.root.add_file("small", small))
        d.addCallback(
            lambda n: self.handler.openFile(b"/uri/" + n.get_uri(), sftp.FXF_READ, {})
        )
    ...
    def test_openFile_write(self):
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "openFile '' WRITE|CREAT|TRUNC nosuch",
                self.handler.openFile,
                b"",
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "openFile newfile WRITE|TRUNC nosuch",
                self.handler.openFile,
                b"newfile",
                sftp.FXF_WRITE | sftp.FXF_TRUNC,
                {},
            )
        )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_BAD_MESSAGE,
                "openFile small WRITE|EXCL bad",
                self.handler.openFile,
                b"small",
                sftp.FXF_WRITE | sftp.FXF_EXCL,
                {},
            )
        )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile tiny_lit_dir WRITE denied",
                self.handler.openFile,
                b"tiny_lit_dir",
                sftp.FXF_WRITE,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile unknown/newfile WRITE|CREAT denied",
                self.handler.openFile,
                b"unknown/newfile",
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile tiny_lit_dir/newfile WRITE|CREAT|TRUNC denied",
                self.handler.openFile,
                b"tiny_lit_dir/newfile",
                sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_TRUNC,
                {},
            )
        )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile tiny_lit_dir/short WRITE denied",
                self.handler.openFile,
                b"tiny_lit_dir/short",
                sftp.FXF_WRITE,
                {},
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile tiny_lit_dir/short WRITE|CREAT denied",
                self.handler.openFile,
                b"tiny_lit_dir/short",
                sftp.FXF_WRITE | sftp.FXF_CREAT,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile readonly WRITE denied",
                self.handler.openFile,
                b"readonly",
                sftp.FXF_WRITE,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile readonly uri WRITE denied",
                self.handler.openFile,
                b"uri/" + self.readonly_uri,
                sftp.FXF_WRITE,
                {},
            )
        )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_FAILURE,
                "openFile small WRITE|CREAT|EXCL failure",
                self.handler.openFile,
                b"small",
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_FAILURE,
                "openFile mutable WRITE|CREAT|EXCL failure",
                self.handler.openFile,
                b"mutable",
                sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_EXCL,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_FAILURE,
                "openFile mutable uri WRITE|CREAT|EXCL failure",
                self.handler.openFile,
                b"uri/" + self.mutable_uri,
                sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_EXCL,
                {},
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_FAILURE,
                "openFile tiny_lit_dir/short WRITE|CREAT|EXCL failure",
                self.handler.openFile,
                b"tiny_lit_dir/short",
                sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_EXCL,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile small uri WRITE denied",
                self.handler.openFile,
                b"uri/" + self.small_uri,
                sftp.FXF_WRITE,
                {},
            )
        )
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile small uri WRITE|CREAT denied",
                self.handler.openFile,
                b"uri/" + self.small_uri,
                sftp.FXF_WRITE | sftp.FXF_CREAT,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile small uri WRITE|CREAT|TRUNC denied",
                self.handler.openFile,
                b"uri/" + self.small_uri,
                sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_TRUNC,
                {},
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "openFile small uri WRITE|CREAT|EXCL denied",
                self.handler.openFile,
                b"uri/" + self.small_uri,
                sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_EXCL,
                {},
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"newfile", sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_TRUNC, {}
            )
        )
        ...
        def _write(wf):
            ...
            d2.addCallback(
                lambda attrs: self._compareAttributes(
                    attrs, {"permissions": S_IFREG | 0o666, "size": 16}
                )
            )
            ...
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_BAD_MESSAGE,
                    "setAttrs with negative size bad",
                    wf.setAttrs,
                    {"size": -1},
                )
            )
            ...
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_PERMISSION_DENIED,
                    "readChunk on write-only handle denied",
                    wf.readChunk,
                    0,
                    1,
                )
        ...
        d.addCallback(lambda ign: self.root.get("newfile"))
        d.addCallback(lambda node: download_to_data(node))
        d.addCallback(
            lambda data: self.failUnlessReallyEqual(
                data, b"012345670123\x00a\x00\x00\x00"
            )
        )
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"newfile",
                sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_TRUNC | sftp.FXF_APPEND,
                {},
            )
        )
        ...
        d.addCallback(
            lambda data: self.failUnlessReallyEqual(
                data, b"01234567890123\x00\x00\x00z"
            )
        )
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"newfile", sftp.FXF_WRITE | sftp.FXF_TRUNC, {}
            )
        )
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"newfile", sftp.FXF_WRITE | sftp.FXF_TRUNC, {"permissions": 0}
            )
        )
        d.addCallback(_write_trunc)
        d.addCallback(lambda ign: self.root.get("newfile"))
        d.addCallback(lambda node: download_to_data(node))
        d.addCallback(lambda data: self.failUnlessReallyEqual(data, b"01234"))
        d.addCallback(lambda ign: self.root.get_metadata_for("newfile"))
        d.addCallback(
            lambda metadata: self.failIf(metadata.get("no-write", False), metadata)
        )
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"excl",
                sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_TRUNC | sftp.FXF_EXCL,
                {},
            )
        )

        def _write_excl(wf):
            d2 = self.root.get("excl")
        ...
        d.addCallback(lambda ign: self.root.get("excl"))
        d.addCallback(lambda node: download_to_data(node))
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"zerolength", sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_EXCL, {}
            )
        )

        def _write_excl_zerolength(wf):
            d2 = self.root.get("zerolength")
            d2.addCallback(lambda node: download_to_data(node))
            d2.addCallback(lambda data: self.failUnlessReallyEqual(data, b""))
        ...
        d.addCallback(lambda ign: self.root.get("zerolength"))
        d.addCallback(lambda node: download_to_data(node))
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"exclappend",
                sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_EXCL | sftp.FXF_APPEND,
                {},
            )
        )

        def _write_excl_append(wf):
            d2 = self.root.get("exclappend")
            d2.addCallback(lambda node: download_to_data(node))
            d2.addCallback(lambda data: self.failUnlessReallyEqual(data, b""))
        ...
        d.addCallback(lambda ign: self.root.get("exclappend"))
        d.addCallback(lambda node: download_to_data(node))
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"creatappend", sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_APPEND, {}
            )
        )
        ...
        d.addCallback(lambda ign: self.root.get("creatappend"))
        d.addCallback(lambda node: download_to_data(node))
        d.addCallback(lambda data: self.failUnlessReallyEqual(data, b"012345678901234"))
        ...
        d.addCallback(lambda node: download_to_data(node))
        d.addCallback(
            lambda data: self.failUnlessReallyEqual(data, b"01234567890123401234")
        )

        # test WRITE | CREAT without TRUNC, when the file does not already exist
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"newfile2", sftp.FXF_WRITE | sftp.FXF_CREAT, {}
            )
        )
        ...
        d.addCallback(lambda ign: self.root.get("newfile2"))
        d.addCallback(lambda node: download_to_data(node))
        d.addCallback(lambda data: self.failUnlessReallyEqual(data, b"0123456789"))
        ...
        d.addCallback(lambda ign: self.root.set_node("mutable2", self.mutable))
        ...
        d.addCallback(lambda ign: self.root.get("mutable"))
        ...
        d.addCallback(
            lambda data: self.failUnlessReallyEqual(data, b"mutable new! contents")
        )

        # ... and with permissions, which should be ignored
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"mutable", sftp.FXF_WRITE, {"permissions": 0}
            )
        )
        d.addCallback(_write_mutable)
        d.addCallback(lambda ign: self.root.get("mutable"))
        ...
        def _write_mutable_setattr(wf):
            ...
            d2.addCallback(
                lambda ign: self.handler.setAttrs(b"mutable", {"permissions": 0o444})
            )

            d2.addCallback(lambda ign: self.root.get("mutable"))
            d2.addCallback(lambda node: self.failUnless(node.is_readonly()))

            d2.addCallback(lambda ign: wf.getAttrs())
            d2.addCallback(
                lambda attrs: self.failUnlessReallyEqual(
                    attrs["permissions"], S_IFREG | 0o666
                )
            )
            d2.addCallback(lambda ign: self.handler.getAttrs(b"mutable", followLinks=0))
            d2.addCallback(
                lambda attrs: self.failUnlessReallyEqual(
                    attrs["permissions"], S_IFREG | 0o444
                )
            )
        ...
        def _check_readonly_file(node):
            ...
            self.failUnlessReallyEqual(
                node.get_storage_index(), self.mutable.get_storage_index()
            )
            return node.download_best_version()
        ...
        d.addCallback(
            lambda data: self.failUnlessReallyEqual(
                data, b"mutable read-only link from parent"
            )
        )

        # ... and then by handle
        d.addCallback(
            lambda ign: self.handler.openFile(b"mutable2", sftp.FXF_WRITE, {})
        )
        ...
        def _write_mutable2_setattr(wf):
            ...
            d2.addCallback(lambda ign: self.root.get("mutable2"))
            ...
            d2.addCallback(
                lambda ign: self.handler.getAttrs(b"mutable2", followLinks=0)
            )
            d2.addCallback(
                lambda attrs: self.failUnlessReallyEqual(
                    attrs["permissions"], S_IFREG | 0o666
        ...
        d.addCallback(lambda ign: self.root.get("mutable2"))
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"small", sftp.FXF_READ | sftp.FXF_WRITE, {}
            )
        )
        ...
        def _read_write(rwf):
            ...
            d2.addCallback(
                lambda data: self.failUnlessReallyEqual(data, b"012345670123")
            )
            d2.addCallback(lambda ign: rwf.close())
            return d2

        d.addCallback(_read_write)
        d.addCallback(lambda ign: self.root.get("small"))
        d.addCallback(lambda node: download_to_data(node))
        d.addCallback(lambda data: self.failUnlessReallyEqual(data, b"012345670123"))

        # test WRITE and rename while still open
        d.addCallback(lambda ign: self.handler.openFile(b"small", sftp.FXF_WRITE, {}))
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"newexcl", sftp.FXF_WRITE | sftp.FXF_CREAT | sftp.FXF_EXCL, {}
            )
        )
        ...
        def _write_creat_excl_rename(wf):
            d2 = wf.writeChunk(0, b"abcd")
            d2.addCallback(
                lambda ign: self.handler.renameFile(b"newexcl", b"renamedexcl")
            )
            d2.addCallback(lambda ign: wf.writeChunk(4, b"efgh"))
            d2.addCallback(lambda ign: wf.close())
            return d2
        ...
        d.addCallback(lambda ign: self.root.get("renamedexcl"))
        d.addCallback(lambda node: download_to_data(node))
        d.addCallback(lambda data: self.failUnlessReallyEqual(data, b"abcdefgh"))
        d.addCallback(
            lambda ign: self.shouldFail(
                NoSuchChildError,
                "rename newexcl while open",
                "newexcl",
                self.root.get,
                "newexcl",
            )
        ...
        def _open_and_rename_race(ign):
            ...
            d2 = self.handler.openFile(
                b"new", sftp.FXF_WRITE | sftp.FXF_CREAT, {}, delay=slow_open
            )
        ...
        d.addCallback(lambda ign: self.root.get("new2"))
        d.addCallback(lambda node: download_to_data(node))
        d.addCallback(lambda data: self.failUnlessReallyEqual(data, b"abcd"))
        d.addCallback(
            lambda ign: self.shouldFail(
                NoSuchChildError, "rename new while open", "new", self.root.get, "new"
            )
        )
        ...
        gross = "gro\u00DF".encode("utf-8")
        d.addCallback(
            lambda ign: self.handler.openFile(gross, sftp.FXF_READ | sftp.FXF_WRITE, {})
        )
        ...
        def _read_write_broken(rwf):
            ...
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_FAILURE, "read/write broken", rwf.readChunk, 0, 100
                )
            )
            # close should fail in this case
            d2.addCallback(
                lambda ign: self.shouldFailWithSFTPError(
                    sftp.FX_FAILURE, "read/write broken close", rwf.close
                )
            )
            return d2
    ...
    def test_removeFile(self):
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE, "removefile ''", self.handler.removeFile, b""
            )
        )
        ...
        d.addCallback(lambda ign: self.root.get("gro\u00DF"))
        d.addCallback(lambda ign: self.handler.removeFile("gro\u00DF".encode("utf-8")))
        d.addCallback(
            lambda ign: self.shouldFail(
                NoSuchChildError, "removeFile gross", "gro", self.root.get, "gro\u00DF"
            )
        )

        # removing an unknown should succeed
        d.addCallback(lambda ign: self.root.get("unknown"))
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"tempfile", sftp.FXF_READ | sftp.FXF_WRITE | sftp.FXF_CREAT, {}
            )
        )
        ...
        d.addCallback(
            lambda ign: self.handler.openFile(
                b"tempfile2", sftp.FXF_READ | sftp.FXF_WRITE | sftp.FXF_CREAT, {}
            )
        )
        ...
        def _write_rename_remove(rwf):
            d2 = rwf.writeChunk(0, b"0123456789")
            d2.addCallback(
                lambda ign: self.handler.renameFile(b"tempfile2", b"tempfile3")
            )
    ...
    def test_removeDirectory(self):
        ...
        d.addCallback(lambda ign: self.root.get("tiny_lit_dir"))
        d.addCallback(lambda ign: self.handler.removeDirectory(b"tiny_lit_dir"))
        d.addCallback(
            lambda ign: self.shouldFail(
                NoSuchChildError,
                "removeDirectory tiny_lit_dir",
                "tiny_lit_dir",
                self.root.get,
                "tiny_lit_dir",
            )
        ...
        d.addCallback(
            lambda err: self.shouldFail(
                NoSuchChildError,
                "removeDirectory unknown",
                "unknown",
                self.root.get,
                "unknown",
            )
    ...
    def test_renameFile(self):
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "renameFile nofile newfile",
                self.handler.renameFile,
                b"nofile",
                b"newfile",
            )
        )
        ...
        def _rename_onto_heisenfile_race(wf):
            ...
            d2 = self.handler.openFile(
                b"heisenfile", sftp.FXF_WRITE | sftp.FXF_CREAT, {}, delay=slow_open
            )

            # deliberate race between openFile and renameFile
            d3 = self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "renameFile small heisenfile",
                self.handler.renameFile,
                b"small",
                b"heisenfile",
            )
        ...
        d.addCallback(lambda ign: self.root.get("new_small"))
        d.addCallback(
            lambda node: self.failUnlessReallyEqual(node.get_uri(), self.small_uri)
        )

        # renaming a file into a subdirectory should succeed (also tests Unicode names)
        d.addCallback(
            lambda ign: self.handler.renameFile(
                "gro\u00DF".encode("utf-8"), "loop/neue_gro\u00DF".encode("utf-8")
            )
        )
        d.addCallback(lambda ign: self.root.get("neue_gro\u00DF"))
        d.addCallback(
            lambda node: self.failUnlessReallyEqual(node.get_uri(), self.gross_uri)
        )

        # renaming a directory to a correct path should succeed
        d.addCallback(
            lambda ign: self.handler.renameFile(b"tiny_lit_dir", b"new_tiny_lit_dir")
        )
        d.addCallback(lambda ign: self.root.get("new_tiny_lit_dir"))
        d.addCallback(
            lambda node: self.failUnlessReallyEqual(
                node.get_uri(), self.tiny_lit_dir_uri
            )
        )
        ...
        d.addCallback(lambda ign: self.root.get("new_unknown"))
        d.addCallback(
            lambda node: self.failUnlessReallyEqual(node.get_uri(), self.unknown_uri)
        )
    ...
    def test_renameFile_posix(self):
        def _renameFile(fromPathstring, toPathstring):
            extData = (
                struct.pack(">L", len(fromPathstring))
                + fromPathstring
                + struct.pack(">L", len(toPathstring))
                + toPathstring
            )
            ...
            d2.addCallbacks(
                lambda res: self.fail(
                    "posix-rename request was supposed to "
        ...
        d.addCallback(lambda ign: self.root.set_node("loop2", self.root))
        d.addCallback(lambda ign: self.root.set_node("unknown2", self.unknown))

        # POSIX-renaming a non-existent file should fail
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "renameFile_posix nofile newfile",
                _renameFile,
                b"nofile",
                b"newfile",
            )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "renameFile_posix small nodir/small",
                _renameFile,
                b"small",
                b"nodir/small",
            )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "renameFile_posix small invalid",
                _renameFile,
                b"small",
                b"\xFF",
            )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "renameFile_posix small from uri",
                _renameFile,
                b"uri/" + self.small_uri,
                b"new",
            )
        ...
        d.addCallback(lambda ign: self.root.get("small2"))
        d.addCallback(
            lambda node: self.failUnlessReallyEqual(node.get_uri(), self.small_uri)
        )

        d.addCallback(lambda ign: _renameFile(b"small2", b"loop2"))
        d.addCallback(lambda ign: self.root.get("loop2"))
        d.addCallback(
            lambda node: self.failUnlessReallyEqual(node.get_uri(), self.small_uri)
        )

        d.addCallback(lambda ign: _renameFile(b"loop2", b"unknown2"))
        d.addCallback(lambda ign: self.root.get("unknown2"))
        d.addCallback(
            lambda node: self.failUnlessReallyEqual(node.get_uri(), self.small_uri)
        )
    ...
    def test_makeDirectory(self):
        ...
        d.addCallback(
            lambda ign: self.handler.makeDirectory(
                b"newdir", {"ext_foo": "bar", "ctime": 42}
            )
        )

        d.addCallback(lambda ign: self.root.get_child_and_metadata("newdir"))
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_NO_SUCH_FILE,
                "makeDirectory invalid UTF-8",
                self.handler.makeDirectory,
                b"\xFF",
                {},
            )
        )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_PERMISSION_DENIED,
                "makeDirectory newdir2 permissions:0444 denied",
                self.handler.makeDirectory,
                b"newdir2",
                {"permissions": 0o444},
            )
    ...
    def test_execCommand_and_openShell(self):
        ...
        def _exec_df(session):
            ...
            d2.addCallback(
                lambda ign: self.failUnlessIn("1024-blocks", protocol.output)
            )
            d2.addCallback(
                lambda ign: self.failUnless(
                    _lines_end_in_crlf(protocol.output), protocol.output
                )
            )
            d2.addCallback(lambda ign: self.failUnlessEqual(protocol.error, ""))
            d2.addCallback(
                lambda ign: self.failUnless(
                    isinstance(protocol.reason.value, ProcessDone)
                )
            )
        ...
        def _check_unsupported(protocol):
            ...
            d2.addCallback(
                lambda ign: self.failUnlessIn("only the SFTP protocol", protocol.error)
            )
            d2.addCallback(
                lambda ign: self.failUnless(
                    _lines_end_in_crlf(protocol.error), protocol.error
                )
            )
            d2.addCallback(
                lambda ign: self.failUnless(
                    isinstance(protocol.reason.value, ProcessTerminated)
                )
            )
            d2.addCallback(
                lambda ign: self.failUnlessEqual(protocol.reason.value.exitCode, 1)
            )
            return d2
    ...
    def test_extendedRequest(self):
        ...
        d.addCallback(
            lambda ign: self.handler.extendedRequest(b"statvfs@openssh.com", b"/")
        )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_OP_UNSUPPORTED,
                "extendedRequest foo bar",
                self.handler.extendedRequest,
                b"foo",
                b"bar",
            )
        )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_BAD_MESSAGE,
                "extendedRequest posix-rename@openssh.com invalid 1",
                self.handler.extendedRequest,
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_BAD_MESSAGE,
                "extendedRequest posix-rename@openssh.com invalid 2",
                self.handler.extendedRequest,
                b"posix-rename@openssh.com",
                b"\x00\x00\x00\x01",
            )
        ...
        d.addCallback(
            lambda ign: self.shouldFailWithSFTPError(
                sftp.FX_BAD_MESSAGE,
                "extendedRequest posix-rename@openssh.com invalid 3",
                self.handler.extendedRequest,
                b"posix-rename@openssh.com",
                b"\x00\x00\x00\x01_\x00\x00\x00\x01",
            )
        )
