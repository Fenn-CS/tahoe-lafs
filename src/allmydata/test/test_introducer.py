...
class Node(testutil.SignalMixin, testutil.ReallyEqualMixin, AsyncTestCase):
    ...
    def test_introducer_clients_unloadable(self):
        ...
        yaml_fname = os.path.join(basedir, "private", "introducers.yaml")
    ...
    def test_web_static(self):
        basedir = "introducer.Node.test_web_static"
        create_node_dir(basedir, "testing")
        fileutil.write(
...
NICKNAME = "n\u00EDickname-%s"  # LATIN SMALL LETTER I WITH ACUTE
    ...
    def do_system_test(self):
        ...
        def _check1(res):
            ...
            self.failUnlessEqual(
                dc["outbound_announcements"], NUM_STORAGE * NUM_CLIENTS
            )
        ...
        def _restart_introducer_tub(_ign):
            ...
            newfurl = self.central_tub.registerReference(
                self.the_introducer, furlFile=iff
            )
...
class FakeRemoteReference(object):
    ...
    def getRemoteTubID(self):
        return "62ubehyunnyhzs7r6vdonnm2hpi52w6y"

    def getLocationHints(self):
        return ["tcp:here.example.com:1234", "tcp:there.example.com2345"]

    def getPeer(self):
        return address.IPv4Address("TCP", "remote.example.com", 3456)
...
class ClientInfo(AsyncTestCase):
    def test_client_v2(self):
        ...
        introducer.remote_subscribe_v2(
            subscriber, "storage", client_v2._my_subscriber_info
        )
        subs = introducer.get_subscribers()
        self.failUnlessEqual(len(subs), 1)
        s0 = subs[0]
        self.failUnlessEqual(s0.service_name, "storage")
        self.failUnlessEqual(s0.nickname, NICKNAME % "v2")
...
class Announcements(AsyncTestCase):
    def test_client_v2_signed(self):
        ...
        self.failUnlessEqual(a[0].nickname, "nick-v2")
    ...
    def test_client_cache(self):
        ...
        self.failUnlessEqual(
            ensure_binary(announcements[public_key_str]["anonymous-storage-FURL"]),
            furl2,
        )
        self.failUnlessEqual(
            ensure_binary(announcements[public_key_str2]["anonymous-storage-FURL"]),
            furl3,
        )
        ...
        yield flushEventualQueue()
        self.assertEqual(
            c2.storage_broker.get_all_serverids(),
...
class ClientSeqnums(AsyncBrokenTestCase):
    ...
    def test_client(self):
        ...
        self.failUnlessEqual(json.loads(published["sA"][0]), outbound["sA"])
        ...
        self.failUnlessEqual(json.loads(published["sA"][0]), outbound["sA"])
        self.failUnlessEqual(json.loads(published["sB"][0]), outbound["sB"])


class TooNewServer(IntroducerService):
    VERSION = {
        "http://allmydata.org/tahoe/protocols/introducer/v999": {},
...
class NonV1Server(SystemTestMixin, AsyncTestCase):
    ...
    def test_failure(self):
        ...
        def _done(res):
            ...
            self.failUnless(
                c._introducer_error.check(InsufficientVersionError), c._introducer_error
            )
...
class DecodeFurl(SyncTestCase):
    def test_decode(self):
        ...
        self.failUnlessEqual(
            nodeid, b"\x9fM\xf2\x19\xcckU0\xbf\x03\r\x10\x99\xfb&\x9b-\xc7A\x1d"
        )
...
class Signatures(SyncTestCase):
    def test_sign(self):
        ...
        # not signed
        self.failUnlessRaises(UnknownKeyError, unsign_from_foolscap, (msg, None, key))
        self.failUnlessRaises(UnknownKeyError, unsign_from_foolscap, (msg, sig, None))
        # bad signature
        bad_ann = {"key1": "value2"}
        bad_msg = json.dumps(bad_ann).encode("utf-8")
        self.failUnlessRaises(BadSignature, unsign_from_foolscap, (bad_msg, sig, key))

        # unrecognized signatures
        self.failUnlessRaises(
            UnknownKeyError, unsign_from_foolscap, (bad_msg, b"v999-sig", key)
        )
        self.failUnlessRaises(
            UnknownKeyError, unsign_from_foolscap, (bad_msg, sig, b"v999-key")
        )

    def test_unsigned_announcement(self):
        ed25519.verifying_key_from_string(
            b"pub-v0-wodst6ly4f7i7akt2nxizsmmy2rlmer6apltl56zctn67wfyu5tq"
        )
        mock_tub = Mock()
        ic = IntroducerClient(
            mock_tub,
