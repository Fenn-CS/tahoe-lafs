...
def _populate_attrs(childnode, metadata, size=None):
    ...
    if childnode and childnode.is_unknown():
        ...
        if size is not None:
            _assert(
                isinstance(size, (int, long)) and not isinstance(size, bool), size=size
            )
            attrs["size"] = size
        perms = S_IFREG | 0o666
...
class OverwriteableFileConsumer(PrefixingLogMixin):
    ...
    def _update_downloaded(self, new_downloaded):
        ...
        while len(self.milestones) > 0:
            ...
            if next > milestone:
                return
            if noisy:
                self.log("MILESTONE %r %r" % (next, d), level=NOISY)
    ...
    def download_done(self, res):
        ...
        while len(self.milestones) > 0:
            (next, d) = self.milestones[0]
            if noisy:
                self.log("MILESTONE FINISH %r %r %r" % (next, d, res), level=NOISY)
            heapq.heappop(self.milestones)
...
class ShortReadOnlySFTPFile(PrefixingLogMixin):
    ...
    def __init__(self, userpath, filenode, metadata):
        ...
        precondition(
            isinstance(userpath, str) and IFileNode.providedBy(filenode),
            userpath=userpath,
            filenode=filenode,
        )
        ...
        def _read(data):
            ...
            if offset >= len(data):
                eventually_errback(d)(
                    Failure(SFTPError(FX_EOF, "read at or past end of file"))
                )
            else:
...
class GeneralSFTPFile(PrefixingLogMixin):
    ...
    def rename(self, new_userpath, new_parent, new_childname):
        ...
        precondition(
            isinstance(new_userpath, str) and isinstance(new_childname, unicode),
...
class SFTPUserHandler(ConchUser, PrefixingLogMixin):
    ...
    def _rename_heisenfiles(
        ...
        precondition(
            (
                isinstance(from_userpath, str)
                and isinstance(from_childname, unicode)
                and isinstance(to_userpath, str)
                and isinstance(to_childname, unicode)
            ),
        ...
        if not overwrite and (
            ...
            def _existing():
                raise SFTPError(
                    FX_PERMISSION_DENIED,
                    "cannot rename to existing path " + to_userpath,
                )
    ...
    def _update_attrs_for_heisenfiles(self, userpath, direntry, attrs):
        ...
        _assert(
            isinstance(userpath, str) and isinstance(direntry, str),
            userpath=userpath,
            direntry=direntry,
        )
    ...
    def _sync_heisenfiles(self, userpath, direntry, ignore=None):
        ...
        _assert(
            isinstance(userpath, str) and isinstance(direntry, (str, NoneType)),
            userpath=userpath,
            direntry=direntry,
        )
    ...
    def _remove_heisenfile(self, userpath, parent, childname, file_to_remove):
        ...
        _assert(
            isinstance(userpath, str) and isinstance(childname, (unicode, NoneType)),
            userpath=userpath,
            childname=childname,
        )
    ...
    def _make_file(
        ...
        _assert(
            (
                isinstance(userpath, str)
                and isinstance(childname, (unicode, NoneType))
        ...
        def _got_root(root_and_path):
            ...
            if not path:
                ...
                if not IFileNode.providedBy(root):
                    raise SFTPError(FX_PERMISSION_DENIED, "cannot open a directory cap")
    ...
    def extendedRequest(self, extensionName, extensionData):
        ...
        if (
            extensionName == "statvfs@openssh.com"
            or extensionName == "fstatvfs@openssh.com"
        ):
...
class SFTPServer(service.MultiService):
    ...
    def __init__(
        self, client, accountfile, accounturl, sftp_portstr, pubkey_file, privkey_file
    ):
