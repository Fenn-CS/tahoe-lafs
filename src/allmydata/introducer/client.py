...
class IntroducerClient(service.Service, Referenceable):
    ...
    def _load_announcements(self):
        ...
        for server_params in servers:
            if not isinstance(server_params, dict):
                log.err(
                    InvalidCacheError("not a dict: %r" % (server_params,)),
                    level=log.WEIRD,
                )
                continue
    ...
    def _got_introducer(self, publisher):
        ...
        default = {
            b"http://allmydata.org/tahoe/protocols/introducer/v1": {},
    ...
    def _maybe_subscribe(self):
        if not self._publisher:
            self.log("want to subscribe, but no introducer yet", level=log.NOISY)
            return
    ...
    def got_announcements(self, announcements, lp=None):
        ...
        for ann_t in announcements:
            try:
                ...
                self.log(
                    "bad signature on inbound announcement: %s" % (ann_t,),
                    parent=lp,
                    level=log.WEIRD,
                    umid="ZAU15Q",
                )
    ...
    def _process_announcement(self, ann, key_s):
        ...
        nick_s = ann.get("nickname", "").encode("utf-8")
        lp2 = self.log(
            format="announcement for nickname '%(nick)s', service=%(svc)s: %(ann)s",
            nick=nick_s,
            svc=service_name,
            ann=ann,
            umid="BoKEag",
        ...
        if (
            index in self._inbound_announcements
            and self._inbound_announcements[index][0] == ann
        ):
            self.log(
                format="reannouncement for [%(service)s]:%(description)s, ignoring",
        ...
        if index in self._inbound_announcements:
            ...
            if "seqnum" in old:
                ...
                if "seqnum" not in ann or not isinstance(ann["seqnum"], (int, long)):
                    self.log(
                        "not replacing old announcement, no valid seqnum: %s" % (ann,),
                        parent=lp2,
                        level=log.NOISY,
                        umid="zFGH3Q",
                    )
                ...
                if ann["seqnum"] <= old["seqnum"]:
                    ...
                    self.log(
                        "not replacing old announcement, "
                        "new seqnum is too old (%s <= %s) "
                        "(replay attack?): %s" % (ann["seqnum"], old["seqnum"], ann),
                        parent=lp2,
                        level=log.UNUSUAL,
                        umid="JAAAoQ",
                    )
            ...
            self.log(
                "replacing old announcement: %s" % (ann,),
                parent=lp2,
                level=log.NOISY,
                umid="wxwgIQ",
            )
            ...
            self.log("new announcement[%s]" % service_name, parent=lp2, level=log.NOISY)
    ...
    def connection_status(self):
        ...
        last_received = (
            self._publisher.getDataLastReceivedAt() if self._publisher else None
        )
