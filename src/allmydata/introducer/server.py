...
def create_introducer(basedir="."):
    ...
    try:
        ...
        ) = create_connection_handlers(config, i2p_provider, tor_provider)
...
class _IntroducerNode(node.Node):
    ...
    def init_introducer(self):
        ...
        old_public_fn = self.config.get_config_path("introducer.furl")
        private_fn = self.config.get_private_path("introducer.furl")
...
class IntroducerService(service.MultiService, Referenceable):
    ...
    def get_subscribers(self):
        ...
        for service_name, subscriptions in list(self._subscribers.items()):
            for rref, (subscriber_info, when) in list(subscriptions.items()):
                ...
                nickname = subscriber_info.get("nickname", "?")
                version = subscriber_info.get("my-version", "?")
    ...
    def remote_subscribe_v2(self, subscriber, service_name, subscriber_info):
        self.log(
            "introducer: subscription[%r] request at %r" % (service_name, subscriber),
            umid="U3uzLg",
        )
